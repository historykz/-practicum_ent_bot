...
from handlers import (common, profile, user, quiz, duel,
                       ...
                       backup,
    restore,
    subscription_watch, notes_promo, reminders_admin, account_bot, zip_import, payments, announce,
    study_admin,
...
async def main() -> None:
    ...
    from handlers import broadcast as _broadcast
    dp.include_router(_broadcast.router)  # глобальная рассылка — раньше роутеров, ловящих любой текст
    from handlers import rewards_admin as _rewards_admin
    dp.include_router(_rewards_admin.router)  # вознаграждения: суммы и причины вводятся текстом
    ...
    dp.include_router(study_admin.router)   # контроль обучения
    ...
    dp.include_router(account_bot.router)       # пароль от сайта
    dp.include_router(notes_promo.router)      # кнопка «КОНСПЕКТЫ ЕНТ» — без Премиума
    dp.include_router(reminders_admin.router)  # тексты и автонапоминания
    ...
    bot._smartent_dp = dp   # рассыльщику нужен доступ к FSM-хранилищу
    asyncio.create_task(_restore_cleanup_loop())
    from handlers.notes_promo import reminder_loop as _rl
    asyncio.create_task(_rl(bot))
    ...
    # Контроль обучения: мягкие и строгие напоминания, отчёт админу
    from services import study_reminders as _sre
    asyncio.create_task(_sre.loop(bot))

    # Вознаграждения по предметам: уведомления, штрафы за пропуски, потеря Премиума
    from services import reward_service as _rws
    asyncio.create_task(_rws.loop(bot))

    # На сервер дошла только часть файлов новой версии — сразу сказать админам,
    # какие именно (webapp/build_check.py). Иначе это видно лишь по ошибкам.
    try:
        import config as _cfg
        from webapp import build_check as _bchk
        _note = _bchk.summary()
        if _note:
            for _aid in getattr(_cfg, "ADMIN_IDS", None) or []:
                try:
                    await bot.send_message(_aid, _note, parse_mode=None)
                except Exception:
                    pass
    except Exception as e:
        log.warning("проверка версии файлов: %s", e)

    # Глобальная рассылка: продолжить отправку или удаление, прерванные перезапуском
    from services import broadcast_service as _bcs
    asyncio.create_task(_bcs.resume_all(bot))

    # Уборка брошенных попыток теста — без неё счётчик «сейчас проходят»
    # только растёт, особенно после каждого перезапуска бота
    from services import stats_service as _sts
    asyncio.create_task(_sts.stale_attempts_loop())
