...
async def daily_backup_loop(bot):
    ...
    while True:
        try:
            ...
            # === Бэкап САЙТА (та же база + uploads: предметы, уроки, конспекты,
            # доступы, результаты) — шлём отдельным файлом вместе с частями бота ===
            try:
                await _send_site_backup(bot, admin_id, today)
            except Exception as e:
                log.warning("site backup send: %s", e)
        except Exception as e:
