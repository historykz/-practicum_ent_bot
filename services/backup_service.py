...
def backup_counts() -> dict:
    ...
    return {
        ...
        "subjects": cnt("SELECT COUNT(*) AS c FROM subjects"),
        "lessons": cnt("SELECT COUNT(*) AS c FROM lessons WHERE "
                       "COALESCE(content_html,'') <> '' OR id IN "
                       "(SELECT lesson_id FROM lesson_images)"),
    }
...
def _copy_marks_snapshot():
    """{урок-копия: (отметка, отпечаток теста)} и {предмет-копия: отметка}."""
    try:
        from services import copy_compare as cc
        lessons = {}
        for r in db.fetchall("SELECT l.id, l.test_id, l.copy_modified_at, s.subject_id FROM lessons l "
                             "JOIN sections s ON s.id=l.section_id "
                             "WHERE l.copied_from_lesson_id IS NOT NULL AND l.test_id IS NOT NULL"):
            lessons[r["id"]] = (r["copy_modified_at"], cc.test_fingerprint(r["test_id"]), r["subject_id"])
        subjects = {r["id"]: r["copy_modified_at"] for r in db.fetchall(
            "SELECT id, copy_modified_at FROM subjects WHERE copied_from_subject_id IS NOT NULL")}
        return {"lessons": lessons, "subjects": subjects}
    except Exception as e:
        log.warning("отметки копий перед восстановлением: %s", e)
        return None


def _copy_marks_restore(marks) -> None:
    try:
        from services import copy_compare as cc
        changed_subjects = set()
        for lid, (mark, fp, sid) in marks["lessons"].items():
            row = db.fetchone("SELECT test_id FROM lessons WHERE id=?", (lid,))
            if row and cc.test_fingerprint(row["test_id"]) == fp:
                db.execute("UPDATE lessons SET copy_modified_at=? WHERE id=?", (mark, lid))
            else:
                changed_subjects.add(sid)
        for sid, mark in marks["subjects"].items():
            if sid not in changed_subjects:
                db.execute("UPDATE subjects SET copy_modified_at=? WHERE id=?", (mark, sid))
    except Exception as e:
        log.warning("отметки копий после восстановления: %s", e)
...
async def restore_backup(bot, zip_path, mode: str = "replace") -> dict:
    ...
    # Отметки «Копия изменена» (v69): пересоздание тестов ниже сработало бы как
    # правка каждой копии. Запоминаем отметки и отпечатки тестов копий — после
    # восстановления вернём отметку там, где тест на деле не изменился.
    marks = _copy_marks_snapshot() if mode == "replace" else None
...
async def _send_site_backup(bot, admin_id, today):
    """Бэкап сайта (bot.db + uploads/) — тот же формат, что кнопка «Скачать
    полный бэкап» в админке. Большой файл режем на части ≤19МБ."""
    import asyncio
    import os as _os
    from aiogram.types import FSInputFile
    from webapp import learning as _lrn

    zip_path = await asyncio.to_thread(_lrn._build_backup_zip_sync)
    size = _os.path.getsize(zip_path)
    limit = MAX_PART_BYTES  # 19МБ — лимит Telegram

    if size <= limit:
        cap = (f"🌐 <b>Бэкап САЙТА за {today}</b> ({size / 1024 / 1024:.1f} МБ)\n\n"
               "Предметы, уроки, конспекты, зачёты, доступы, результаты, файлы.\n"
               "Восстановление: админка сайта → «Восстановить из бэкапа».")
        await bot.send_document(admin_id, FSInputFile(zip_path), caption=cap,
                                 parse_mode="HTML")
        try:
            _os.remove(zip_path)
        except OSError:
    ...
    # Режем zip на части (склеить обратно: cat part1 part2 ... > backup.zip)
    parts = []
    ...
    for i, p in enumerate(parts, 1):
        mb = _os.path.getsize(p) / 1024 / 1024
        cap = (f"🌐 <b>Бэкап САЙТА за {today} — часть {i} из {n}</b> ({mb:.1f} МБ)\n\n"
               f"⚠️ Сохрани ВСЕ {n} частей. Склеить в один файл:\n"
               f"<code>cat *.part1 *.part2 … &gt; backup.zip</code>")
        await bot.send_document(admin_id, FSInputFile(p), caption=cap, parse_mode="HTML")
        try:
            _os.remove(p)
        except OSError:
            pass
...
async def daily_backup_loop(bot):
    ...
    while True:
        try:
            # Одна общая копия платформы (база сайта и бота + файлы),
            # нарезанная на части ≤19 МБ. Отдельного бэкапа сайта больше нет —
            # раньше приходили два разных архива и их путали при восстановлении.
            import asyncio as _aio
            from webapp import learning as _lg
            from services import restore_service as _rs

            def _build():
                zip_path = _lg._build_backup_zip_sync()
                return zip_path, _rs.split_backup(zip_path)

            zip_path, parts = await _aio.to_thread(_build)
            import os as _os
            from datetime import datetime as _dt
            today = _dt.now().strftime("%d.%m.%Y")
            n = len(parts)
            for i, path in enumerate(parts, 1):
                size = _rs.human_size(_os.path.getsize(path))
                if n == 1:
                    cap = (f"🗓 <b>Авто-бэкап за {today}</b> ({size})\n\n"
                           "Полная копия платформы: предметы, уроки, конспекты, "
                           "тесты, пользователи, доступы, подписки, платежи, "
                           "настройки — и сайта, и бота.")
                else:
                    cap = (f"🗓 <b>Авто-бэкап за {today} — часть {i} из {n}</b> "
                           f"({size})\n\n"
                           f"⚠️ Сохраните ВСЕ {n} частей — для восстановления нужны все.")
