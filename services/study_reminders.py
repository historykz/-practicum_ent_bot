...
def candidates() -> list:
    """Ученики, за которыми следим.

    Если подключены предметы под контролем — это их ученики (доступ выдан
    именно на предмет или общий Премиум, который предмет принимает). Пока
    ни один предмет не подключён — как раньше: все с активным Премиумом.
    """
    st.ensure_schema()
    premium_only = ss.get_bool("study_premium_only")
    ids = []
    if sj.tracked():
        sj.sync_all(max_age_seconds=1200)      # новые Премиумы попадают сами
        ids = sj.tracked_tg_ids()
    elif premium_only:
        rows = db.fetchall(
    ...
    if not premium_only:
        ...
        seen = set(ids)
        ids += [r["tg_id"] for r in rows if r["tg_id"] not in seen]
    return ids


# ---------- Что именно написать ----------

def _plan_reminder_due(tg_id: int) -> bool:
    """Про отставание от плана напоминаем не чаще, чем задал админ."""
    days = ss.get_int("study_plan_repeat_days", 3)
    try:
        row = db.fetchone(
            "SELECT sent_at FROM study_notifications WHERE tg_id=? AND kind='behind_plan' "
            "ORDER BY id DESC LIMIT 1", (tg_id,))
    except Exception:
        return True
    if not row:
        return True
    passed = st.days_since(row["sent_at"])
    return passed is None or passed >= days
...
def decide(data: dict) -> tuple:
    ...
    Сначала — лестница «сколько дней не занимается». Если по ней писать
    нечего, но по учебному плану предмета человек отстаёт (занимается, но
    медленно), уходит письмо про план.
    """
    key, level = _decide_idle(data)
    if key:
        return key, level
    plan = data.get("plan") or {}
    if plan.get("status") in ("behind", "far") and plan.get("lag", 0) > 0 \
            and data.get("tg_id") and _plan_reminder_due(data["tg_id"]):
        return "behind_plan", data.get("warn_level") or 0
    return None, 0


def _decide_idle(data: dict) -> tuple:
...
async def run_once(bot) -> dict:
    ...
    sent = 0
    praised = 0
    # Предметы под контролем: подтянуть новых Премиум-учеников и пересчитать
    # прогресс до того, как решать, кому писать
    try:
        sj.sync_all(max_age_seconds=1200)
    except Exception as e:
        log.warning("синхронизация предметов: %s", e)
    people = candidates()
    for tg_id in people:
        try:
...
def summary() -> dict:
    ...
    groups = {"d2": [], "d3": [], "d4": [], "reading_no_hw": [], "never": [], "plan": []}
    d1 = ss.get_int("study_warn1_days", 2)
    d2 = ss.get_int("study_warn2_days", 3)
    d3 = ss.get_int("study_warn3_days", 4)
    for tg_id in candidates():
        data = st.profile(tg_id)
        if data.get("plan_status") in ("behind", "far"):
            groups["plan"].append(data)
        idle = data.get("days_idle")
        if idle is None:
...
def summary_text(groups: dict = None) -> str:
    ...
    return (
        ...
        f"📉 Отстают от учебного плана — <b>{len(groups.get('plan') or [])}</b>"
        + _subjects_line()
    )


def _subjects_line() -> str:
    try:
        tot = sj.totals()
    except Exception:
        return ""
    if not tot.get("subjects"):
        return ("\n\n<i>Предметы под контролем не подключены — слежение идёт "
                "по общему Премиуму. Подключите предмет по ссылке платформы.</i>")
    return (f"\n\n📚 Предметов под контролем: <b>{tot['subjects']}</b> · "
            f"учеников: <b>{tot['students']}</b>")
