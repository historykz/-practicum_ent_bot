...
async def maybe_give_reward(bot: Bot, inviter_tg_id: int) -> int:
    ...
    # Инструкция после Премиума за друзей НЕ отправляется: это бонус тому,
    # кто платформой уже пользуется. Ему достаточно сообщения о награде ниже.
...
def _grant_premium(user_id: int, days: int):
    """Выдать/продлить премиум пользователю (звёзды, награда за друзей).

    Срок прибавляется к текущему, если тот ещё идёт. Бессрочный Премиум не
    трогаем: раньше покупка или награда у такого человека заменяла
    «навсегда» на N дней.
    """
    import utils as _u
    existing = db.fetchone(
        "SELECT expires_at FROM premium_users WHERE user_id=?", (user_id,))
    if existing and not str(existing.get('expires_at') or '').strip():
        logging.getLogger(__name__).info(
            "user_id=%s с бессрочным Премиумом — срок не меняем", user_id)
        new_exp = None
    else:
        now = datetime.utcnow()
        cur_exp = _u.parse_utc_naive(existing['expires_at']) if existing else None
        base = max(now, cur_exp) if cur_exp else now
        new_exp = _u.to_utc_iso(base + timedelta(days=days))   # единый формат срока
    db.execute(
    ...
    try:
        ...
        log_premium_grant(user_id, "referral", days, 0, "", 0,
                          "награда за приглашённых друзей", new_exp)
    except Exception:
        pass
