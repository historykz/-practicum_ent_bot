...
def _fetch_test(test_id: int, user_tg_id: Optional[int] = None) -> Optional[dict]:
    row = db.fetchone(
        "SELECT * FROM tests WHERE id=? AND status='active' "
        "AND COALESCE(is_private,0)=0", (test_id,))
    if not row:
        return None
    test = dict(row)
    # Раздел теста тоже может быть закрыт — той же проверкой, что и на
    # запуске теста (handlers/user.py show_test_card/cb_run_test). Иначе
    # прямая ссылка test:<id> в inline-режиме отдавала бы название, тему
    # и число вопросов теста из закрытого раздела кому угодно.
...
def build_inline_results(query: str, user_lang: Optional[str],
    ...
    if q.lower().startswith("test:"):
        ...
        if rest.isdigit():
            test = _fetch_test(int(rest), user_tg_id)
            if test:
