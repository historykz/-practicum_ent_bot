    ...
    for q in questions:
        try:
            ...
            for i, opt_text in enumerate(q["options"]):
                # Правильных может быть несколько — сохраняем все отмеченные
                correct_set = q.get("correct_indexes") or [q.get("correct_index", 0)]
                is_correct = 1 if i in correct_set else 0
                db.execute(
