...
def _pick_questions(count: int, lang: str = 'ru', category_id=None) -> list[int]:
    ...
    # Вопросы закрытых разделов в дуэль не берём даже в режиме «все разделы»:
    # иначе приватный материал утекал бы через случайные вопросы.
    rows = db.fetchall(f"""
...
def get_duel_categories(lang: str = 'ru') -> list:
    ...
    rows = db.fetchall("""
        ...
        WHERE t.status='active' AND t.is_paid=0
          ...
          AND COALESCE(c.is_private,0)=0      -- закрытые разделы в дуэли не зовём
