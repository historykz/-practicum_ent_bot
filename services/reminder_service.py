...
# --- сообщения после покупки Премиума ---
PREMIUM_OK = "premium_ok"
PREMIUM_QUIZLET = "premium_quizlet"

DEFAULT_PREMIUM_OK_TEXT = (
    ...
    "🔓 Платных тестов открыто: {tests}\n"
    "Выберите тест, чтобы начать:")

DEFAULT_QUIZLET_TEXT = (
    "💎 <b>Вы приобрели Premium-доступ!</b>\n\n"
    "Чтобы получить карточки Quizlet, заполните короткую форму по кнопке ниже.")

DEFAULT_QUIZLET_URL = "https://forms.gle/yKmabQ599AHYS2qM9"

_DEFAULTS = {
    MANUAL: (DEFAULT_MANUAL_TEXT, 1),
    REMINDER: (DEFAULT_REMINDER_TEXT, 0),   # автонапоминания по умолчанию выключены
    PREMIUM_OK: (DEFAULT_PREMIUM_OK_TEXT, 1),
    PREMIUM_QUIZLET: (DEFAULT_QUIZLET_TEXT, 1),
}

# Своя подпись кнопок и ссылки у каждого сообщения
_BUTTON_DEFAULTS = {
    PREMIUM_OK: ("📚 КОНСПЕКТЫ ЕНТ", DEFAULT_URL,
                 "🔓 ОТКРЫТЬ ПЛАТНЫЕ ТЕСТЫ", "tests"),
    PREMIUM_QUIZLET: ("🃏 ПОЛУЧИТЬ КАРТОЧКИ QUIZLET", DEFAULT_QUIZLET_URL, "", ""),
}
...
def get_campaign(key: str) -> dict:
    ...
    b1t, b1u, b2t, b2u = _BUTTON_DEFAULTS.get(key, (DEFAULT_BUTTON, DEFAULT_URL, "", ""))
    db.execute(
        "INSERT INTO reminder_campaigns (campaign_key, version, enabled, message_text, "
        "button_text, button_url, button2_text, button2_url, safe_delay_seconds) "
        "VALUES (?,1,?,?,?,?,?,?,?)",
        (key, enabled, text, b1t, b1u, b2t, b2u,
         2 if key in (PREMIUM_OK, PREMIUM_QUIZLET) else 600))
...
def update_campaign(key: str, **fields) -> dict:
    ...
    allowed = ("enabled", "message_text", "button_text", "button_url",
               "button2_text", "button2_url", "cooldown_seconds",
               "safe_delay_seconds", "sort_order")
    sets, args = [], []
    for f in allowed:
