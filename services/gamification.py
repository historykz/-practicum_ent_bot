...
def invalidate(subject_id=None) -> None:
    ...
    if subject_id is None:
        _cache.clear()
        return
    for k in [k for k in _cache if isinstance(k, tuple) and k[-1] == int(subject_id)]:
        _cache.pop(k, None)
...
def _points_by_user(subject_id: int) -> dict:
    ...
    """
    tids = subject_test_ids(subject_id)
    if not tids:
        return {}
    ph = ",".join("?" * len(tids))
    rows = db.fetchall(
        f"SELECT user_id, SUM(best) AS pts FROM ("
        f"  SELECT user_id, test_id, MAX(COALESCE(score,0)) AS best"
        f"  FROM test_attempts"
        f"  WHERE test_id IN ({ph}) AND status='finished'"
        f"    AND COALESCE(publication_id,0)=0 AND COALESCE(attempt_num,1)<>999"
        f"  GROUP BY user_id, test_id"
        f") GROUP BY user_id", tuple(tids))
    return {r["user_id"]: int(r["pts"] or 0) for r in rows}
...
def leaderboard(subject_id: int) -> list:
    """[(user_id, баллы)] по убыванию — ВСЕ ученики предмета, включая тех, у
    кого баллов пока нет.

    Раньше сюда попадали только успевшие набрать баллы, и человек видел
    «2 место из 3», хотя предмет проходят сорок с лишним Премиум-учеников:
    знаменатель считался по трём, кто уже сдал хотя бы одно ДЗ. Соперники —
    это все, кому предмет открыт, а не только вырвавшиеся вперёд.
    """
    real = int(sc.orig_subject_id(int(subject_id)))

    def _build():
        pts = _points_by_user(real)
        students = _students_of(real)
        rows = [(uid, int(pts.get(uid, 0))) for uid in students]
...
def position(user_id: int, subject_id: int) -> dict:
    """Место среди учеников предмета: {place, total, points, to_next}.

    Место спортивное: сколько человек строго выше по баллам, плюс один.
    Одинаковые баллы — одинаковое место. Пока баллов нет, места тоже нет
    (place=0) — в карточке на этом месте прочерк, а не «последний из сорока».
    """
    board = leaderboard(subject_id)
    my = points(user_id, subject_id)
    mine_in = any(uid == user_id for uid, _p in board)
    higher = [p for _uid, p in board if p > my]
    place = (len(higher) + 1) if (mine_in and my > 0) else 0
    to_next = (min(higher) - my) if higher else 0
...
def award(tg_id: int, user_id: int = None) -> list:
    ...
    row = db.fetchone(
        "SELECT COUNT(DISTINCT test_id) AS n, MAX(CASE WHEN (correct_answers+wrong_answers)>0 "
        "THEN correct_answers*100.0/(correct_answers+wrong_answers) ELSE 0 END) AS best "
        "FROM test_attempts WHERE user_id=? AND status='finished' "
        "AND COALESCE(publication_id,0)=0", (user_id,))
    ...
    total_points = 0
    for s in _user_subjects(tg_id, user_id):
...
def _user_subjects(tg_id: int, user_id: int) -> list:
    ...
    try:
        rows = db.fetchall(
            "SELECT id, title, pass_percent FROM subjects WHERE status='active'")
    except Exception:
        return out
    for s in rows:
        sid = s["id"]
...
def overall(tg_id: int, user_id: int) -> dict:
    ...
    return {
        ...
        "lessons_done": sum(s["lessons_done"] for s in subjects),
        "lessons_total": sum(s["lessons_total"] for s in subjects),
        "achievements_earned": len(earned(user_id)),
        "achievements_total": TOTAL_ACHIEVEMENTS,
...
def top_opened(tg_id: int, subject_id=None, limit: int = 8) -> list:
    ...
    try:
        rows = db.fetchall(
            ...
            f"GROUP BY lesson_id ORDER BY n DESC, lesson_id LIMIT ?",
            tuple(params + [limit]))
    except Exception:
        return []
    out = []
    for r in rows:
        ...
        out.append({"lesson_id": r["lesson_id"],
                    "title": (les["title"] if les else f"Урок {r['lesson_id']}"),
                    "opens": r["n"]})
    return out
...
def admin_subject_stats(tg_id: int, user_id: int, subject_id: int) -> dict:
    ...
    tids = subject_test_ids(real)
    tests_done = attempts = 0
    if tids:
        ph = ",".join("?" * len(tids))
        row = db.fetchone(
            f"SELECT COUNT(DISTINCT test_id) AS d, COUNT(*) AS a FROM test_attempts "
            f"WHERE user_id=? AND test_id IN ({ph}) AND status='finished' "
            f"AND COALESCE(publication_id,0)=0", (user_id, *tids))
        tests_done = int((row or {}).get("d") or 0)
