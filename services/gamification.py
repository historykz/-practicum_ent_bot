...
def _participant_ids(subject_id: int) -> list:
    """Участники рейтинга предмета — кто нажал «✨ НАЧАТЬ ОБУЧЕНИЕ ✨» и не
    потерял доступ (services/reward_service.py, таблица subject_participants)."""
    try:
        rows = db.fetchall("SELECT user_id FROM subject_participants "
                           "WHERE subject_id=? AND is_active=1", (int(subject_id),))
    except Exception:
        return []
