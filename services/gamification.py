...
def leaderboard(subject_id: int) -> list:
    """[(user_id, баллы)] по убыванию — только ученики предмета с баллами."""
    real = int(sc.orig_subject_id(int(subject_id)))

    def _build():
        pts = _points_by_user(real)
        students = _students_of(real)
        rows = [(uid, p) for uid, p in pts.items() if p > 0 and uid in students]
...
def position(user_id: int, subject_id: int) -> dict:
    """Место человека в предмете: {place, total, points}. place=0 — ещё нет баллов."""
    board = leaderboard(subject_id)
    my = points(user_id, subject_id)
    place = 0
    for i, (uid, _p) in enumerate(board, start=1):
        if uid == user_id:
            place = i
            break
    ahead = 0
    if place > 1:
        ahead = max(0, board[place - 2][1] - my)      # до места выше
    return {"place": place, "total": len(board), "points": my, "to_next": ahead}
