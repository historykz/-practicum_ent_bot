...
def rating_group(subject_id) -> list:
    ...
    def _build():
        ...
        mine = root(real)
        members = sorted(sid for sid in rows if root(sid) == mine) or [real]
        # Корень — первым: у него хранятся настройки вознаграждений и цены программы
        return [mine] + [m for m in members if m != mine] if mine in members else members

    return _cached(("group", real), _build)


def rating_root(subject_id) -> int:
    """Корень группы копий: предмет, от которого пошли остальные. Настройки
    вознаграждений, цены уроков и счета учеников программы живут у него."""
    return rating_group(subject_id)[0]


def group_lesson_keys(subject_id) -> dict:
    """{урок: ключ} для ВСЕХ уроков предметов группы (включая закрытые и
    строки-ярлыки). Копии одного урока внутри группы — один ключ: это один и
    тот же урок, сколько бы копий предмета его ни содержало. Родитель за
    пределами группы не учитывается: отдельная программа — отдельные уроки."""
    group = rating_group(subject_id)

    def _build():
        ph = ",".join("?" * len(group))
        try:
            rows = {r["id"]: (r["cf"], r["lg"], r["oi"]) for r in db.fetchall(
                f"SELECT l.id, l.copied_from_lesson_id AS cf, l.legacy_lesson_id AS lg, "
                f"l.original_id AS oi FROM lessons l JOIN sections s ON s.id=l.section_id "
                f"WHERE s.subject_id IN ({ph})", tuple(group))}
        except Exception:
            return {}
        out = {}
        for lid in rows:
            ...
            while cur in rows and cur not in seen:
                ...
                cf, lg, oi = rows[cur]
                if oi:
                    # Старый ярлык — это сам урок-оригинал (прогресс и тест у него),
                    # даже если оригинал лежит в другой программе
                    cur = oi
                    continue
                parent = lg or cf
    ...
    return _cached(("gkeys", tuple(group)), _build)
...
def _test_keys(subject_id) -> dict:
    ...
    keys = group_lesson_keys(subject_id)
    out = {}
