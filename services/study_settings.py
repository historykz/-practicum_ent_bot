...
DEFAULTS = {
    ...
    # Учебный план по предметам под контролем (services/study_subjects.py)
    "study_pace_per_week": "3",      # уроков в неделю, если у предмета не задан свой темп
    "study_lag_slight": "1",         # отставание в уроках: «чуть отстаёт»
    "study_lag_behind": "3",         # «отстаёт»
    "study_lag_far": "6",            # «сильно отстаёт»
    "study_plan_repeat_days": "3",   # не чаще писать «ты отстаёшь от плана»
}
...
TEMPLATE_TITLES = {
    ...
    "behind_plan": "Отстаёт от учебного плана",
}
...
TEMPLATE_DEFAULTS = {
    ...
    "behind_plan": ("{name}, по плану по предмету «{subject}» к сегодняшнему дню "
                    "должно быть закрыто {expected_topics} тем, а закрыто "
                    "{completed_topics} 📉\n\nОтставание — {lag_topics}. "
                    "Не пытайся нагнать всё разом: закрой сегодня одну тему с ДЗ, "
                    "завтра ещё одну — и график выровняется."),
}
...
VARIABLES = ["{name}", "{days_without_activity}", "{unfinished_homeworks}",
             ...
             "{subject}", "{expected_topics}", "{lag_topics}", "{plan_progress}"]
...
def render(text: str, data: dict) -> str:
    ...
    worst = data.get("plan") or {}
    lag = int(worst.get("lag") or 0)
    lag_word = "тема" if lag % 10 == 1 and lag % 100 != 11 else (
        "темы" if 2 <= lag % 10 <= 4 and not 12 <= lag % 100 <= 14 else "тем")
    values = {
        "{subject}": worst.get("title") or "предмет",
        "{expected_topics}": str(worst.get("expected_done") or 0),
        "{lag_topics}": f"{lag} {lag_word}",
        "{plan_progress}": f"{worst.get('lessons_done') or 0} из {worst.get('lessons_total') or 0}",
        ...
        "{completed_topics}": str(worst.get("lessons_done")
                                  if worst.get("title") else data.get("done_topics", 0)),
