...
# Что ученик отметил, но ещё не подтвердил: {(attempt_id, question_id): [option_id]}
# Держим в памяти — до нажатия «Ответить» это черновик, в базу он не нужен.
_picked: dict = {}


def get_picked(attempt_id: int, question_id: int) -> list:
    return list(_picked.get((attempt_id, question_id), []))


def toggle_picked(attempt_id: int, question_id: int, option_id: int) -> list:
    """Поставить или снять отметку на варианте. Возвращает текущий набор."""
    key = (attempt_id, question_id)
    cur = _picked.get(key, [])
    if option_id in cur:
        cur = [o for o in cur if o != option_id]
    else:
        cur = cur + [option_id]
    _picked[key] = cur
    return list(cur)
...
def clear_picked(attempt_id: int, question_id: int = None) -> None:
    if question_id is None:
        for key in [k for k in _picked if k[0] == attempt_id]:
            _picked.pop(key, None)
    else:
        _picked.pop((attempt_id, question_id), None)
...
def _can_use_quiz_poll(question_text: str, options: list[dict]) -> bool:
    """Раньше короткие вопросы уходили родным опросом Telegram.

    Больше так не делаем: в опросе можно выбрать только один вариант, и
    правильный подсвечивается сразу. Вопросы с несколькими правильными
    ответами так не задать, а если часть вопросов показывать опросом, а
    часть кнопками — ученик сразу увидит, какой вопрос «с подвохом».
    Поэтому все вопросы показываем одинаково, кнопками.
    """
    return False


def _quiz_poll_limits_ok(question_text: str, options: list[dict]) -> bool:
...
async def send_current_question(bot: Bot, attempt_id: int, chat_id: int) -> None:
    ...
    try:
        if use_poll:
            ...
            # Кнопки вариантов: отмечаем, потом подтверждаем «Ответить»
            clear_picked(attempt_id, qid)      # новый вопрос — черновик пуст
...
async def _process_answer_inner(bot: Bot, attempt_id: int, question_id: int,
    ...
    clear_picked(attempt_id, question_id)
    if is_correct:
        db.execute(
