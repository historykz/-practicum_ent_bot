...
async def finalize_attempt(bot: Bot, attempt_id: int, chat_id: int,
    ...
    if status == "finished":
        # Вознаграждение за урок и рейтинг предмета — и когда тест сдан в боте
        from services import reward_service as _rs
        await asyncio.to_thread(_rs.on_learning_event, None, attempt["user_id"],
                                None, attempt["test_id"])

    # Учебное событие «сдал ДЗ». Раньше его писало только приложение, поэтому
    # ученик, который проходит тесты в боте, выглядел не занимающимся: серия
    # была нулевой, а контроль обучения считал его отстающим.
    if not aborted:
        try:
            from services import study_tracker as _stt
            from services import gamification as _gm
            u_row = db.fetchone("SELECT tg_id FROM users WHERE id=?", (attempt["user_id"],))
            les = db.fetchone("SELECT id FROM lessons WHERE test_id=? LIMIT 1",
                              (attempt["test_id"],))
            if u_row and u_row.get("tg_id"):
                _stt.track(u_row["tg_id"], _stt.HW_DONE,
                           lesson_id=(les["id"] if les else None),
                           test_id=attempt["test_id"])
                _gm.invalidate()
                _gm.award(u_row["tg_id"], attempt["user_id"])
        except Exception as e:
            logger.warning("учебное событие после теста %s: %s", attempt_id, e)
