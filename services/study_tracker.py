...
def days_without_study(tg_id: int) -> Optional[float]:
    ...
    Смотрим на все следы учёбы, а не только на журнал событий: открытые
    уроки (lesson_progress) и попытки тестов — в том числе пройденные в боте,
    где журнал событий не ведётся. Иначе человек, сдавший тест в боте,
    числился «ни разу не занимался».
    """
    ensure_schema()
    stamps = []
    row = db.fetchone(
    ...
    try:
        row = db.fetchone("SELECT MAX(viewed_at) AS m FROM lesson_progress WHERE user_tg_id=?",
                          (tg_id,))
        if row and row["m"]:
            stamps.append(row["m"])
        row = db.fetchone(
            "SELECT MAX(COALESCE(ta.end_time, ta.start_time)) AS m FROM test_attempts ta "
            "JOIN users u ON u.id = ta.user_id WHERE u.tg_id=? "
            "AND COALESCE(ta.publication_id, 0)=0", (tg_id,))
        if row and row["m"]:
            stamps.append(row["m"])
    except Exception:
        pass
    dts = [d for d in (_parse(x) for x in stamps) if d]
    if not dts:
        return None                     # ни разу не занимался
    return (now() - max(dts)).total_seconds() / 86400
...
def profile(tg_id: int) -> dict:
    ...
    # Предметы под контролем: реальный прогресс и отставание от плана
    try:
        from services import study_subjects as _sj
        data["subjects"] = _sj.user_summary(tg_id)
        data["plan"] = _sj.worst_of(data["subjects"])
    except Exception as e:
        log.debug("profile subjects %s: %s", tg_id, e)
        data["subjects"], data["plan"] = [], {}
    data["plan_status"] = (data["plan"] or {}).get("status") or "ok"
    data["plan_lag"] = (data["plan"] or {}).get("lag") or 0
    data["risk"] = risk_level(data)
    # Отставание от плана поднимает статус, даже если человек заходит каждый день
    _order = {"ok": 0, "slight": 1, "behind": 2, "far": 3}
    if _order.get(data["plan_status"], 0) > _order.get(data["risk"], 0):
        data["risk"] = data["plan_status"]
...
def mark_lesson_opened(tg_id: int, lesson_id: int) -> bool:
    """Ученик открыл урок (конспект). True — ВПЕРВЫЕ: это новое учебное
    действие (засчитывается в активность, у урока без теста — приносит
    вознаграждение). Повторные открытия только увеличивают open_count и
    last_opened_at — для статистики; ни денег, ни сброса счётчика пропусков.
    Одна точка для сайта и бота."""
    if not tg_id or not lesson_id:
        return False
    stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    cur = db.execute(
        "INSERT OR IGNORE INTO lesson_progress (user_tg_id, lesson_id, viewed_at, first_opened_at, "
        "last_opened_at, open_count) VALUES (?,?,?,?,?,1)", (tg_id, lesson_id, stamp, stamp, stamp))
    if cur.rowcount:
        return True
    db.execute("UPDATE lesson_progress SET last_opened_at=?, open_count=COALESCE(open_count,1)+1 "
               "WHERE user_tg_id=? AND lesson_id=?", (stamp, tg_id, lesson_id))
    return False
