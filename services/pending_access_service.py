...
async def activate_for_user(bot, tg_id: int, username: str,
        ...
        try:
            if kind == 'subject' and r.get('test_id'):
                ...
                if days > 0:
                    ...
                    import utils as _ut
                    expires_at = _ut.to_utc_iso(exp_dt)    # единый формат срока
                db.execute(
    ...
    # Отложенный Премиум = ручная выдача, применённая при первом /start:
    # инструкция идёт после уведомления о доступе, ключ — id заявок.
    if onboard_needed and tg_id and notified:
        try:
            from services import onboarding_service as _onb
            await _onb.premium_activated(bot, tg_id, premium_days, source="pending",
                                         event_key="pending:" + ",".join(premium_ids))
        except Exception as e:
