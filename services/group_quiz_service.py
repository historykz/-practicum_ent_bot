...
def _mark_activity(chat_id, tg_id) -> None:
    """Участие в тесте — активность в чате для статистики автозапуска."""
    try:
        from services import auto_schedule_service as _ass
        _ass.record_chat_activity(chat_id, tg_id)
    except Exception:
        pass
...
async def start_lobby(bot: Bot, test: dict, chat_id: int,
    ...
    """
    # Защита: один активный тест на группу. Тест-«сирота» (остался от
    # процесса до перезапуска бота — таймеры потеряны, он никогда не
    # завершится) не считается: закрываем его и запускаем новый. Раньше такой
    # хвост молча блокировал чат навсегда: анонс уходил, а лобби не появлялось.
    for row in db.fetchall(
            "SELECT id FROM group_quizzes WHERE chat_id=? AND status IN ('lobby','running','paused')",
            (chat_id,)):
        if is_orphan(row["id"]):
            await cancel_orphan(bot, row["id"], "перезапуск бота")
            continue
    ...
    try:
        ...
        # Сеть/таймаут: строка не должна остаться «лобби» без карточки — она заняла бы чат
        db.execute("UPDATE group_quizzes SET status='cancelled' WHERE id=?", (gq_id,))
        raise
...
async def join_player(bot: Bot, group_quiz_id: int, user) -> tuple[bool, str]:
    ...
    _mark_activity(gq['chat_id'], user.id)
    db.execute(
...
async def stop_quiz(bot: Bot, chat_id: int, requester_tg_id: int) -> tuple[bool, str]:
    ...
    """
    gq = db.fetchone(
        "SELECT * FROM group_quizzes WHERE chat_id=? AND status IN ('lobby','running','paused')",
        (chat_id,))
    if not gq:
    ...
    if gq['status'] == 'lobby':
        ...
        try:
            ...
            pass
        # Расписание: запуск не состоялся — отметить и двинуть очередь слота
        try:
            from services import auto_schedule_service as _ass
            await _ass.on_quiz_done(bot, gq['id'], cancelled=True)
        except Exception as e:
            logger.warning("scheduler stop hook: %s", e)
    ...
    try:
        ...
        try:
            ...
            await autopub_service.on_series_test_finished(
                bot, gq['test_id'], gq['chat_id'], gq_id=gq_id, cancelled=True)
        except Exception as e:
            logger.warning("cancel open chat: %s", e)
        # Расписание: отметить запуск как несостоявшийся и двинуть очередь слота
        try:
            from services import auto_schedule_service as _ass
            await _ass.set_chat_slowmode(bot, gq['chat_id'], _ass.SLOWMODE_NORMAL)
            await _ass.on_quiz_done(bot, gq_id, cancelled=True)
        except Exception as e:
            logger.warning("scheduler cancel hook: %s", e)
...
ORPHAN_MIN_AGE = 120     # секунд: моложе — возможно, таймер ещё не успел записаться
# Сирота — только сессия, созданная ДО запуска этого процесса. Живой тест между
# вопросами на долю секунды остаётся без таймера (идёт отправка опроса) — раньше
# его принимали за сироту и закрывали посреди прохождения.
_PROCESS_STARTED_AT = datetime.utcnow()


def _created_age(gq: dict) -> float:
    """Сколько секунд назад создана сессия (created_at пишет SQLite, UTC)."""
    raw = str(gq.get("created_at") or "").replace("T", " ")[:19]
    try:
...
def is_orphan(gq_id: int) -> bool:
    """Сессия lobby/running без единого живого таймера в этом процессе —
    осталась от прошлого запуска бота и сама уже не двинется."""
    gq = db.fetchone("SELECT * FROM group_quizzes WHERE id=?", (gq_id,))
    if not gq or gq["status"] not in ("lobby", "running"):
        return False
    if gq_id in _lobby_timers or gq_id in _question_timers or gq_id in _countdown_tasks \
            or gq_id in _poll_to_gq.values():
        return False
    age = _created_age(dict(gq))
    if age >= 10 ** 9 or datetime.utcnow() - _PROCESS_STARTED_AT >= __import__("datetime").timedelta(seconds=age):
        return False          # создана этим процессом (или дата не читается) — не сирота
    return age > ORPHAN_MIN_AGE


def cancel_orphan_sync(gq_id: int, why: str = "") -> bool:
    cur = db.execute("UPDATE group_quizzes SET status='cancelled', finished_at=? WHERE id=? "
                     "AND status IN ('lobby','running')", (now_iso(), gq_id))
    if cur.rowcount:
        logger.warning("Групповой тест #%s закрыт как сирота (%s)", gq_id, why or "без причины")
    return bool(cur.rowcount)


async def cancel_orphan(bot: Bot, gq_id: int, why: str = "") -> bool:
    """Закрыть сессию-сироту и убрать её мёртвое лобби из чата."""
    gq = db.fetchone("SELECT * FROM group_quizzes WHERE id=?", (gq_id,))
    if not gq or not cancel_orphan_sync(gq_id, why):
        return False
    if gq["status"] == "lobby" and gq["lobby_message_id"]:
        try:
            await bot.delete_message(gq["chat_id"], gq["lobby_message_id"])
        except Exception:
            pass
    try:
        await bot.send_message(
            gq["chat_id"],
            "⚠️ Прошлый тест был прерван перезапуском бота и закрыт. "
            "Следующий тест придёт по расписанию.")
    except Exception:
        pass
    return True


async def cleanup_orphans(bot: Bot) -> int:
    """При старте бота: закрыть все сессии, оставшиеся от прошлого процесса."""
    n = 0
    for row in db.fetchall("SELECT id FROM group_quizzes WHERE status IN ('lobby','running')"):
        if is_orphan(row["id"]) and await cancel_orphan(bot, row["id"], "перезапуск бота"):
            n += 1
    if n:
        logger.warning("Закрыто групповых тестов-сирот после перезапуска: %d", n)
    return n
...
async def _question_timeout(bot: Bot, gq_id: int, sleep_seconds: int):
    ...
    try:
        ...
        # Убираем только СВОЙ таймер: _send_question уже поставил таймер следующего
        # вопроса, и раньше этот finally его стирал — тест выглядел «без таймера»
        if _question_timers.get(gq_id) is asyncio.current_task():
...
async def _finalize(bot: Bot, gq_id: int, aborted: bool = False):
    ...
    try:
        ...
        await _ass.set_chat_slowmode(bot, gq['chat_id'], _ass.SLOWMODE_NORMAL)
        # Итоги запуска в историю расписания + следующий тест слота, если есть
        await _ass.on_quiz_done(bot, gq_id, cancelled=False)
    except Exception as e:
    ...
    try:
        from services import autopub_service
        # По id группового теста: двигается ТОЛЬКО серия, которой он принадлежит
        await autopub_service.on_series_test_finished(bot, gq['test_id'], gq['chat_id'],
                                                       gq_id=gq_id, cancelled=aborted)
    except Exception as e:
