...
def decode_import(data: bytes) -> str:
    """Текст из файла, как бы его ни сохранили.

    Блокнот Windows пишет UTF-8 с BOM, иногда UTF-16 или cp1251. Раньше
    бралось только utf-8 без BOM: первая строка получала невидимый символ
    впереди, переставала считаться комментарием и уходила ученикам как
    «мотивашка», а cp1251 превращался в кракозябры.
    """
    if not data:
        return ""
...
def parse_import(raw: str) -> tuple:
    ...
    Одна строка — одна мотивашка; пустые строки пропускаем; строки с # —
    служебные, ученикам не уходят.
    """
    if not raw:
        return [], 0
    raw = raw.replace("\ufeff", "")
...
def stats() -> dict:
    """Сколько материалов активно и сколько реально уходило ученикам."""
    ensure_schema()
    out = {"total": 0, "active": 0, "sent_7d": 0, "last_sent": ""}
    try:
        out["total"] = db.fetchone("SELECT COUNT(*) AS c FROM motivations")["c"]
        out["active"] = db.fetchone("SELECT COUNT(*) AS c FROM motivations WHERE enabled=1")["c"]
        out["sent_7d"] = db.fetchone(
            "SELECT COUNT(*) AS c FROM motivation_log "
            "WHERE datetime(sent_at) >= datetime('now', '-7 days')")["c"]
        row = db.fetchone("SELECT MAX(sent_at) AS m FROM motivation_log")
        out["last_sent"] = (row["m"] or "") if row else ""
    except Exception:
        pass
    return out
