        ...
        with conn.cursor() as cur:
            ...
            for q_id, q_text, pos in questions:
                ...
                result.append({
                    ...
                    "position": pos,
                    "options": [{"text": x[0], "is_correct": x[1], "position": x[2]} for x in opts]
                })
            return result
...
def reset_user_progress(user_id: int, test_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO user_test_progress (user_id, test_id, current_question_pos, score, finished)
...
def update_user_progress(user_id: int, test_id: int, current_question_pos: int, score: int, finished: bool):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
...
def is_option_line(line: str) -> bool:
    ...
    1) текст
    """
    return bool(re.match(r"^\s*([A-Za-zА-Яа-яЁёІіҢңҒғҮүҰұҚқӨөҺһ0-9]+)[\)\.\-]\s+.+", line.strip()))

def clean_option_prefix(line: str) -> str:
    return re.sub(r"^\s*([A-Za-zА-Яа-яЁёІіҢңҒғҮүҰұҚқӨөҺһ0-9]+)[\)\.\-]\s+", "", line.strip())
...
def parse_questions_block(text: str) -> List[Dict]:
    ...
    for block in blocks:
        ...
        if not question_lines or len(option_lines) < 2:
            raise ValueError(
                "Неверный формат одного из вопросов. У каждого вопроса должен быть текст и минимум 2 варианта ответа."
            )
    ...
    if not parsed:
        raise ValueError("Не удалось найти ни одного вопроса.")
...
async def send_next_question(
    ...
    correct_indexes = [idx for idx, opt in enumerate(options) if opt["is_correct"]]

    # Для обычного quiz-вопроса нужен один правильный вариант.
    ...
    # Сохраняем связь poll_id -> данные
...
async def tests_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tests = get_tests()
    if not tests:
        await update.message.reply_text("Пока тестов нет.")
        return
...
async def addtest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not title:
        await msg.reply_text("Укажите название теста.")
        return
...
async def listtests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tests = get_tests()
    if not tests:
        await msg.reply_text("Тестов пока нет.")
        return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если админ находится в режиме импорта — импортируем вопросы
    if pending_title and is_admin(user.id):
        try:
            parsed_questions = parse_questions_block(msg.text)
            ...
            await msg.reply_text(
                ...
                f"Или напишите /cancel"
            )
            return

    # Обычным пользователям подсказываем команды
    await msg.reply_text(
        "Нажмите /start или /tests, чтобы выбрать тест."
    )
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and is_admin(user.id):
        admin_block = (
            "\n\n<b>Команды админа:</b>\n"
            "/addtest Название теста — добавить новый тест\n"
