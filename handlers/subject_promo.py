...
def miniapp_url(param: str = "") -> str:
    """Прямая ссылка в мини-приложение Telegram.

    Открывается внутри Telegram: вход происходит сам, отдельная регистрация
    на сайте не нужна. Параметр прилетает в приложение как start_param.
    """
...
def subject_link(subject_id: int) -> str:
    """Ссылка-витрина предмета для публикации в канале — в мини-приложение."""
    return miniapp_url(f"subj_{subject_id}")


def subject_bot_link(subject_id: int) -> str:
    """Запасной вариант: та же витрина, но карточкой в переписке с ботом."""
...
def build_promo(subject_id: int):
    ...
    # Кнопки ведут в мини-приложение, а не на сайт: там человек входит сам,
    # без отдельной регистрации через Telegram-виджет.
    rows = [[InlineKeyboardButton(text="📚 Открыть каталог предмета",
                                  url=miniapp_url(f"subj_{subject_id}"))]]
    if free:
        rows.append([InlineKeyboardButton(
            text="🆓 Начать с бесплатного урока",
            url=miniapp_url(f"lesson_{free[0]['id']}"))])
    if paid:
        # Колбэк именно buy:premium — на m:premium никто не отвечал,
        # и кнопка просто молчала.
