...
async def cb_download(call: CallbackQuery, bot: Bot):
    """Полная копия ВСЕГО: бот (тесты, вопросы, картинки, пользователи)
    + сайт (предметы, уроки, конспекты, доступы, зачёты, файлы).
    Раньше вторую часть приходилось качать вручную с сайта."""
    await call.answer()
    c = backup_service.backup_counts()
    ...
    # --- Вторая половина: бэкап САЙТА (предметы, уроки, конспекты, доступы) ---
    site_msg = await call.message.answer("🌐 Собираю бэкап сайта…")
    try:
        from datetime import datetime
        await backup_service._send_site_backup(
            bot, call.message.chat.id, datetime.now().strftime("%d.%m.%Y"))
        await site_msg.edit_text(
            "✅ <b>Готово — прислал ВСЁ.</b>\n\n"
            "🤖 Бот: тесты, вопросы, картинки, пользователи, покупки.\n"
            "🌐 Сайт: предметы, разделы, уроки, конспекты, зачёты, "
            "кому выдан доступ, результаты, журнал просмотров.\n\n"
            "Заходить на сайт за второй частью больше не нужно.",
            parse_mode="HTML")
    except Exception as e:
        log.exception("site backup from bot: %s", e)
        await site_msg.edit_text(
            f"⚠️ Бэкап бота ушёл, а сайт собрать не смог: {e}\n\n"
            "Скачайте его в админке сайта → «Скачать полный бэкап».")
