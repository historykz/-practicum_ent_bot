...
class NotesStates(StatesGroup):
    ...
    waiting_video = State()       # приём видео урока
...
def _subjects_screen():
    ...
    rows.append([_btn(f"⏱ Срок хранения конспектов: {_ttl_label()}", "na:ttl")])
...
def _ttl_label() -> str:
    from handlers.lesson_notes import notes_ttl_hours
    h = notes_ttl_hours()
    return "не удалять" if h == 0 else f"{h} ч"


TTL_CHOICES = (1, 3, 6, 12, 24, 48, 72, 168, 0)


@router.callback_query(F.data == "na:ttl", IsAdmin())
async def cb_ttl(call: CallbackQuery):
    """Через сколько конспект исчезает из переписки ученика."""
    rows = []
    line = []
    for h in TTL_CHOICES:
        line.append(_btn("не удалять" if h == 0 else f"{h} ч", f"na:ttlset:{h}"))
        if len(line) == 3:
            rows.append(line); line = []
    if line:
        rows.append(line)
    rows.append([_btn("⬅️ Назад", "na:home")])
    await _show(call,
                "⏱ <b>Срок хранения конспектов</b>\n\n"
                f"Сейчас: <b>{_ttl_label()}</b>\n\n"
                "Через это время конспект сам пропадёт из личных сообщений ученика. "
                "Отсчёт идёт с момента выдачи.\n\n"
                "Если забрать доступ к предмету или доступ закончится — конспекты "
                "сотрутся сразу, не дожидаясь срока.",
                InlineKeyboardMarkup(inline_keyboard=rows))
    await call.answer()


@router.callback_query(F.data.startswith("na:ttlset:"), IsAdmin())
async def cb_ttl_set(call: CallbackQuery):
    from handlers.lesson_notes import set_notes_ttl_hours
    hours = int(call.data.split(":")[2])
    set_notes_ttl_hours(hours)
...
def _lesson_screen(lesson_id):
    ...
    if l.get("video_file_id"):
        lines.append("🎬 Видео: <b>есть</b> (хранится в Telegram)")
    elif l.get("youtube_id") or l.get("video_url"):
        lines.append("🎬 Видео: есть (внешняя ссылка)")
    else:
        lines.append("🎬 Видео: <i>нет</i>")
    ...
    rows = [
        ...
        [_btn("🎬 Загрузить видео", f"na:vid:{lesson_id}")]
        + ([_btn("🗑 Убрать видео", f"na:vidclear:{lesson_id}")]
           if l.get("video_file_id") else []),
...
# ===================== видео урока =====================
# Видео храним так же, как страницы конспекта: файл остаётся в Telegram,
# у нас только file_id. Место на сервере не тратится, а в бэкап видео
# попадает вместе с базой — file_id лежит в ней.

@router.callback_query(F.data.startswith("na:vid:"), IsAdmin())
async def cb_video_ask(call: CallbackQuery, state: FSMContext):
    lesson_id = int(call.data.split(":")[2])
    await state.set_state(NotesStates.waiting_video)
    await state.update_data(lesson_id=lesson_id)
    await call.message.answer(
        "🎬 <b>Пришлите видео урока</b>\n\n"
        "Одним файлом — как видео или как документ. Оно останется в Telegram, "
        "место на сервере не займёт.\n\n"
        "Ученик получит его в личные сообщения по кнопке «Смотреть видео»: "
        "переслать и сохранить видео он не сможет.\n\n"
        "/cancel — отмена",
        parse_mode="HTML")
    await call.answer()


@router.message(NotesStates.waiting_video, F.video | F.document | F.video_note,
                IsAdmin())
async def msg_video(message: Message, state: FSMContext):
    ...
    if not lesson_id:
        await state.clear()
        await message.answer("Не понял, к какому уроку. Откройте урок заново.")
        return
    media = message.video or message.video_note or message.document
    file_id = media.file_id
    unique = getattr(media, "file_unique_id", "") or ""
    db.execute("UPDATE lessons SET video_file_id=?, video_file_unique=? WHERE id=?",
               (file_id, unique, lesson_id))
    await state.clear()
    size = getattr(media, "file_size", 0) or 0
    text, kb = _lesson_screen(lesson_id)
    await message.answer(
        f"✅ Видео сохранено{f' ({size / 1024 / 1024:.1f} МБ)' if size else ''}.\n\n"
        + text, reply_markup=kb, parse_mode="HTML")


@router.message(NotesStates.waiting_video, IsAdmin())
async def msg_video_other(message: Message, state: FSMContext):
    if (message.text or "").startswith("/cancel"):
        await state.clear()
        await message.answer("❌ Отменено.")
        return
    await message.answer("Нужно именно видео — пришлите файлом или как видео.")


@router.callback_query(F.data.startswith("na:vidclear:"), IsAdmin())
async def cb_video_clear(call: CallbackQuery, state: FSMContext):
    await state.clear()
    lesson_id = int(call.data.split(":")[2])
    db.execute("UPDATE lessons SET video_file_id=NULL, video_file_unique=NULL WHERE id=?",
               (lesson_id,))
    text, kb = _lesson_screen(lesson_id)
    await _show(call, "🗑 Видео убрано.\n\n" + text, kb)
