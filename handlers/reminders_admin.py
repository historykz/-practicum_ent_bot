...
TITLES = {rs.MANUAL: "📚 Сообщение раздела «КОНСПЕКТЫ ЕНТ»",
          ...
          rs.PREMIUM_OK: "🎉 После покупки: поздравление",
          rs.PREMIUM_QUIZLET: "🃏 После покупки: карточки Quizlet"}
...
def _screen(key: str):
    ...
    if (c.get("button2_text") or "").strip():
        lines += [f"🔘 Вторая кнопка: <b>{utils.escape_html(c['button2_text'])}</b>",
                  f"🔗 Ссылка: <code>{utils.escape_html(c.get('button2_url') or '')}</code>"]
    if key in (rs.PREMIUM_OK, rs.PREMIUM_QUIZLET):
        lines.insert(2, "Статус: " + ("🟢 Включено" if c["enabled"] else "🔴 Выключено"))
        lines.insert(3, f"Пауза перед следующим: <b>{c.get('safe_delay_seconds') or 0} с</b>")
        lines.insert(4, f"Порядок: <b>{c.get('sort_order') or 0}</b>")
        lines.insert(5, "\nВ тексте можно писать {days}, {until}, {tests} — "
                        "подставятся срок, дата окончания и число открытых тестов.")
    ...
    if key in (rs.PREMIUM_OK, rs.PREMIUM_QUIZLET):
        rows.insert(2, [_btn("🔘 Вторая кнопка: текст", f"rem:btn2:{key}"),
                        _btn("🔗 Вторая кнопка: ссылка", f"rem:url2:{key}")])
        rows.append([_btn("🔴 Выключить сообщение" if c["enabled"]
                          else "🟢 Включить сообщение", f"rem:toggle:{key}")])
        rows.append([_btn("🕒 Пауза перед следующим", f"rem:delay:{key}"),
                     _btn("🔀 Порядок", f"rem:order:{key}")])
...
async def cb_home(call: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [_btn(("🟢 " if rs.get_campaign(rs.PREMIUM_OK)["enabled"] else "🔴 ")
              + "После покупки: поздравление", f"rem:open:{rs.PREMIUM_OK}")],
        [_btn(("🟢 " if rs.get_campaign(rs.PREMIUM_QUIZLET)["enabled"] else "🔴 ")
              + "После покупки: карточки Quizlet", f"rem:open:{rs.PREMIUM_QUIZLET}")],
...
# ===================== вторая кнопка и порядок сообщений =====================

@router.callback_query(F.data.startswith("rem:btn2:"), IsAdmin())
async def cb_btn2(call: CallbackQuery, state: FSMContext):
    key = call.data.split(":")[2]
    c = rs.get_campaign(key)
    await _ask(call, state, RemStates.waiting_button2, key,
               "🔘 Пришлите подпись ВТОРОЙ кнопки.\n\n"
               f"Сейчас: <b>{utils.escape_html(c.get('button2_text') or '— нет —')}</b>\n\n"
               "Чтобы убрать вторую кнопку, отправьте <code>-</code>")


@router.message(RemStates.waiting_button2, IsAdmin())
async def msg_button2(message: Message, state: FSMContext):
    if (message.text or "").startswith("/cancel"):
        await state.clear(); await message.answer("❌ Отменено."); return
    t = (message.text or "").strip()
    await _save(message, state, "button2_text", "" if t == "-" else t[:64])


@router.callback_query(F.data.startswith("rem:url2:"), IsAdmin())
async def cb_url2(call: CallbackQuery, state: FSMContext):
    key = call.data.split(":")[2]
    c = rs.get_campaign(key)
    await _ask(call, state, RemStates.waiting_url2, key,
               "🔗 Пришлите ссылку ВТОРОЙ кнопки.\n\n"
               f"Сейчас: <code>{utils.escape_html(c.get('button2_url') or '— нет —')}</code>\n\n"
               "Можно написать <code>tests</code> — тогда кнопка откроет "
               "список платных тестов прямо в боте.")


@router.message(RemStates.waiting_url2, IsAdmin())
async def msg_url2(message: Message, state: FSMContext):
    if (message.text or "").startswith("/cancel"):
        await state.clear(); await message.answer("❌ Отменено."); return
    u = (message.text or "").strip()
    if u != "tests" and not u.startswith("http"):
        await message.answer("Ссылка должна начинаться с https:// или быть словом tests")
        return
    await _save(message, state, "button2_url", u)


@router.callback_query(F.data.startswith("rem:order:"), IsAdmin())
async def cb_order(call: CallbackQuery):
    """Переставить сообщение вперёд или назад в очереди."""
    key = call.data.split(":")[2]
    c = rs.get_campaign(key)
    new_order = 0 if (c.get("sort_order") or 0) else 1
    rs.update_campaign(key, sort_order=new_order)
    text, kb = _screen(key)
    await _show(call, text, kb)
