...
async def cb_answer(call: CallbackQuery, user: dict):
    ...
    # Нажатие на вариант только ставит или снимает отметку. Ответ уходит
    # кнопкой «Ответить» — одинаково для вопросов с одним и с несколькими
    # правильными ответами, чтобы тип вопроса ниоткуда не был виден.
    picked = test_runner.toggle_picked(attempt_id, question_id, option_id)
    try:
        options = test_runner.get_ordered_options(attempt_id, question_id)
        await call.message.edit_reply_markup(
            reply_markup=options_kb(attempt_id, question_id, options, picked))
    except Exception:
        pass
    await call.answer()
    return
...
@router.callback_query(F.data.startswith("ansok:"))
async def cb_answer_confirm(call: CallbackQuery, user: dict):
    """«✅ Ответить» — фиксируем набор отмеченных вариантов."""
    lang = user.get('language') or 'ru'
    parts = call.data.split(":")
    if len(parts) != 3:
        await call.answer()
    ...
    picked = test_runner.get_picked(attempt_id, question_id)
    if not picked:
        await call.answer("Отметьте хотя бы один вариант.", show_alert=True)
        return

    result = await test_runner.process_answer(
        call.bot, attempt_id, question_id, picked, call.message.chat.id)
