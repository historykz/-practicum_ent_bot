...
# Личные ответы
# ============================
# Ответ на вопрос выбирается только внутри родного опроса Telegram и приходит
# событием poll_answer (см. on_poll_answer ниже). Инлайн-кнопок вариантов нет.
...
async def on_poll_answer(poll_answer: PollAnswer):
    ...
    try:
        bot = poll_answer.bot
        if bot is None or poll_answer.user is None:
            return
        # Дуэль (Quiz Poll)
        try:
