...
async def cb_sched_new(call: CallbackQuery, state: FSMContext):
    ...
    kb.button(text="🎲 Все разделы", callback_data="schedcat:all")
    kb.button(text="❌ Отмена", callback_data="sched:menu")
    kb.adjust(1)
    await call.message.answer("📂 <b>Шаг 1/5:</b> Выбери раздел для автозапуска:",
