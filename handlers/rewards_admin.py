...
def _participant_text(pid: int) -> tuple:
    ...
    lines = [
        ...
        f"💸 Денежная акция: <b>{'участвует' if not p.get('money_off') else 'нет — пришёл после закрытия акции (только рейтинг)'}</b>",
        f"📌 Статус: <b>{rs.status_title(p)}</b>"
