...
def _lesson_access_sync(lesson_id: int, tg_id: int):
    ...
    # «Начать обучение» — обязательная точка входа (общий рейтинг предмета)
    try:
        from webapp import learning as _lg
        if _lg._needs_start_sync(subject["id"], tg_id):
            return None, "need_start"
    except Exception as e:
        log.warning("проверка старта обучения: %s", e)
    return lesson, subject


def _lesson_subject_id_sync(lesson_id: int):
...
def _lesson_subject_ids(raw) -> list:
    ...
    ids = []
    # и урока, из ярлыка которого он сделан (v69): доступ к тому предмету
    # открывал этот урок и до перевода ярлыков в независимые копии
    legacy = dict(raw).get("legacy_lesson_id")
    for lid in {raw["id"], sc.orig_lesson_id(raw["id"])} | ({legacy} if legacy else set()):
        row = db.fetchone(
...
def _mark_note_opened_sync(tg_id: int, lesson_id: int) -> None:
    """Одно открытие темы — одно учебное событие.

    Раньше отметка стояла внутри _remember_msg_sync, а его зовут на КАЖДОЕ
    отправленное сообщение: у конспекта на 20 страниц выходило два десятка
    событий за один просмотр. Из-за этого «часто открываемые темы» показывали
    не то, что человек открывает чаще, а у какой темы длиннее конспект.
    Заодно отмечаем просмотр урока — раньше это делало только приложение,
    и у тех, кто читает конспекты в боте, прогресс не двигался вовсе.
    """
    try:
        ...
        pass
    try:
        # Первое открытие — новое учебное действие; повторное только считается.
        # Урок без теста и без зачёта этим и пройден — сразу начисляем за него.
        from services import study_tracker as _stt_open
        lid = sc.orig_lesson_id(lesson_id)
        if _stt_open.mark_lesson_opened(tg_id, lid):
            row = db.fetchone("SELECT test_id, is_zachet FROM lessons WHERE id=?", (lid,))
            if row and not row["test_id"] and not row["is_zachet"]:
                from services import reward_service as _rs
                _rs.on_learning_event(tg_id, None, lesson_id=lid)
    except Exception as e:
        log.warning("отметка открытия конспекта: %s", e)
...
def _hw_keyboard(lesson: dict, video_sent: bool = False) -> InlineKeyboardMarkup:
    ...
    video_sent — видео уже прислано выше вместе с конспектом, кнопка не нужна."""
    rows = []
    ...
    if lesson.get("video_file_id") and not video_sent:
...
async def send_lesson_note(bot: Bot, chat_id: int, tg_id: int, lesson_id: int) -> bool:
    ...
    if not lesson:
        ...
        msgs = {
            ...
            "need_start": "✨ Сначала нажмите «Начать обучение» на странице предмета.\n\n"
                          "Так вы попадёте в общий рейтинг предмета — и все его уроки откроются.",
        }
        if reason == "need_start":
            try:
                from aiogram.types import WebAppInfo
                from webapp.learning import _site_url_sync
                sid = await asyncio.to_thread(_lesson_subject_id_sync, lesson_id)
                kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
                    text="✨ Открыть предмет и начать обучение",
                    web_app=WebAppInfo(url=f"{_site_url_sync()}/learn/{sid}#start"))]]) if sid else None
                await bot.send_message(chat_id, msgs["need_start"], reply_markup=kb)
            except Exception as e:
                log.warning("need_start message: %s", e)
                await bot.send_message(chat_id, msgs["need_start"])
            return False
    ...
    # Отметка об открытии темы — ровно одна на выдачу конспекта
    await asyncio.to_thread(_mark_note_opened_sync, tg_id, lesson["id"])
    ...
    # 3) Видео урока — сразу после конспекта, тем же порядком и в тот же
    #    реестр: исчезнет из переписки вместе с конспектом по сроку админа.
    video_sent = False
    if lesson.get("video_file_id"):
        try:
            video_sent = await _deliver_video(bot, chat_id, tg_id, lesson, subject)
        except Exception as e:
            log.warning("send note video: %s", e)

    if not text and not sent_images and not video_sent:
        await bot.send_message(chat_id, "ℹ️ У этого урока пока нет конспекта.")

    # 4) Кнопка «ВЫПОЛНИТЬ ДЗ» (и карточки) — под всем материалом
    if video_sent and (text or sent_images):
        done_text = "✅ Конспект и видео-урок отправлены.\n\nКогда изучите — выполните домашнее задание 👇"
    elif video_sent:
        done_text = "✅ Видео-урок отправлен.\n\nКогда посмотрите — выполните домашнее задание 👇"
    else:
        done_text = "✅ Конспект отправлен.\n\nКогда изучите — выполните домашнее задание 👇"
    await bot.send_message(
        chat_id, done_text,
        reply_markup=_hw_keyboard(lesson, video_sent=video_sent), protect_content=True)
...
async def send_lesson_video(bot: Bot, chat_id: int, tg_id: int, lesson_id: int) -> bool:
    ...
    if not lesson.get("video_file_id"):
        ...
        return False
    return await _deliver_video(bot, chat_id, tg_id, lesson, subject)


async def _deliver_video(bot: Bot, chat_id: int, tg_id: int, lesson: dict, subject: dict) -> bool:
    """Само видео: персональная подпись, метка в кадрах (если есть ffmpeg),
    запрет пересылки и запись в реестр на автоудаление. Доступ проверяет
    вызывающий код."""
    file_id = lesson.get("video_file_id")
    if not file_id:
        return False
    user = await asyncio.to_thread(utils.get_user_by_tg, tg_id) or {}
    label = f"ID:{tg_id}" + (f" @{user['username']}" if user.get("username") else "")
    caption = (f"🎬 <b>{utils.escape_html(lesson['title'])}</b>\n"
               f"<i>{utils.escape_html(subject['title'])}</i>\n\n"
               f"👤 Ваша персональная копия: <code>{utils.escape_html(label)}</code>\n"
               f"Переслать, сохранить и заскринить нельзя.")
    # Метка прямо в кадрах — если на сервере есть ffmpeg (см. video_mark_service)
    from services import video_mark_service as _vm
    marked_path = await _vm.make_marked(bot, file_id, label)
    try:
        if marked_path:
            from aiogram.types import FSInputFile
            m = await bot.send_video(chat_id, FSInputFile(marked_path, filename="lesson.mp4"),
