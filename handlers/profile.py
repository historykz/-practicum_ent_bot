...
def _build_profile_text(user: dict, lang: str) -> str:
    ...
    tests_done = db.fetchone(
        ...
        "AND cloned_from_attempt_id IS NULL",        # перенесённые в копию — не новые
        (user['id'],))['c']
    streak = user.get('current_streak') or 0
    best = user.get('best_streak') or 0
    refs = referral_service.count_referrals(user['id'])

    # Раньше здесь смотрели только на то, есть ли строка в базе, — и человек
    # с истёкшим Премиумом видел «Premium активен до <прошедшая дата>».
    ps = utils.premium_status(user['id'])
    if ps["has_row"] and ps["forever"]:
        prem_line = f"{t('profile_premium_on', lang)} ({t('profile_premium_forever', lang)})"
    elif ps["active"]:
        prem_line = (f"{t('profile_premium_on', lang)} {t('profile_premium_until', lang)} "
                     f"{ps['until_date']}")
    elif ps["has_row"] and ps["until_date"]:
        prem_line = (f"{t('profile_premium_off', lang)} "
                     f"({t('profile_premium_expired', lang)} {ps['until_date']})")
    else:
...
# === Номер телефона: сохраняем, если ученик им поделился ===

@router.message(F.contact)
async def msg_contact(message: Message, user: dict):
    """Ученик прислал свой контакт — сохраняем номер (для журнала и связи).
    Чужой контакт игнорируем: в журнале должен быть телефон самого ученика."""
    c = message.contact
    if not c or (c.user_id and c.user_id != message.from_user.id):
        await message.answer("Это чужой контакт — пришлите свой номер кнопкой ниже.")
        return
    ...
    if not phone.startswith("+"):
        phone = "+" + phone
    db.execute("UPDATE users SET phone=? WHERE tg_id=?", (phone, message.from_user.id))
    log.info("Телефон сохранён для %s", message.from_user.id)
...
def phone_request_kb():
    """Клавиатура «Поделиться номером» — вызывать там, где номер нужен."""
