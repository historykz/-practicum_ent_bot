...
def user_has_private_access(test_id: int, user_tg_id: int) -> bool:
    ...
    if not row:
        return False
    # Срок пишется с часовым поясом, а сравнивался с «наивным» utcnow():
    # сравнение падало, и except возвращал True — после дедлайна тест
    # продолжал открываться. Теперь решение общее для бота и сайта.
    return utils.deadline_active(dict(row).get('expires_at'))
...
def list_user_private_tests(user_tg_id: int) -> list[dict]:
    ...
    if utils.is_admin(user_tg_id):
        ...
        # Срок проверяем тем же решением, что и сам вход в тест
        # (user_has_private_access), а не строковым сравнением в SQL.
        rows = []
        for r in db.fetchall(
                """SELECT t.*, p.expires_at AS _access_until FROM tests t
                   JOIN private_test_access p ON p.test_id = t.id
                   WHERE t.is_private=1 AND t.status='active' AND p.user_tg_id=?
                   ORDER BY t.id DESC""",
                (user_tg_id,)):
            r = dict(r)
            if utils.deadline_active(r.pop("_access_until", None)):
                rows.append(r)
...
async def cb_grant_save(call: CallbackQuery, state: FSMContext):
    ...
    if days > 0:
        ...
        expires_at = utils.to_utc_iso(exp_dt)          # единый формат срока
    ...
    # Отложенная выдача: не найденные получат доступ при первом /start
    nf = data.get('grant_not_found') or []
    if nf and tests_to_grant:
        try:
            ...
            for uname in nf:
                added_any = False
                for tst in tests_to_grant:
                    if _pas.add_pending(uname, 'private', tst['id'], days,
                                         call.from_user.id):
                        added_any = True
                if added_any:
                    pend_users += 1
            if pend_users:
                summary += (f"\n\n⏳ <b>В ожидании старта: {pend_users} чел.</b>\n"
                            f"Получат доступ автоматически при первом /start бота.")
        except Exception:
            pass
