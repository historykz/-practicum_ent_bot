...
import utils
from filters import IsAdmin, IsOwner, MinLevel
...
async def cmd_admin(message: Message, state: FSMContext, user: dict):
    ...
    await message.answer(t("admin_menu", lang), reply_markup=admin_menu_kb(lang, user.get('tg_id')))
...
async def cb_admin_menu(call: CallbackQuery, state: FSMContext, user: dict):
    ...
    lang = user.get('language') or 'ru'
    try:
        await call.message.edit_text(t("admin_menu", lang), reply_markup=admin_menu_kb(lang, user.get('tg_id')))
    except Exception:
        await call.message.answer(t("admin_menu", lang), reply_markup=admin_menu_kb(lang, user.get('tg_id')))
    await call.answer()
...
async def cb_my_tests(call: CallbackQuery, user: dict):
    ...
    lang = user.get('language') or 'ru'
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    # Уровень 1 видит только СВОИ тесты; уровень 2+ и владелец — все
    mine_only = utils.admin_level(user.get('tg_id')) < 2
    own = " AND created_by=?" if mine_only else ""
    own_p = (user['id'],) if mine_only else ()
    cats = db.fetchall("SELECT * FROM test_categories ORDER BY sort_order, id")
    kb = InlineKeyboardBuilder()
    total_tests = 0
    for c in cats:
        cnt = db.fetchone(
            f"SELECT COUNT(*) AS c FROM tests WHERE category_id=?{own}",
            (c['id'], *own_p))['c']
        if cnt == 0:
            continue
        total_tests += cnt
    ...
    # Тесты без раздела
    no_cat = db.fetchone(
        f"SELECT COUNT(*) AS c FROM tests WHERE category_id IS NULL{own}", own_p)['c']
    if no_cat:
    ...
    total = db.fetchone(
        f"SELECT COUNT(*) AS c FROM tests WHERE 1=1{own}", own_p)['c']
    if total == 0:
...
async def cb_admin_cat_tests(call: CallbackQuery, user: dict):
    ...
    if arg == "none":
        rows = db.fetchall(
            f"SELECT * FROM tests WHERE category_id IS NULL{own} ORDER BY id DESC", own_p)
        cat_title = "📭 Без раздела"
    else:
        cat_id = int(arg)
        cat = db.fetchone("SELECT * FROM test_categories WHERE id=?", (cat_id,))
        rows = db.fetchall(
            f"SELECT * FROM tests WHERE category_id=?{own} ORDER BY id DESC",
            (cat_id, *own_p))
...
async def _show_test_admin_card(bot, chat_id: int, test_id: int, lang: str = 'ru'):
    ...
    st = _sts.test_stats(test_id)
    avg_line = f"{st['avg_percent']}%" if st['avg_percent'] is not None else "—"
    text = (
        f"<b>{title_safe}</b>\n\n"
        f"ID: {test_id}\n"
        ...
        f"📊 <b>Статистика</b>\n"
        f"Начали: {st['started']} · Дошли до конца: {st['finished']} "
        f"({st['completion_rate']}%)\n"
        f"Средний результат: {avg_line}\n"
        f"🟢 Проходят сейчас: {st['active']} · ⏸ На паузе: {st['paused']} · "
        f"Не завершили: {st['aborted']}"
    )
...
@router.callback_query(F.data == "adm:import_file", IsOwner())
...
@router.callback_query(F.data == "adm:import_text", MinLevel(2))
...
async def cb_import_text_done(call: CallbackQuery, state: FSMContext, user: dict):
    ...
    if not tid or not buf.strip():
        await state.clear()
        await call.message.answer("Нечего сохранять.", reply_markup=admin_menu_kb(lang, user.get('tg_id')))
        await call.answer()
        return
...
async def cb_import_poll_done(call: CallbackQuery, state: FSMContext, user: dict):
    ...
    if not tid or not buf:
        await state.clear()
        await call.message.answer("В буфере пусто.", reply_markup=admin_menu_kb(lang, user.get('tg_id')))
        await call.answer()
        return
...
async def s_grant_test(message: Message, state: FSMContext, user: dict):
    ...
    await state.clear()
    await message.answer(t("grant_done", lang), reply_markup=admin_menu_kb(lang, user.get('tg_id')))
...
@router.callback_query(F.data == "adm:premium", IsOwner())
...
async def cb_premium_list(call: CallbackQuery, user: dict):
    ...
    if not rows:
        await call.message.answer(
            ...
            reply_markup=admin_menu_kb(user.get('language') or 'ru', user.get('tg_id')))
        await call.answer()
        return
    ...
    active_lines = []
    expired_lines = []
    # Если utils.py на сервере старее этого файла, общего разбора сроков нет —
    # раньше это молча превращалось в «Статус: ?» у каждого ученика.
    describe = getattr(utils, "describe_deadline", None)
    if describe is None:
        await call.message.answer(
            "⚠️ <b>На сервере устаревший utils.py</b> — сроки Премиума посчитать "
            "нельзя. Замените папку проекта целиком из последнего архива и "
            "перезапустите бота.", parse_mode="HTML")
    for r in rows:
        uname = ("@" + r['username']) if r['username'] else (r['first_name'] or f"id{r['tg_id']}")
        granted = (r['granted_at'] or "")[:10]
        exp = r['expires_at']

        # Статус и оставшиеся дни — то же решение, что у проверки доступа
        d = describe(exp, now) if describe else None
        if d is None:
            status = "♾ бессрочно" if not exp else f"⚠️ {utils.escape_html(str(exp))}"
            days_left_str = "—"
            is_active = not exp
        elif d["forever"]:
            status = "♾ бессрочно"
            ...
            status = (f"⚠️ дата не читается: «{utils.escape_html(str(exp))}» — "
                      f"доступ закрыт, выдайте заново")
            days_left_str = "—"
            is_active = False
        elif d["active"]:
            days_left_str = (f"{d['days_left']} дн." if d["days_left"] > 0
                             else f"{d['hours_left']} ч.")
            status = f"✅ до {d['until']} (Астана)"
            is_active = True
        else:
            status = f"❌ истёк {d['until']} (Астана)"
            days_left_str = "—"
            is_active = False

        # Сколько дней УЖЕ прошло с момента выдачи
        _parse = getattr(utils, "parse_utc_naive", None)
        granted_dt = _parse(r['granted_at']) if _parse else None
        passed_str = f"{(now - granted_dt).days} дн. назад" if granted_dt else "—"
...
async def s_premium_revoke(message: Message, state: FSMContext, user: dict):
    ...
    if not info:
        await message.answer(
            ...
            reply_markup=admin_menu_kb(lang, user.get('tg_id')))
        await state.clear()
        return
    utils.revoke_premium(target['id'])
    # Вознаграждение по предметам аннулируется, из рейтинга выбывает сразу
    import asyncio as _aio
    from services import reward_service as _rs
    await _aio.to_thread(_rs.check_user, target['tg_id'])
...
async def s_premium_user(message: Message, state: FSMContext, user: dict):
    ...
    if info:
        ...
        if not exp:
            ...
            try:
                exp_dt = utils.parse_utc_naive(exp)
                if exp_dt is None:
...
async def s_premium_days(message: Message, state: FSMContext, user: dict):
    ...
    # ── Уведомление самому пользователю: поздравление, затем инструкция ──
    if target and target.get('tg_id'):
        from services import onboarding_service as _onb
        res = await _onb.notify_manual_grant(message.bot, target['tg_id'], days,
                                             source="admin_bot")
        if not res.get("congrats"):
            await message.answer("⚠️ Premium выдан, но уведомление не дошло: "
                                 "человек ещё не открывал бота или заблокировал его.")
        elif not res.get("instruction"):
            await message.answer("ℹ️ Поздравление ушло; инструкция не отправлена "
                                 "(выключена в настройках или уже уходила только что).")
...
@router.callback_query(F.data == "adm:block", IsOwner())
...
async def s_block(message: Message, state: FSMContext, user: dict):
    ...
    await message.answer(msg, reply_markup=admin_menu_kb(lang, user.get('tg_id')))
...
@router.callback_query(F.data == "adm:channels", IsOwner())
...
async def s_channel(message: Message, state: FSMContext, user: dict):
    ...
    if cat_id:
        ...
        await message.answer(t("channel_added", lang), reply_markup=admin_menu_kb(lang, user.get('tg_id')))
    await state.clear()
...
@router.callback_query(F.data == "adm:export", IsOwner())
...
async def cb_export_users(call: CallbackQuery, user: dict):
    ...
    rows = db.fetchall("""
        SELECT u.tg_id, u.username, u.first_name, u.language, u.school, u.city,
               ...
               (SELECT COUNT(*) FROM test_attempts a WHERE a.user_id=u.id AND a.status='finished'
                  AND a.cloned_from_attempt_id IS NULL) AS attempts,
...
async def cb_export_results(call: CallbackQuery, user: dict):
    ...
    rows = db.fetchall("""
        ...
        WHERE a.status IN ('finished', 'aborted') AND a.cloned_from_attempt_id IS NULL
...
async def cb_admins_list(call: CallbackQuery, state: FSMContext, user: dict):
    ...
    rows = db.fetchall(
        """SELECT a.tg_id, a.granted_by, a.created_at, a.level, u.username, u.first_name
           FROM admins a
           LEFT JOIN users u ON u.tg_id = a.tg_id
           ORDER BY a.created_at DESC""")
    lines.append("\n🛠 <b>Добавленные через бот:</b> (нажми чтобы сменить уровень)")
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    kb = InlineKeyboardBuilder()
    if rows:
        for r in rows:
            ...
            lvl = r.get('level') or 1
            lines.append(f"  • {label}  <i>· {lvl} ур.</i>")
            btn_label = (f"@{r['username']}" if r['username'] else str(r['tg_id']))[:20]
            kb.button(text=f"⚙️ {btn_label} — {lvl} ур.",
                      callback_data=f"admlvl:{r['tg_id']}:{2 if lvl == 1 else 1}")
    else:
...
def _admin_level_kb(tg_id: int):
    """Кнопки выбора должности (уровня) админа."""
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    kb = InlineKeyboardBuilder()
    cur = utils.admin_level(tg_id)
    kb.button(text=("✅ " if cur == 1 else "") + "1 ур. — только тесты",
              callback_data=f"admlvl:{tg_id}:1")
    kb.button(text=("✅ " if cur == 2 else "") + "2 ур. — тесты + импорт txt",
              callback_data=f"admlvl:{tg_id}:2")
    kb.button(text="↩️ К админам", callback_data="adm:admins")
    kb.adjust(1)
    return kb.as_markup()


@router.callback_query(F.data.startswith("admlvl:"), IsOwner())
async def cb_admin_set_level(call: CallbackQuery, user: dict):
    """Главный админ выбирает должность (уровень) бот-админа."""
    _, tg_s, lvl_s = call.data.split(":")
    tg_id, lvl = int(tg_s), int(lvl_s)
    if lvl not in (1, 2):
        await call.answer()
        return
    db.execute("UPDATE admins SET level=? WHERE tg_id=?", (lvl, tg_id))
    name = "тесты" if lvl == 1 else "тесты + импорт txt"
    try:
        await call.message.edit_text(
            f"✅ Должность обновлена: <b>{lvl} уровень</b> ({name})\n"
            f"tg_id: <code>{tg_id}</code>",
            reply_markup=_admin_level_kb(tg_id), parse_mode="HTML")
    except Exception:
        pass
    try:
        await call.bot.send_message(
            tg_id, f"🔔 Ваш уровень админа изменён на <b>{lvl}</b> ({name}).",
            parse_mode="HTML")
    except Exception:
        pass
...
async def s_admin_add(message: Message, state: FSMContext, user: dict):
    ...
    if not target:
        ...
        if arg.isdigit():
            ...
            await message.answer(
                ...
                f"Выберите <b>должность</b> (по умолчанию 1 уровень):",
                parse_mode="HTML",
                reply_markup=_admin_level_kb(tg_id))
            await state.clear()
            # Попробуем уведомить
            try:
    ...
    await message.answer(
        ...
        f"Выберите <b>должность</b> (по умолчанию 1 уровень):",
        parse_mode="HTML",
        reply_markup=_admin_level_kb(target['tg_id']))
    await state.clear()
...
# Ловит любой adm:* callback который не отловлен НИКЕМ — снимает «загрузку».
# ВАЖНО: живёт в отдельном роутере, который подключается последним. Иначе он
# перехватывал бы adm:conspects и adm:premium_settings у их собственных
# хендлеров (они лежат в роутерах, идущих после admin) и выдавал бы
# «функция временно недоступна».

fallback_router = Router(name="admin_fallback")
...
async def cmd_pending(message: Message, user: dict):
    """Кто в ожидании отложенной выдачи (ещё не нажал /start)."""
    from services import pending_access_service as _pas
    items = _pas.list_pending(30)
    total = _pas.pending_count()
    if not items:
        await message.answer(
            "⏳ Никто не ждёт.\n\n"
            "Когда при выдаче доступа кто-то «не найден» — он попадает сюда "
            "и получает доступ автоматически при первом /start.")
        return
    lines = [f"⏳ <b>Ожидают первого запуска: {total} чел.</b>\n"]
    for it in items:
        what = f"тест «{it.get('title') or '?'}»" if it['kind'] == 'private' else "премиум"
        dur = "бессрочно" if not it.get('days') else f"{it['days']} дн."
        lines.append(f"• @{it['username']} — {what}, {dur}")
    if total > len(items):
        lines.append(f"… и ещё {total - len(items)}")
