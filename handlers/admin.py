...
async def s_premium_days(message: Message, state: FSMContext, user: dict):
    ...
    # Инструкция должна прийти и при ручной выдаче — ровно та же, что при покупке
    try:
        from services import onboarding_service as _onb
        _t = db.fetchone("SELECT tg_id FROM users WHERE id=?", (uid,))
        if _t and _t.get("tg_id"):
            await _onb.premium_activated(message.bot, _t["tg_id"], days,
                                         source="admin_bot")
    except Exception as _e:
        log.warning("premium onboarding (ручная выдача): %s", _e)
