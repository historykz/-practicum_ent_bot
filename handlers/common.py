...
async def cmd_start_deep(message: Message, command: CommandObject, state: FSMContext, user: dict):
    ...
    # Отложенная выдача: если админ внёс этого юзера заранее — применяем
    try:
        from services import pending_access_service as _pas
        await _pas.activate_for_user(message.bot, user['tg_id'],
    ...
    # === Сразу открыть предложение Премиума (start=premium) ===
    # Используется кнопкой "Приобрести" с сайта — карточки/заучивание за Премиум.
    if arg == "premium" and message.chat.type == "private":
        if not user.get('language'):
            await state.update_data(pending={'open_premium': True})
    ...
    # === Витрина предмета (start=subj_<id>) — ссылка для канала ===
    # Админ кидает такую ссылку в канал: человек жмёт, попадает в бота (значит
    # уже зарегистрирован) и сразу видит карточку предмета с каталогом уроков.
    if arg.startswith("subj_") and message.chat.type == "private":
        try:
            _sid = int(arg[5:])
        except ValueError:
            _sid = 0
        if _sid:
            from handlers.subject_promo import send_subject_promo
            if await send_subject_promo(message, _sid):
                return

    # === Конспект урока (start=lesson_<id>) — выдача в личку с водяным знаком ===
    if arg.startswith("lesson_") and message.chat.type == "private":
        try:
            _lid = int(arg[7:])
        except ValueError:
            _lid = 0
        if _lid:
            ...
            await send_lesson_note(message.bot, message.chat.id, message.from_user.id, _lid)
            return

    # === Подборка вопросов из канала (start=pub_936) ===
    # Обычно ссылка открывает мини-приложение сама, но если человек пришёл
    # в переписку с ботом — даём кнопку, которая откроет тест на весь экран.
    if arg.startswith("pub_") and message.chat.type == "private":
        try:
            _pid = int(arg[4:])
        except ValueError:
            _pid = 0
        if _pid:
            ...
            pub = _ps.get(_pid)
            if not pub:
                await message.answer("Тест не найден — возможно, публикацию удалили.")
                return
            info = _ps.describe(pub)
            title = info["lesson_title"] or info["test_title"] or "Тест"
            from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
            kb = InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="▶️ Начать тест",
                                     url=_ps.publication_link(_pid))
            ]])
            await message.answer(
                f"📝 <b>{title}</b>\n"
                f"Вопросов: <b>{info['count']}</b>\n\n"
                f"Тест откроется в приложении на весь экран 👇",
                parse_mode="HTML", reply_markup=kb)
            return

    # === Покупка урока/раздела за звёзды (start=buyles_X / buysec_X) ===
    # Сюда ведут кнопки из мини-приложения. Backend сам решает, продавать ли:
    # выключена продажа — счёта не будет, какой бы ссылкой ни пришли.
    if (arg.startswith("buyles_") or arg.startswith("buysec_")) \
            and message.chat.type == "private":
        from services import unit_sales as us
        import json as _json
        is_lesson = arg.startswith("buyles_")
        try:
            obj_id = int(arg.split("_", 1)[1])
        except ValueError:
            obj_id = 0
        allowed = (us.sale_enabled_for_lesson(obj_id) if is_lesson
                   else us.sale_enabled_for_section(obj_id))
        if not obj_id or not allowed:
            await message.answer("🔒 Отдельная покупка этого материала недоступна. "
                                 "Посмотрите Премиум — он открывает всё сразу.")
            return
        price = us.lesson_price(obj_id) if is_lesson else us.section_price(obj_id)
        import database as db          # модуль импортируется точечно, как и в соседних ветках
        title_row = db.fetchone(
            "SELECT title FROM lessons WHERE id=?" if is_lesson
            else "SELECT title FROM sections WHERE id=?", (obj_id,))
        title = (title_row or {}).get("title") or "Материал"
        payload = _json.dumps(
            {"k": "unit_lesson" if is_lesson else "unit_section",
             ("l" if is_lesson else "s"): obj_id}, separators=(",", ":"))
        try:
            await message.bot.send_invoice(
                chat_id=message.chat.id,
                title=(f"Урок: {title}" if is_lesson else f"Раздел: {title}")[:32],
                description=("Доступ к уроку: конспект, тест и материалы."
                             if is_lesson else
                             "Доступ ко всем урокам и конспектам раздела."),
                payload=payload, currency="XTR",
                prices=[LabeledPrice(label="Доступ", amount=price)])
        except Exception as e:
            await message.answer(f"⚠️ Не удалось выставить счёт: {e}")
        return

    # === Видео урока (start=video_<id>) — в личку, без пересылки ===
    if arg.startswith("video_") and message.chat.type == "private":
        try:
            _vid = int(arg[6:])
        except ValueError:
            _vid = 0
        if _vid:
            from handlers.lesson_notes import send_lesson_video
            await send_lesson_video(message.bot, message.chat.id,
                                    message.from_user.id, _vid)
            return
...
async def _apply_pending_and_show_menu(message: Message, state: FSMContext, user: dict):
    ...
    # Сразу открыть предложение Премиума
    if pending.get('open_premium'):
        from handlers.premium import build_premium_offer
        text, markup = build_premium_offer(user)
        await message.answer(text, reply_markup=markup, parse_mode="HTML")
...
async def cb_help(call: CallbackQuery, user: dict):
    lang = _resolve_lang(user)
    text = t("help_text", lang, manager="@" + utils.manager_username())
    try:
...
@router.callback_query(F.data.in_({"m:support", "support:open"}))
async def cb_support(call: CallbackQuery, user: dict = None):
    """Поддержка: контакт менеджера и текст берём из настроек, не из кода.

    Поменяли ник менеджера в админке — все кнопки сразу ведут на новый
    аккаунт, обновлять бота не нужно.
    """
    from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
    from services import study_settings as ss

    lang = _resolve_lang(user) if user else "ru"
    manager = utils.manager_username()

    text = (ss.get("support_text") or "").strip()
    if not text:
        text = t("support_text", lang, manager=("@" + manager) if manager else "—")
    elif manager:
