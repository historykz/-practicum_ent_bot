...
async def cb_tests_menu(call: CallbackQuery, user: dict):
    ...
    # Приватные разделы убираем из каталога: их видят только те, кому админ
    # выдал доступ. Иначе «Другое» открывало бы вообще всё, включая закрытые.
    from services import category_service as _cs
    cats = [c for c in cats if _cs.has_access(c['id'], call.from_user.id)]

    required_cats = [c for c in cats if c.get('is_required')]
    if has_other:
        # «Другое» — показываем ВСЕ доступные профильные разделы
    ...
    # Если профильных нет или их стало меньше нужного (например, админ закрыл
    # раздел и снял его у всех) — предлагаем выбрать заново.
    from handlers.profile_subjects import REQUIRED_COUNT as _NEED
    _weight = len(profile_ids) + (2 if has_other else 0)
    if _weight < _NEED:
...
async def cb_tests_by_category(call: CallbackQuery, user: dict):
    ...
    if arg == "none":
        ...
        if not cat:
            ...
            return
        # Приватный раздел открывается только тем, кому его выдали
        from services import category_service as _cs
        if not _cs.has_access(cat_id, call.from_user.id):
            await call.answer("Раздел не найден.", show_alert=True)
            return
...
async def cb_private_tests_list(call: CallbackQuery, user: dict):
    ...
    from services import category_service as _cs
    cats = [c for c in db.fetchall("SELECT * FROM test_categories ORDER BY sort_order, name")
            if _cs.has_access(c['id'], call.from_user.id)]
...
async def show_test_card(bot: Bot, chat_id: int, user_tg_id: int, test_id: int, lang: str):
    ...
    # Раздел, в котором лежит тест, тоже может быть закрыт — проверяем и это.
    # Тест внутри закрытого раздела обычно НЕ помечен is_private сам по себе
    # (эти два признака разные), поэтому без этой проверки карточка утекала
    # бы через inline-поиск и прямую ссылку в обход самого каталога.
    if test.get('category_id'):
        from services import category_service as _cs
        if not _cs.has_access(test['category_id'], user_tg_id):
            await bot.send_message(
                chat_id, t("test_not_found", lang),
                reply_markup=back_kb(lang, "m:tests"))
            return
    ...
    if test['is_paid'] and not has_access:
        ...
        if not _rs.user_can_unlock_paid_test(user['id']):
            ...
            await bot.send_message(
                chat_id,
                t("paid_test_card", lang, price=test['price'], manager=utils.manager_username()),
                reply_markup=paid_test_kb(
                    test['id'], lang, utils.manager_username(),
...
async def cb_run_test(call: CallbackQuery, state: FSMContext, user: dict):
    ...
    # Раздел теста тоже может быть закрыт (см. show_test_card) — не даём
    # запустить тест в обход карточки, если её как-то обошли.
    if test.get('category_id'):
        from services import category_service as _cs
        if not _cs.has_access(test['category_id'], call.from_user.id):
            await call.answer(t("test_not_found", lang), show_alert=True)
            return
    ...
    if test['is_paid'] and not utils.has_paid_access(user['id'], test_id=test['id']):
        ...
        from services import referral_service as _rs
        if not _rs.user_can_unlock_paid_test(user['id']):
            await call.answer(t("noaccess_test", lang, manager=utils.manager_username()),
                              show_alert=True)
            return
...
async def cb_my_results(call: CallbackQuery, user: dict):
    ...
    rows = db.fetchall(
        """SELECT a.id, a.score, a.correct_answers, a.wrong_answers, a.skipped_answers,
           ...
           WHERE a.user_id=? AND a.status='finished' AND a.cloned_from_attempt_id IS NULL
