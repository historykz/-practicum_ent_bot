...
async def cb_announce_ask(call: CallbackQuery):
    ...
    if test.get('is_private'):
        ...
        return
    # Раздел тоже может быть закрытым: рассылать такой тест всем подряд
    # означало бы раскрыть материал, доступ к которому выдаётся поимённо.
    if test.get('category_id'):
        from services import category_service as _cs
        _cat = db.fetchone("SELECT is_private FROM test_categories WHERE id=?",
                           (test['category_id'],))
        if _cat and _cat.get('is_private'):
            await call.answer(
                "🔒 Раздел закрытый — анонс всем не отправляется.\n"
                "Доступ к нему выдаётся поимённо.", show_alert=True)
            return
    qcount = db.fetchone(
    ...
    for u in targets:
        ...
        if is_paid:
            ...
            rows.append([InlineKeyboardButton(
                ...
                url=f"https://t.me/{utils.manager_username()}")])
