...
async def handle_premium_payment(message: Message, bot: Bot, pl: dict,
    ...
    if not user:
        ...
        return
    # Один и тот же чек не должен выдавать Премиум дважды: Telegram может
    # прислать подтверждение повторно, да и мы сами можем перезапуститься.
    if charge:
        dup = db.fetchone("SELECT id FROM purchases WHERE charge_id=?", (charge,))
        if dup:
            log.info("Платёж %s уже обработан — повторно не начисляем", charge)
            from services.premium_welcome import send_after_purchase as _sap
            exp_row = db.fetchone("SELECT expires_at FROM premium_users WHERE user_id=?",
                                  (user['id'],))
            until_s = ""
            if exp_row and exp_row.get('expires_at'):
                try:
                    from utils import parse_utc_naive as _p
                    from datetime import timezone as _tz
                    until_s = _p(exp_row['expires_at']).replace(
                        tzinfo=_tz.utc).astimezone(ALMATY).strftime("%Y-%m-%d")
                except Exception:
                    pass
            # сообщения могли не уйти в прошлый раз — попробуем ещё, дублей не будет
            await _sap(bot, tg_id, days, until_s, charge)
            try:
                from services import onboarding_service as _onb
                await _onb.premium_activated(bot, tg_id, days, source="stars",
                                             event_key=charge)
            except Exception as e:
                log.warning("premium onboarding (повтор чека): %s", e)
            return
    ...
    if exp and exp.get('expires_at'):
        try:
            from utils import parse_utc_naive as _p
            from datetime import timezone as _tz
            dt = _p(exp['expires_at']).replace(tzinfo=_tz.utc)
            exp_label = dt.astimezone(ALMATY).strftime("%d.%m.%Y")
            exp_label_iso = dt.astimezone(ALMATY).strftime("%Y-%m-%d")
        except Exception:
            pass
    # Поздравление и форма Quizlet: тексты и кнопки настраиваются в панели
    try:
        from services.premium_welcome import send_after_purchase as _sap
        await _sap(bot, tg_id, days, exp_label_iso, charge)
    except Exception as e:
        log.warning("premium welcome: %s", e)
    # Инструкция из админки — строго после поздравления. Ключ события — чек:
    # повторное подтверждение того же платежа инструкцию второй раз не пришлёт.
    try:
        from services import onboarding_service as _onb
        await _onb.premium_activated(bot, tg_id, days, source="stars",
                                     event_key=charge or None)
    except Exception as e:
        log.warning("premium onboarding: %s", e)
