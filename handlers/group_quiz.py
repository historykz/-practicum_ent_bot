    ...
    if just_added and actor_id and utils.is_admin(actor_id):
        try:
            recent = db.fetchall(
                ...
                "AND COALESCE((SELECT c.is_private FROM test_categories c "
                "              WHERE c.id = tests.category_id), 0)=0 "
...
def _cancel_chat_series(chat_id) -> list:
    try:
        from services import autopub_service
        return autopub_service.cancel_series_in_chat(chat_id, "остановлена командой /stop в чате")
    except Exception as e:
        logging.getLogger(__name__).warning("/stop: серия: %s", e)
        return []


async def _is_group_admin(bot: Bot, chat_id, user_id) -> bool:
    try:
        m = await bot.get_chat_member(chat_id, user_id)
        return str(getattr(m.status, "value", m.status)) in ("creator", "administrator")
    except Exception:
        return False


async def _after_series_stop(bot: Bot, message: Message, stopped: list) -> None:
    if not stopped:
        return
    try:
        from services import autopub_service
        if any(s.get("chat_locked") for s in stopped):
            await autopub_service._unlock_chat(bot, message.chat.id)
        await message.reply(f"🛑 <b>Серия тестов остановлена</b> — следующие тесты серии не запустятся.",
                            parse_mode="HTML")
    except Exception as e:
        logging.getLogger(__name__).warning("/stop: после серии: %s", e)
...
async def cmd_stop_group(message: Message, bot: Bot):
    ...
    if requester_tg_id is None and message.sender_chat:
        ...
        if message.sender_chat.id == message.chat.id:
            ...
            stopped = _cancel_chat_series(message.chat.id)
            ok, key = await group_quiz_service.stop_quiz(
                bot, message.chat.id, requester_tg_id=0)
            await _after_series_stop(bot, message, stopped)
    ...
    # Серия тестов этого чата останавливается ДО текущего теста: иначе его
    # завершение запустило бы следующий тест серии через 20–30 сек.
    stopped = []
    if utils.is_admin(requester_tg_id) or await _is_group_admin(bot, message.chat.id, requester_tg_id):
        stopped = _cancel_chat_series(message.chat.id)
...
async def cmd_launch_test(message: Message, m):
    ...
    # Тест из закрытого раздела в группе запускает только админ бота.
    # Анонимный админ группы или репост из канала — это ещё не основание
    # показать всей группе материал, закрытый для посторонних.
    if test.get('category_id') and not is_bot_admin:
        from services import category_service as _cs
        if not _cs.has_access(test['category_id'], from_user_id or 0):
            await message.reply("❌ Тест не найден.")
            return
