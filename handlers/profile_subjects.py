...
def get_optional_categories(tg_id: int = None) -> list:
    """Категории-предметы которые можно выбрать (НЕ обязательные).

    Приватные разделы в этом списке не показываем: их видят только админы
    и те, кому доступ выдали поимённо.
    """
    if tg_id is not None:
        from services import category_service as cs
        return cs.visible_categories(tg_id, only_optional=True)
    try:
        return db.fetchall(
            "SELECT * FROM test_categories WHERE COALESCE(is_required,0)=0 "
            "AND COALESCE(is_private,0)=0 ORDER BY sort_order, id")
    except Exception:
        # База ещё без колонки is_private — работаем как раньше
        return db.fetchall(
            "SELECT * FROM test_categories WHERE COALESCE(is_required,0)=0 "
...
def _subjects_kb(selected: set, lang: str, from_profile: bool,
                 tg_id: int = None) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    cats = get_optional_categories(tg_id)
...
async def show_subjects_screen(call: CallbackQuery, state: FSMContext,
    ...
    cats = get_optional_categories(call.from_user.id)
    if not cats:
    ...
    if from_profile and not selected:
        ...
        # Если раздел закрыли уже после выбора, он пропадает с экрана —
        # и человек не смог бы его снять, чтобы выбрать другой предмет.
        # Поэтому недоступные предметы из выбора убираем.
        from services import category_service as _cs
        selected = {x for x in selected
                    if x == 'other' or _cs.has_access(x, call.from_user.id)}
    ...
    kb = _subjects_kb(selected, lang, from_profile, call.from_user.id)
    try:
...
async def cb_toggle_subject(call: CallbackQuery, state: FSMContext):
    ...
    if item is None:
        try:
            ...
            await call.answer()
            return
        # Кнопки закрытого раздела на экране нет, но её можно подделать —
        # поэтому решает сервер, а не то, что пришло от клиента.
        from services import category_service as _cs
        if not _cs.has_access(item, call.from_user.id):
            await call.answer("Этот предмет сейчас недоступен.", show_alert=True)
            return
    data = await state.get_data()
    selected = set(data.get('psub_selected') or [])
    lang = _resolve_lang(call.from_user.id)
