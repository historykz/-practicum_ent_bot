...
def main_menu_kb(lang: str, is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardBuilder()
    # 🔥 Заметные кнопки — каждая на всю ширину, вверху
    kb.row(InlineKeyboardButton(
        text="📚✨ ПРОЙТИ ТЕСТЫ ✨📚", callback_data="m:tests"))
    kb.row(InlineKeyboardButton(
        text="📖🔥 КОНСПЕКТЫ ЕНТ 🔥📖", callback_data="m:conspects"))
    kb.row(InlineKeyboardButton(
        text="💎🔥 ПРЕМИУМ-ПОДПИСКА 🔥💎", callback_data="buy:premium"))
    # Остальные — по 2 в ряд
    kb.row(
        InlineKeyboardButton(text=t("btn_duel", lang), callback_data="m:duel"),
        InlineKeyboardButton(text=t("btn_profile", lang), callback_data="m:profile"))
    kb.row(
        InlineKeyboardButton(text="🎁 Пригласить друзей", callback_data="m:referral"),
        InlineKeyboardButton(text=t("btn_support", lang), callback_data="m:support"))
    kb.row(InlineKeyboardButton(text=t("btn_help", lang), callback_data="m:help"))
    if is_admin:
        kb.row(InlineKeyboardButton(text="🛠 Admin", callback_data="m:admin"))
    return kb.as_markup()
...
def profile_kb(lang: str) -> InlineKeyboardMarkup:
    ...
    # Инструкцию можно пересмотреть в любой момент — админ мог обновить видео
    kb.button(
        text=("🎥 Платформаны қалай пайдалану" if lang == "kz"
              else "🎥 Как пользоваться платформой"),
...
def options_kb(attempt_id: int, question_id: int, options: list[dict],
               picked: list = None) -> InlineKeyboardMarkup:
    """Кнопки вариантов ответа для одного вопроса теста.

    Нажатие только ставит или снимает отметку, ответ засчитывается кнопкой
    «Ответить». Так любой вопрос выглядит одинаково: ученик сам решает,
    сколько вариантов отметить, и по виду кнопок не может понять, сколько
    правильных ответов на самом деле.
    """
    picked = set(picked or [])
    kb = InlineKeyboardBuilder()
    for i, opt in enumerate(options):
        # callback хранит attempt_id и option_id (НЕ is_correct, чтобы не дать подсмотреть)
        mark = "☑️ " if opt["id"] in picked else "▫️ "
        kb.button(
            text=f"{mark}{chr(ord('A') + i)}) {opt['text'][:58]}",
            callback_data=f"ans:{attempt_id}:{question_id}:{opt['id']}",
        )
    # Подтверждение — одна и та же кнопка для всех вопросов
    kb.button(text="✅ Ответить" if picked else "✅ Ответить (выберите вариант)",
              callback_data=f"ansok:{attempt_id}:{question_id}")
...
def admin_menu_kb(lang: str, tg_id: int = None) -> InlineKeyboardMarkup:
    # Уровень доступа: 3 = владелец (всё), 2 = тесты + импорт txt, 1 = только тесты
    import utils
    level = utils.admin_level(tg_id) if tg_id else 3

    kb = InlineKeyboardBuilder()
    # Базовое — доступно всем админам
    ...
    if level >= 3:
        ...
        kb.button(text="🎯 Контроль обучения", callback_data="adm:study")
        kb.button(text="📊 Статистика", callback_data="adm:stats")
        kb.button(text="📖 Конспекты уроков", callback_data="na:home")
        kb.button(text="📕 Конспекты ЕНТ (продажа)", callback_data="adm:conspects")
        kb.button(text="📚 Тексты и автонапоминания", callback_data="adm:notes_msgs")
