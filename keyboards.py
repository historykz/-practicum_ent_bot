...
def question_controls_kb(attempt_id: int, question_id: int, lang: str) -> InlineKeyboardMarkup:
    """Управление тестом над родным опросом: пауза, апелляция, стоп.

    Варианты ответа здесь не выбираются — для этого сам Telegram Poll.
    """
    kb = InlineKeyboardBuilder()
    kb.button(text=t("btn_pause_test", lang), callback_data=f"tpz:{attempt_id}")
    kb.button(text=t("btn_appeal", lang), callback_data=f"tap:{attempt_id}:{question_id}")
    kb.button(text=t("btn_stop_test", lang), callback_data=f"abort:{attempt_id}")
    kb.adjust(2, 1)
    return kb.as_markup()
