...
from webapp import shortcuts as sc, learning
...
def _lesson_mode_access_sync(lesson_id: int, tg_id):
    ...
    raw = db.fetchone("SELECT * FROM lessons WHERE id=? AND status='open'", (lesson_id,))
    if not raw:
        return None, "not_found"
    # Копия-ярлык своего теста не имеет — он у оригинала. Без этого карточки
    # и заучивание на копиях отдавали 404.
    lesson = sc.resolve_lesson(raw)
    if not lesson or not lesson.get("test_id"):
        return None, "not_found"
    ...
    # Карточки и Заучивание — всегда только по Премиуму (независимо от того,
    # платный сам урок или нет), как и в боте.
    if not utils.is_admin(tg_id):
        user = utils.get_user_by_tg(tg_id)
        if not user or not utils.is_premium(user["id"]):
