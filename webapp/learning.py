...
def _lesson_subject_ids_sync(lesson: dict) -> list:
    ...
    """
    ids = []
    for lid in {lesson.get("id"), lesson.get("copy_id"),
...
async def admin_edit_subject(request: web.Request) -> web.Response:
    ...
    if title:
        await asyncio.to_thread(
            ...
            "unit_sale_enabled=?, unit_price_stars=? WHERE id=?",
...
async def admin_copy_subject_full(request: web.Request) -> web.Response:
    """Независимая копия предмета со всем содержимым (services/deep_copy_service)."""
    await _require_admin(request)
    data = await request.post()
    subject_id = int(request.match_info["subject_id"])
    from services import deep_copy_service as _dc
    new_id = await asyncio.to_thread(
        _dc.copy_subject_full, subject_id, (data.get("title") or "").strip())
    if not new_id:
        raise _back(data, "Предмет не найден")
    ...
    raise web.HTTPFound(
        _hier_url(new_id) + "&message=" + _q(
            f"Создана полная копия: разделов {stats['sections']}, уроков {stats['lessons']}, "
            f"вопросов {stats['questions']}. Оригинал и копия теперь независимы"))
