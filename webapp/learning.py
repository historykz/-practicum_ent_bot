...
async def admin_create_subject(request: web.Request) -> web.Response:
    ...
    if title:
        await asyncio.to_thread(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (title, description, is_open, is_private, tg_id, min_read, seq, pass_pct, live,
...
async def admin_edit_subject(request: web.Request) -> web.Response:
    ...
    if title:
        await asyncio.to_thread(
            ...
            (title, description, is_open, is_private, min_read, seq, pass_pct, live,
    ...
    if title:
        if update_video:
            await asyncio.to_thread(
                ...
                "video_url=?, youtube_id=?, quizlet_url=? WHERE id=?",
            ...
            await asyncio.to_thread(
                ...
                "UPDATE lessons SET title=?, description=?, content_html=?, status=?, is_paid=?, "
                "quizlet_url=? WHERE id=?",
                (title, description, content_html, status, is_paid, quizlet_url, lesson_id),
            )
