...
async def admin_question_save(request: web.Request) -> web.Response:
    ...
    def _save():
        ...
        if correct_raw == "__new__" and new_id:
            db.execute("UPDATE question_options SET is_correct=0 WHERE question_id=?",
                       (question_id,))
            db.execute("UPDATE question_options SET is_correct=1 WHERE id=?", (new_id,))
        elif correct_raw and str(correct_raw).isdigit():
            db.execute("UPDATE question_options SET is_correct=0 WHERE question_id=?",
                       (question_id,))
            db.execute("UPDATE question_options SET is_correct=1 WHERE id=? "
                       "AND question_id=?", (int(correct_raw), question_id))
        return q["test_id"], ""
