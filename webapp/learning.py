...
def _needs_start_sync(subject_id: int, tg_id) -> bool:
    ...
    try:
        ...
        if not rs.eligible(tg_id, user["id"], real):
            return False
...
async def admin_learn_index(request: web.Request) -> web.Response:
    ...
    for sub in context["subjects"]:
        ...
        if sel_subject_id:
            ...
            # Цена урока — именно этого предмета (у ярлыка из другого предмета — своя)
            _pr = sub["reward"].get("prices") or {}
            for _sec in sub.get("sections") or []:
                for _l in _sec.get("lessons") or []:
                    _l["reward_price"], _l["reward_required"] = _pr.get(
                        _l.get("orig_id") or _l["id"], (0, 1))
...
async def admin_questions(request: web.Request) -> web.Response:
    ...
    data["open_q"] = request.query.get("q")
