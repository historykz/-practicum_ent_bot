...
from services import stop_override_service as _so
...
def _site_url_sync() -> str:
    """Публичный адрес сайта — для ссылок, которые уходят наружу."""
    url = (_setting_sync("site_public_url", "")
           or getattr(config, "SITE_URL", "") or "").strip().rstrip("/")
    return url or "https://practicumentbot-production.up.railway.app"


def _manager_link_sync(text: str) -> str:
    """Ссылка на менеджера с уже подставленным сообщением о покупке."""
    from urllib.parse import quote
    contact = _setting_sync("site_contact_username", "").lstrip("@").strip()
    if not contact:
        return ""
    return f"https://t.me/{contact}?text={quote(text)}"
...
def _lesson_premium_ignored_sync(lesson: dict) -> bool:
    """Продаётся ли этот урок отдельно от общего Премиума.

    Смотрим все предметы урока — и тот, где лежит карточка, и предмет
    оригинала: иначе Премиум открывал бы отдельно продаваемый предмет
    через копию-витрину.
    """
...
def _has_lesson_paid_access_sync(lesson: dict, tg_id: Optional[int]) -> bool:
    ...
    if user and utils.is_premium(user["id"]) \
            and not _lesson_premium_ignored_sync(lesson):
        return True
    row = db.fetchone(
    ...
    if row is not None:
        return True
    # Куплен весь раздел — открыты все его уроки
    row = db.fetchone(
        "SELECT sa.id FROM section_access sa JOIN lessons l ON l.section_id=sa.section_id "
        "WHERE l.id=? AND sa.user_tg_id=?",
        (sc.orig_lesson_id(lesson["id"]), tg_id))
    if row is not None:
        return True
...
def _lesson_subject_ids_sync(lesson: dict) -> list:
    ...
    Урок сюда приходит и сырым, и уже развёрнутым: у развёрнутой копии `id`
    подменён на оригинал, а собственный id лежит в `copy_id` — берём оба,
    иначе предмет самой копии-витрины выпадал бы из проверки.
    """
    ids = []
    # legacy_lesson_id — урок, из ярлыка которого сделан этот урок (v69): доступ
    # к предмету того урока открывал его и до перевода — открывает и после
    for lid in {lesson.get("id"), lesson.get("copy_id"), lesson.get("legacy_lesson_id"),
                sc.orig_lesson_id(lesson.get("id"))} - {None}:
        row = db.fetchone(
...
def _has_subject_access_sync(subject_id: int, tg_id: int) -> bool:
    ...
    if not row["expires_at"]:
        return True
    return utils.deadline_active(row["expires_at"])
...
def _list_subjects_public_sync(tg_id: Optional[int]) -> list:
    ...
    state = _completion_state_sync(user_id, tg_id)
    result = []
    for s in subjects:
        s = dict(s)
        ...
        if user_id and total:
            ...
            for l in lessons:
                if _lesson_completed_sync(user_id, tg_id, l, pass_pct, state):
                    done += 1
        s["lessons_total"] = total
        s["lessons_done"] = done
        s["progress_percent"] = round(done / total * 100) if total else 0
        # Прогресс и игровые элементы — часть Премиума. Тому, кому предмет не
        # открыт, показываем только витрину, без цифр «пройдено / осталось».
        try:
            from services import gamification as _g
            s["show_progress"] = bool(user_id) and _g.has_access(tg_id, user_id, s["id"])
        except Exception:
            s["show_progress"] = False
        # Сразу видно, есть ли с чего начать бесплатно — без захода внутрь
        s["free_count"] = sum(1 for l in lessons
                              if not l.get("is_paid") and l.get("status") == "open")
        s["paid_count"] = sum(1 for l in lessons
                              if l.get("is_paid") and l.get("status") == "open")
        result.append(s)
    return result
...
# Кэш структуры курса: уроки предмета меняются редко (админ правит их
# раз в день), а читаются на каждой странице каждым учеником. Держим готовый
# список 30 секунд — это снимает с базы и с процессора самую частую работу.
# Личные данные (прогресс, доступ) сюда НЕ попадают — они считаются отдельно
# на каждого человека.
_CATALOG_TTL = 30
_lessons_cache: dict = {}        # subject_id -> (expires_at, [lessons])


def invalidate_catalog_cache() -> None:
    """Вызывается после правок уроков/разделов в админке."""
...
def _flatten_subject_lessons_sync(subject_id: int) -> list:
    ...
    now = _time.time()
    hit = _lessons_cache.get(subject_id)
    if hit and hit[0] > now:
        return [dict(l) for l in hit[1]]      # копии: вызывающий код может менять поля
    rows = db.fetchall(
    ...
    lessons = [sc.resolve_lesson(r) for r in rows]
    if len(_lessons_cache) > 500:             # на всякий случай не растём бесконечно
        _lessons_cache.clear()
    _lessons_cache[subject_id] = (now + _CATALOG_TTL, lessons)
...
def _lesson_best_percent_sync(user_id: int, test_id: int):
    ...
    row = db.fetchone(
        ...
        "AND COALESCE(publication_id, 0)=0 "   # подборка из канала — не сдача урока
        "AND COALESCE(attempt_num, 1) <> 999 "  # повтор ошибок — тоже не сдача
...
def _completion_state_sync(user_id, tg_id) -> dict:
    """Всё, что нужно знать о прогрессе ученика, — тремя запросами на страницу.

    Раньше каталог для вошедшего человека спрашивал базу отдельно про КАЖДЫЙ
    урок каждого предмета (лучший результат теста, просмотрен ли, сдан ли
    зачёт) — сотни запросов на один показ. При наплыве людей именно это
    выстраивало всех в очередь. Теперь три выборки — и дальше всё в памяти.
    """
    state = {"best": {}, "viewed": set(), "zachet": set()}
    if not tg_id:
        return state
    if user_id:
        for r in db.fetchall(
                "SELECT test_id, MAX(CASE WHEN (correct_answers+wrong_answers)>0 "
                "THEN correct_answers*100.0/(correct_answers+wrong_answers) "
                "ELSE 0 END) AS pct FROM test_attempts "
                "WHERE user_id=? AND status='finished' "
                "AND COALESCE(publication_id, 0)=0 "
                # Повтор ошибок (attempt_num=999) содержит только заваленные
                # вопросы: сдать его на 100% легко, и урок закрывался бы им.
                "AND COALESCE(attempt_num, 1) <> 999 GROUP BY test_id", (user_id,)):
            state["best"][r["test_id"]] = round(r["pct"] or 0.0, 1)
    state["viewed"] = {r["lesson_id"] for r in db.fetchall(
        "SELECT lesson_id FROM lesson_progress WHERE user_tg_id=?", (tg_id,))}
    state["zachet"] = {r["lesson_id"] for r in db.fetchall(
        "SELECT DISTINCT lesson_id FROM zachet_attempts WHERE user_tg_id=? AND passed=1",
        (tg_id,))}
    return state


def _lesson_completed_sync(user_id: int, tg_id: int, lesson: dict, pass_percent: int,
    ...
    Если передано готовое состояние (_completion_state_sync), в базу не ходим.
    """
    if state is not None:
        if lesson.get("is_zachet"):
            return lesson["id"] in state["zachet"]
        if lesson.get("test_id"):
            pct = state["best"].get(lesson["test_id"])
            return pct is not None and pct >= (pass_percent or 0)
        return lesson["id"] in state["viewed"]
    if lesson.get("is_zachet"):
        row = db.fetchone(
...
def _stop_relief_sync(subject: dict, lesson_id, tg_id) -> dict:
    """Что админ разрешил этому ученику вручную (services/stop_override_service).

    off         — стоп-уроки в предмете для него выключены целиком;
    passed      — этот урок считается пройденным (входит в «открыто до …»):
                  таймер чтения на нём не нужен;
    prev_passed — предыдущий урок считается пройденным: этот открыт без сдачи.
    """
    out = {"off": False, "passed": False, "prev_passed": False, "unlock_to": 0}
    if not tg_id or not subject:
        return out
    st = _so.state(tg_id, subject["id"])
    out["off"] = st["off"]
    out["unlock_to"] = st["unlock_to"]
    if st["unlock_to"] and lesson_id:
        ids = [l["id"] for l in _flatten_subject_lessons_sync(subject["id"])]
        real = sc.orig_lesson_id(lesson_id)
        if real in ids and st["unlock_to"] in ids:
            idx, upto = ids.index(real), ids.index(st["unlock_to"])
            out["passed"] = idx <= upto
            out["prev_passed"] = idx - 1 <= upto
    return out


def _progression_active_sync(subject: dict, tg_id) -> bool:
    """Работает ли на этом человеке пошаговое прохождение.

    Порядок уроков — часть платного обучения. Пока человек не купил, он
    просто смотрит витрину: показывать ему «сначала сдайте предыдущий»
    бессмысленно и отпугивает — особенно на бесплатных уроках, которые как
    раз и должны пробовать первыми. Включаем после покупки.
    """
    ...
    if utils.is_site_admin(tg_id):
        return False
    if _so.state(tg_id, subject["id"])["off"]:
        return False        # админ снял стоп-уроки с этого ученика
    return bool(_has_subject_access_sync(subject["id"], tg_id)
                or _is_premium_tg_sync(tg_id))
...
def _prev_lesson_gate_sync(subject: dict, lesson_id: int, user_id: int, tg_id: int) -> Optional[dict]:
    ...
    if not _progression_active_sync(subject, tg_id):
        return None
    ...
    if _stop_relief_sync(subject, lesson_id, tg_id)["prev_passed"]:
        return None  # админ открыл ученику уроки до этого — сдавать предыдущий не нужно
    prev = ordered[idx - 1]
...
def _subject_detail_sync(subject_id: int, tg_id: Optional[int]) -> Optional[dict]:
    ...
    # У предмета, который продаётся отдельно, Премиум покупкой не считается.
    premium_counts = _is_premium_tg_sync(tg_id) and not sc.premium_ignored(subject)
    paid_ok = is_adm or has_access or premium_counts
    relief = _stop_relief_sync(subject, None, tg_id) if tg_id else {"off": False, "unlock_to": 0}
    ...
    if sequential and user_id:
        ...
        prev_done = True
        state = _completion_state_sync(user_id, tg_id)
        # «Открыто до …» от админа: эти уроки считаем пройденными для цепочки
        unlock_idx = next((i for i, l in enumerate(ordered)
                           if l["id"] == relief.get("unlock_to")), -1)
        for i, l in enumerate(ordered):
            if not prev_done:
                locked_ids.add(l["id"])
            prev_done = (i <= unlock_idx
                         or _lesson_completed_sync(user_id, tg_id, l, pass_pct, state))
    ...
    for sec in sections:
        ...
        sec_out = dict(sec)
        # Счётчики раздела: сколько уроков, сколько платных и бесплатных,
        # и сколько уже пройдено. Их видно на карточке раздела — чтобы было
        # понятно, куда заходить и сколько осталось, ещё до открытия.
        sec_out["lessons_total"] = len(lessons_out)
        sec_out["free_count"] = sum(1 for l in lessons_out if not l.get("is_paid"))
        sec_out["paid_count"] = sum(1 for l in lessons_out if l.get("is_paid"))
        done = 0
        if user_id:
            pass_pct = subject.get("pass_percent") or 0
        ...
        sec_out["done_count"] = done
        sec_out["percent"] = round(done * 100 / len(lessons_out)) if lessons_out else 0
        sec_out["locked_count"] = sum(1 for l in lessons_out if l.get("needs_purchase"))
        result_sections.append({"section": sec_out, "lessons": lessons_out})

    # Итог по предмету — для полосы прогресса и «уровня» ученика
    all_lessons = [l for it in result_sections for l in it["lessons"]]
    total = len(all_lessons)
    done_all = sum(it["section"]["done_count"] for it in result_sections)
    return {
        "subject": subject,
        "sections": result_sections,
        "has_access": has_access,
        "subject_total": total,
        "subject_done": done_all,
        "subject_percent": round(done_all * 100 / total) if total else 0,
        "subject_free": sum(1 for l in all_lessons if not l.get("is_paid")),
        "subject_paid": sum(1 for l in all_lessons if l.get("is_paid")),
        "rank": _rank_for(done_all, total),
        # Игровая часть — только тем, кому предмет реально открыт. Остальные
        # не видят ни баллов, ни прогресса: пустых карточек тоже не показываем.
        "game": _game_card_sync(tg_id, user_id, subject_id, done_all, total),
    }


def _game_card_sync(tg_id, user_id, subject_id: int, done: int, total: int):
    """Баллы, место, серия и мотивация по предмету. None — если не Премиум."""
    if not tg_id or not user_id:
        return None
    try:
        from services import gamification as g
        return g.subject_card(tg_id, user_id, subject_id,
                              lessons_done=done, lessons_total=total)
    except Exception as e:
        logger.warning("геймификация предмета %s: %s", subject_id, e)
        return None


# Лёгкая геймификация: ранг зависит от доли пройденного. Не игра ради игры —
# ученику нужен ориентир «сколько я уже сделал» крупнее, чем проценты.
RANKS = [
    (0,   "🌱", "Новичок"),
    (10,  "📗", "Начал путь"),
    (30,  "📘", "Втянулся"),
    (50,  "🔥", "Половина позади"),
    (70,  "⚡️", "На финишной прямой"),
    (90,  "🏆", "Почти мастер"),
    (100, "👑", "Предмет закрыт"),
]


def _rank_for(done: int, total: int) -> dict:
    ...
    need = 0
    if nxt and total:
        need = max(0, -(-nxt[0] * total // 100) - done)   # сколько уроков до ранга
    return {"icon": cur[1], "title": cur[2], "percent": pct,
            "next_title": nxt[2] if nxt else "", "next_icon": nxt[1] if nxt else "",
            "need": need}
...
_sub_cache: dict = {}
# 20 секунд: отписавшийся теряет доступ почти сразу. Мгновенно его закрывает
# событие chat_member (handlers/subscription_watch.py) — этот кэш лишь подстраховка
# на случай, если бот не админ канала и события не приходят.
SUB_CACHE_TTL = 20
...
async def _is_subscribed(tg_id: int, channel_username: str, force: bool = False) -> bool:
    ...
    try:
        ...
        ok = True
    # Кэш не должен расти бесконечно вместе с числом учеников: при 100 000
    # пользователей это утечка памяти. Переполнился — выбрасываем устаревшее.
    if len(_sub_cache) > 5000:
        for k in [k for k, v in _sub_cache.items() if now - v[1] >= SUB_CACHE_TTL]:
            _sub_cache.pop(k, None)
        if len(_sub_cache) > 5000:
            _sub_cache.clear()
    # Кэш не должен расти бесконечно: при 100 000 учеников он съел бы память.
    # Когда записей много — выбрасываем протухшие.
    if len(_sub_cache) > 5000:
        for k in [k for k, v in _sub_cache.items() if now - v[1] >= SUB_CACHE_TTL]:
            _sub_cache.pop(k, None)
...
async def learn_subject(request: web.Request) -> web.Response:
    """Экран предмета: список РАЗДЕЛОВ. Уроки — на следующем экране.

    Раньше все уроки валились одним полотном: у предмета на 50 уроков
    страница превращалась в бесконечную ленту, где непонятно, куда нажимать.
    """
    tg_id = await auth.get_logged_in_tg_id(request)
    ...
    if sub_page is not None:
        return sub_page
    # Вознаграждения, рейтинг и кнопка «✨ НАЧАТЬ ОБУЧЕНИЕ ✨» (None — нет Премиума)
    from services import reward_service as _rs
    data["reward"] = await asyncio.to_thread(_rs.subject_view, tg_id, subject_id)
    data["start_flag"] = request.query.get("start")     # required — пришёл с урока без старта
    data.update(await auth.nav_context(request))
    return aiohttp_jinja2.render_template("learn_sections.html", request, data)


async def learn_section(request: web.Request) -> web.Response:
    """Экран раздела: его уроки и кнопка назад к разделам предмета."""
    tg_id = await auth.get_logged_in_tg_id(request)
    subject_id = int(request.match_info["subject_id"])
    ...
    if sub_page is not None:
        return sub_page
    item = next((it for it in data["sections"]
                 if it["section"]["id"] == section_id), None)
    if item is None:
        raise web.HTTPFound(f"/learn/{subject_id}")
    # Соседние разделы — чтобы после последнего урока идти дальше, а не назад
    ids = [it["section"]["id"] for it in data["sections"]]
    pos = ids.index(section_id)
    data["section"] = item["section"]
    data["lessons"] = item["lessons"]
    data["section_pos"] = pos + 1
    data["section_count"] = len(ids)
    data["prev_section"] = data["sections"][pos - 1]["section"] if pos > 0 else None
    data["next_section"] = (data["sections"][pos + 1]["section"]
                            if pos + 1 < len(ids) else None)

    # Покупка раздела: показываем, только если админ включил продажу,
    # раздел ещё не куплен и в нём есть платные уроки без доступа.
    def _section_offer():
        from services import unit_sales as us
        if not tg_id:
            has = False
        else:
            has = us.has_section_access(section_id, tg_id)
        paid_left = any(l.get("needs_purchase") for l in data["lessons"])
        if has or not paid_left or not us.sale_enabled_for_section(section_id):
            return {}
        return {"section_sale": True,
                "section_price": us.section_price(section_id),
                "section_buy_link":
                    f"https://t.me/{config.WEB_BOT_USERNAME}?start=buysec_{section_id}",
                "manager_link": _manager_link_sync(
                    us.manager_text("section", section_id))}
    data.update(await asyncio.to_thread(_section_offer))
    data.update(await auth.nav_context(request))
    return aiohttp_jinja2.render_template("learn_section.html", request, data)


# === Студент: урок ===

def _needs_start_sync(subject_id: int, tg_id) -> bool:
    """«✨ Начать обучение» — обязательная точка входа: без неё ученика нет в
    общем рейтинге предмета, поэтому уроки открываются только после неё.
    Касается тех, кому предмет открыт (Премиум или выданный доступ): гостю
    на бесплатном уроке кнопка недоступна, его не трогаем. Копии предмета —
    один рейтинг: нажал в одной из них — открыто везде."""
    if not tg_id or utils.is_site_admin(tg_id):
        return False
    try:
        from services import reward_service as rs, gamification as gm
        user = utils.get_user_by_tg(tg_id)
        if not user:
            return False
        real = rs.real_subject_id(subject_id)
        if not rs.eligible(tg_id, user["id"], real, here=True):
            return False
        group = gm.rating_group(real)
        row = db.fetchone(
            f"SELECT 1 FROM subject_participants WHERE user_id=? AND is_active=1 "
            f"AND subject_id IN ({','.join('?' * len(group))}) LIMIT 1", (user["id"], *group))
        return row is None
    except Exception as e:
        logger.warning("проверка «Начать обучение» tg=%s предмет=%s: %s", tg_id, subject_id, e)
        return False
...
def _lesson_detail_sync(lesson_id: int, tg_id: Optional[int]) -> Optional[dict]:
    ...
    if tg_id is None:
        # Гостю на сайте вход не нужен: конспект он открывает в Telegram,
        # а бот уже сам решает по его Telegram ID — выдать материал или
        # предложить Премиум. Никаких форм входа и проверок сессии здесь,
        # именно они раньше и роняли страницу в перезагрузки.
        base["access_state"] = "guest"
        base["paid_questions_count"] = (db.fetchone(
            "SELECT COUNT(*) AS c FROM questions WHERE test_id=?",
            (lesson.get("test_id"),)) or {"c": 0})["c"] if lesson.get("test_id") else 0
        return base
    ...
    if not is_adm and not _has_lesson_paid_access_sync(lesson, tg_id):
        qcount = 0
        if lesson["test_id"]:
            qcount = db.fetchone(
        ...
        from services import unit_sales as us
        real_l = sc.orig_lesson_id(lesson["id"])
        base["unit_sale"] = us.sale_enabled_for_lesson(real_l)
        base["unit_price"] = us.lesson_price(real_l) if base["unit_sale"] else 0
        base["unit_buy_link"] = (
            f"https://t.me/{config.WEB_BOT_USERNAME}?start=buyles_{real_l}")
        base["paid_questions_count"] = qcount
        return base

    if not is_adm and _needs_start_sync(section["subject_id"], tg_id):
        base["access_state"] = "need_start"
        return base

    base["access_state"] = "full"

    # Просмотр отмечаем на ОРИГИНАЛЕ урока: прогресс, доступ и статистика
    # у копии-витрины общие с ним. Раньше здесь стоял id копии, и урок без
    # теста, прочитанный через витрину, навсегда оставался «не пройденным».
    # Первое открытие — новое учебное действие; повторные только считаются
    # (open_count, last_opened_at): не приносят денег и не сбрасывают пропуски
    from services import study_tracker as _stt_open
    _first_view = _stt_open.mark_lesson_opened(tg_id, lesson["id"])
    # Урок без теста и без зачёта считается пройденным, когда прочитан:
    # начисляем вознаграждение за него при первом открытии.
    if _first_view and not lesson.get("test_id") and not lesson.get("is_zachet"):
        from services import reward_service as _rs
        _rs.on_learning_event(tg_id, None, lesson_id=lesson["id"])
    # Открытие темы — учебное действие. Раньше в приложении оно нигде не
    # отмечалось: lesson_progress пишется один раз за всю жизнь, поэтому
    # «сколько раз открывал тему» посчитать было нечем, а ученик, который
    # читает конспекты в приложении и не сдаёт ДЗ, выглядел бездельником.
    if not is_adm:
        _track_lesson_open_sync(tg_id, lesson["id"])
    ...
    if not is_adm:
        db.execute(
            ...
            (datetime.utcnow().isoformat(timespec="seconds"), tg_id, lesson["id"]),
        )
    ...
    if min_read:
        _rel = _stop_relief_sync(subject, lesson_id, tg_id)
        if _rel["off"] or _rel["passed"]:
            min_read = 0            # админ снял ограничение с этого ученика
...
async def learn_lesson(request: web.Request) -> web.Response:
    ...
    if data["access_state"] == "need_start":
        # Сначала «Начать обучение» на странице предмета — там же и подсказка
        raise web.HTTPFound(f"/learn/{data['subject']['id']}?start=required#start")
    ...
    if data["access_state"] == "full":
        ...
        # Рабочая тетрадь показывается только тем, у кого открыт урок
        from services import workbook_service as _wb

        def _wb_info():
            info = _wb.get(lesson_id)
            if info:
                info["size_label"] = _wb.size_label(info.get("size") or 0)
            return info

        data["workbook"] = await asyncio.to_thread(_wb_info)
        if data["workbook"]:
            # Ссылка с подписью: Telegram скачивает файл сам, без cookies сайта
            from services import workbook_mark_service as _wm
            data["workbook"]["download_url"] = (
                data["workbook"]["url"] + "?t=" + _wm.make_token(lesson_id, tg_id))
            data["workbook"]["personal_name"] = _wm.personal_name(
                data["workbook"]["name"], tg_id)
...
def _start_attempt_sync(lesson_id: int, tg_id: int, restart: bool = False) -> Optional[dict]:
    ...
    if raw["is_paid"] and not utils.is_site_admin(tg_id) \
        ...
        return None
    if _needs_start_sync(section["subject_id"], tg_id):
        return {"need_start": True, "subject_id": section["subject_id"]}
    lesson_id = lesson["id"]
    ...
    if subject and not is_adm:
        ...
        # b) минимальное время чтения — тоже часть платного обучения
        min_read = subject["min_read_min"] or 0
        if min_read:
            _rel = _stop_relief_sync(dict(subject), lesson_id, tg_id)
            if _rel["off"] or _rel["passed"]:
                min_read = 0        # послабление от админа
        if min_read and (_has_subject_access_sync(subject["id"], tg_id)
                         or _is_premium_tg_sync(tg_id)):
            row = db.fetchone(
                "SELECT read_started_at FROM lesson_progress WHERE user_tg_id=? AND lesson_id=?",
                (tg_id, lesson_id))
    ...
    # status='idle' — уборка брошенных попыток (services/stats_service.py)
    # пометила её «примолкшей» из-за долгой тишины, чтобы не портить живую
    # статистику. Это НЕ то же самое, что явный отказ от теста (status
    # 'aborted' — только когда человек сам нажал «Начать заново»): ученик мог
    # просто закрыть приложение и вернуться на следующий день, и его прогресс
    # должен вернуться как ни в чём не бывало.
    existing = db.fetchone(
        "SELECT * FROM test_attempts WHERE user_id=? AND test_id=? "
        "AND status IN ('in_progress', 'idle') "
        "AND COALESCE(publication_id, 0)=0 "      # подборки из канала — отдельно
        "ORDER BY id DESC LIMIT 1",
        (user_id, test_id),
    )
    if existing and existing["status"] == "idle" and not restart:
    ...
    if existing:
        ...
        answered_rows = db.fetchall(
            "SELECT question_id, selected_option_id, selected_option_ids, "
            "is_correct, skipped FROM attempt_answers WHERE attempt_id=?",
            (existing["id"],),
        )
        show_correct = bool(test["show_correct"])
        answered = {}
        for r in answered_rows:
            # У старых ответов набора нет — берём одиночный вариант
            picked = _ac.load(r["selected_option_ids"]) or (
                [r["selected_option_id"]] if r["selected_option_id"] else [])
            entry = {
            ...
            if show_correct:
                right = _ac.correct_option_ids(r["question_id"])
                entry["correct_option_ids"] = right
                entry["correct_option_id"] = right[0] if right else None
    ...
    _track_study_sync(tg_id, "hw_start", test_id=test_id, lesson_id=lesson_id)
...
async def learn_test_start(request: web.Request) -> web.Response:
    ...
    if data.get("need_start"):
        raise web.HTTPFound(f"/learn/{data['subject_id']}?start=required#start")
    if data.get("blocked"):
    ...
    def _note_present():
        ...
        real = sc.orig_lesson_id(les.get("id") or lesson_id)
        if (db.fetchone("SELECT COUNT(*) AS c FROM lesson_images WHERE lesson_id=?",
                        (real,)) or {"c": 0})["c"]:
            return True
        row = db.fetchone("SELECT content_html FROM lessons WHERE id=?", (real,))
        return bool(row and (row["content_html"] or "").strip())

    data["has_note"] = await asyncio.to_thread(_note_present)
    if isinstance(data.get("lesson"), dict):
        data["lesson"].setdefault("url_id", lesson_id)
    return aiohttp_jinja2.render_template("learn_test.html", request, data)


def _answer_sync(attempt_id: int, tg_id: int, question_id: int,
                 option_id: Optional[int], option_ids=None) -> dict:
    """Ответ на вопрос. Ученик мог отметить один вариант или несколько —
    для системы это одно и то же, она сравнивает набор целиком."""
    from services import answer_check as _ac
    ...
    # Ответы принимаем только у незавершённой попытки: иначе после подсчёта
    # результата можно было дослать ответы и поднять себе процент.
    if attempt["status"] not in ("in_progress", "idle"):
        return {"error": "attempt_finished"}
    ...
    def _correct_opt_id():
        ids = _ac.correct_option_ids(question_id)
        return ids[0] if ids else None

    def _correct_opt_ids():
        return _ac.correct_option_ids(question_id)
    ...
    if existing:
        return {
            ...
            "correct_option_ids": _correct_opt_ids() if show_correct else None,
            "skipped": bool(existing["skipped"]),
            "duplicate": True,
        }

    # Что именно отметил ученик. Чужие варианты отсекаем: список приходит
    # от клиента, и подставленный id из другого вопроса не должен пройти.
    chosen = _ac.belongs_to_question(question_id, option_ids
                                     if option_ids is not None else option_id)

    if not chosen:
        # Вышло время (таймер) или ничего не отмечено — засчитываем как пропуск
        try:
            db.execute(
    ...
    # Засчитываем, только если набор совпал полностью: все правильные и ни
    # одного лишнего. Часть правильных ответов — это ошибка.
...
async def learn_test_answer(request: web.Request) -> web.Response:
    ...
    # Новый фронт присылает набор отмеченных вариантов; старый — один вариант.
    # Понимаем оба, чтобы вкладка, открытая до обновления, продолжала работать.
    option_ids = body.get("option_ids")
    result = await asyncio.to_thread(_answer_sync, attempt_id, tg_id, question_id,
                                     option_id, option_ids)
    return web.json_response(result)


# Через сколько минут повторное открытие той же темы считается новым заходом.
# Обновление страницы и переходы назад-вперёд новых событий не создают.
LESSON_OPEN_GAP_MINUTES = 10


def _track_lesson_open_sync(tg_id: int, lesson_id: int) -> None:
    """Отметить открытие темы. Это НЕ прохождение урока — отдельная метрика."""
    if not tg_id or not lesson_id:
        return
    try:
        from services import study_tracker as _stt
        recent = db.fetchone(
            "SELECT id FROM study_events WHERE tg_id=? AND lesson_id=? AND event=? "
            "AND datetime(created_at) > datetime('now', ?) LIMIT 1",
            (tg_id, lesson_id, _stt.NOTE_OPEN, f"-{LESSON_OPEN_GAP_MINUTES} minutes"))
        if recent:
            return
        _stt.track(tg_id, _stt.NOTE_OPEN, lesson_id=lesson_id)
    except Exception:
        pass


def _track_study_sync(tg_id: int, event: str, test_id=None, lesson_id=None) -> None:
    """Отметить учебное действие. Урок вычисляем по тесту, если он не передан."""
    try:
        from services import study_tracker as _stt
        if lesson_id is None and test_id:
            row = db.fetchone("SELECT id FROM lessons WHERE test_id=? LIMIT 1",
                              (test_id,))
            lesson_id = row["id"] if row else None
        _stt.track(tg_id, event, lesson_id=lesson_id, test_id=test_id)
    except Exception:
        pass
...
def _finish_attempt_sync(attempt_id: int, tg_id: int) -> dict:
    ...
    # Брошенную попытку (её отменили кнопкой «начать заново») не оживляем —
    # иначе её результат можно было бы «обналичить» вторым запросом.
    if attempt["status"] in ("in_progress", "idle"):
        db.execute(
        ...
        # Домашнее задание сдано — тема закрыта, строгие напоминания снимаются
        _track_study_sync(tg_id, "hw_done", test_id=attempt["test_id"])
        # Баллы уже записаны выше в score. Достижения — по факту действий.
        try:
            from services import gamification as _g
            _g.invalidate()
            _g.award(tg_id, user["id"])
        except Exception as e:
            logger.warning("достижения после ДЗ tg_id=%s: %s", tg_id, e)
        # Вознаграждение за урок и место в рейтинге предмета — сразу после сдачи
        from services import reward_service as _rs
        _rs.on_learning_event(tg_id, user["id"], test_id=attempt["test_id"])
    if not show_results:
...
def _attempt_result_sync(attempt_id: int, tg_id: int) -> Optional[dict]:
    ...
    if lesson:
        ...
        if subject:
            ...
            _rel = _stop_relief_sync(dict(subject), lesson["id"], tg_id)
            if _rel["off"] or _rel["passed"]:
                passed_gate = True  # админ открыл дорогу дальше вручную
            if passed_gate:
                ordered = _flatten_subject_lessons_sync(subject["id"])
    ...
    # Работа над ошибками: сколько вопросов ученик завалил в этой попытке.
    # Кнопка появляется, только если админ включил режим у теста.
    wrong_ids = [r["question_id"] for r in db.fetchall(
        "SELECT question_id FROM attempt_answers WHERE attempt_id=? "
        "AND COALESCE(is_correct,0)=0 AND COALESCE(skipped,0)=0", (attempt_id,))]
    allow_retry = bool(test and (test["allow_retry_wrong"] if "allow_retry_wrong"
                                 in test.keys() else 0))
...
def _retry_wrong_data_sync(attempt_id: int, tg_id: int):
    """Вопросы этой попытки, где ученик ошибся — для работы над ошибками."""
    attempt = db.fetchone("SELECT * FROM test_attempts WHERE id=?", (attempt_id,))
    user = utils.get_user_by_tg(tg_id)
    if not attempt or not user or attempt["user_id"] != user["id"]:
        return None
    test = db.fetchone("SELECT * FROM tests WHERE id=?", (attempt["test_id"],))
    if not test or not (test["allow_retry_wrong"] if "allow_retry_wrong"
                        in test.keys() else 0):
        return None    # админ не включал этот режим
    rows = db.fetchall(
        "SELECT aa.question_id AS qid, q.text AS qtext, q.explanation, "
        "q.web_image_path, q.photo_file_id "
        "FROM attempt_answers aa JOIN questions q ON q.id = aa.question_id "
        "WHERE aa.attempt_id=? AND COALESCE(aa.is_correct,0)=0 "
        "AND COALESCE(aa.skipped,0)=0 ORDER BY aa.id", (attempt_id,))
    items = []
    for r in rows:
        opts = [dict(o) for o in db.fetchall(
            "SELECT id, text, is_correct FROM question_options WHERE question_id=? "
            "ORDER BY order_num, id", (r["qid"],))]
        if not opts:
            continue
        random.shuffle(opts)
        ...
        items.append({
            ...
            "image": f"/uploads/questions/{os.path.basename(r['web_image_path'])}"
                     if r["web_image_path"] else "",
            "options": [{"text": o["text"], "correct": bool(o["is_correct"])} for o in opts],
            "correct_text": correct["text"],
        })
    lesson = db.fetchone("SELECT * FROM lessons WHERE test_id=?", (attempt["test_id"],))
    return {"items": items, "test": dict(test),
            "lesson": dict(lesson) if lesson else None,
            "attempt_id": attempt_id}


async def learn_retry_wrong(request: web.Request) -> web.Response:
    """Прогнать заново только заваленные вопросы. Результат никуда не пишется —
    это тренировка, а не новая попытка: иначе она съедала бы лимит попыток."""
    tg_id = await _require_login(request)
    attempt_id = int(request.match_info["attempt_id"])
    data = await asyncio.to_thread(_retry_wrong_data_sync, attempt_id, tg_id)
    if data is None:
        raise web.HTTPFound(f"/learn/test/{attempt_id}/result")
    data.update(await auth.nav_context(request))
    return aiohttp_jinja2.render_template("learn_retry_wrong.html", request, data)
...
async def lesson_workbook(request: web.Request) -> web.Response:
    """Отдать рабочую тетрадь урока — персональную копию с меткой ученика.

    Доступ такой же, как к самому уроку: платный урок без оплаты тетрадь
    не отдаёт. Вход — по cookie сайта или по подписанному токену ?t=… (так
    файл скачивает сам клиент Telegram из мини-приложения).
    """
    lesson_id = int(request.match_info["lesson_id"])
    from services import workbook_service as wb
    from services import workbook_mark_service as wm

    tg_id = wm.parse_token(lesson_id, request.query.get("t") or "")
    if tg_id is None:
        tg_id = await _require_login(request)
    ...
    def _check():
        ...
        if raw["is_paid"] and not utils.is_site_admin(tg_id) \
                and not _has_lesson_paid_access_sync(lesson, tg_id):
            return None, None
        return wb.get(lesson_id), lesson

    info, lesson = await asyncio.to_thread(_check)
    if not info:
        raise web.HTTPNotFound(text="Рабочая тетрадь недоступна")

    path = wb.upload_dir() / info["file"]
    if not path.exists():
        raise web.HTTPNotFound(text="Файл не найден")

    def _personal():
        user = utils.get_user_by_tg(tg_id) or {}
        return wm.personalize(path.read_bytes(), info["name"], tg_id, user.get("username"))

    body, fname = await asyncio.to_thread(_personal)
    from urllib.parse import quote as _q
    return web.Response(
        body=body,
        headers={
            "Content-Type": wb.mime_for(fname),
            # filename* — чтобы русское имя файла не превращалось в кракозябры
...
def _admin_overview_sync(sel_subject_id=None, sel_section_id=None) -> list:
    ...
    if sel_subject_id:
        ...
        subjects = db.fetchall("SELECT * FROM subjects "
                               "ORDER BY COALESCE(is_pinned,0) DESC, sort_order, id")
    from services import copy_compare as _cmp
    result = []
    for s in subjects:
        s = dict(s)
        s["sections"] = []
        s["copy_info"] = _cmp.info(s)       # «Копия предмета · создано на основе…»
        ...
        for sec in secs:
            ...
            if sec.get("copied_from_section_id"):
                src_sec = db.fetchone(
                    "SELECT s.title, sub.title AS subject_title FROM sections s "
                    "LEFT JOIN subjects sub ON sub.id=s.subject_id WHERE s.id=?",
                    (sec["copied_from_section_id"],))
                sec["copy_src"] = dict(src_sec) if src_sec else {"title": None}
            ...
            if sel_section_id and int(sel_section_id) == sec["id"]:
                ...
                for l in db.fetchall(
                    ...
                    l["copy_info"] = _cmp.lesson_info(l)
                    lessons.append(l)
                sec["lessons"] = lessons
            else:
                sec["lessons"] = []
        ...
        if not s["is_open"]:
            ...
            s["access_list"] = []
            s["pending_list"] = []
        # Послабления стоп-уроков для отдельных учеников + список уроков по порядку
        try:
            s["stop_overrides"] = _so.list_for_subject(s["id"])
        except Exception:
            s["stop_overrides"] = []
        s["lessons_flat"] = [{"id": l["id"], "title": l.get("title") or ""}
                             for l in _flatten_subject_lessons_sync(s["id"])]
        result.append(s)
    return result
...
async def admin_learn_index(request: web.Request) -> web.Response:
    ...
    context["site_url"] = _site_url_sync()
    _short = (getattr(config, "WEB_APP_SHORT_NAME", "") or "").strip()
    context["miniapp_base"] = (
        f"https://t.me/{config.WEB_BOT_USERNAME}"
        + (f"/{_short}" if _short else ""))
    if not sel_subject_id:
        from services import reward_service as _rsv
        context["reward_promo"] = await asyncio.to_thread(_rsv.promo_overview)
    for sub in context["subjects"]:
        sub["mode"] = sc.subject_mode(sub)
        if sel_subject_id:
            from webapp import rewards_web as _rw
            sub["reward"] = await asyncio.to_thread(_rw.admin_summary, sub["id"])
            # Цена урока — программы этого предмета (копии одного урока — одна цена)
            for _sec in sub.get("sections") or []:
                for _l in _sec.get("lessons") or []:
                    _l["reward_price"], _l["reward_required"] = await asyncio.to_thread(
                        _rw.rs.lesson_setting, _l["id"])
...
def _back(data, message: str = "", fallback: str = "/admin/learn"):
    ...
    invalidate_catalog_cache()     # админ что-то поменял — ученики увидят сразу
    from urllib.parse import quote as _q
    nxt = ""
    try:
...
def _free_category_name(title: str, own_id=None) -> str:
    """Имя раздела бота уникально (UNIQUE в test_categories). Копия с тем же
    названием, что у оригинала, получает раздел с номером: «История (2)» —
    иначе сохранение такого предмета падало бы."""
    name, n = title, 1
    while True:
        row = db.fetchone("SELECT id FROM test_categories WHERE name=? COLLATE NOCASE", (name,))
        if not row or row["id"] == own_id:
            return name
        n += 1
        name = f"{title} ({n})"
...
def _ensure_bot_category_sync(subject_id: int, title: str, created_by: int) -> int:
    ...
    if row and row["bot_category_id"]:
        db.execute("UPDATE test_categories SET name=? WHERE id=?",
                   (_free_category_name(title, row["bot_category_id"]), row["bot_category_id"]))
        return row["bot_category_id"]
    # Одноимённый раздел бота берём, только если он ничей: иначе копия с тем же
    # названием прицепилась бы к разделу оригинала и переименовывала его
    exist = db.fetchone(
        "SELECT id FROM test_categories WHERE name=? COLLATE NOCASE AND id NOT IN "
        "(SELECT bot_category_id FROM subjects WHERE bot_category_id IS NOT NULL AND id<>?)",
        (title, subject_id))
    if exist:
        cat_id = exist["id"]
    else:
        db.execute(
            ...
            (_free_category_name(title), created_by or None))
...
async def admin_create_subject(request: web.Request) -> web.Response:
    ...
    if title:
        await asyncio.to_thread(
            ...
            "premium_ignored, own_price, unit_sale_enabled, unit_price_stars) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (title, description, is_open, is_private, mode, tg_id, min_read, seq, pass_pct, live,
             _clean_url(data.get("quizlet_url")),
             1 if data.get("premium_ignored") == "on" else 0,
             (data.get("own_price") or "").strip()[:120],
             1 if data.get("unit_sale_enabled") == "on" else 0,
             int(data.get("unit_price_stars") or 0)
             if str(data.get("unit_price_stars") or "").strip().isdigit() else 0),
        )
...
async def admin_edit_subject(request: web.Request) -> web.Response:
    ...
    if title:
        await asyncio.to_thread(
            ...
            "quizlet_url=?, premium_ignored=?, own_price=?, "
            "unit_sale_enabled=?, unit_price_stars=?, separate_rating=? WHERE id=?",
            (title, description, is_open, is_private, mode, min_read, seq, pass_pct, live,
             ...
             int(data.get("unit_price_stars") or 0)
             if str(data.get("unit_price_stars") or "").strip().isdigit() else 0,
             1 if data.get("separate_rating") == "on" else 0,    # копия: свой рейтинг
             subject_id),
        )
        # Рейтинг общий на копии предмета: галочка меняет состав всех досок группы
        from services import gamification as _gm
        _gm.invalidate()
...
def _move_row_sync(table: str, row_id: int, direction: str,
                   scope_col: str = "", scope_val=None) -> None:
    """Сдвинуть запись на шаг вверх или вниз внутри её списка.

    Меняем sort_order местами с соседом. Стрелки надёжнее drag&drop:
    на телефоне перетаскивание почти не работает.
    """
    where = f"{scope_col}=?" if scope_col else "1=1"
    args = (scope_val,) if scope_col else ()
    rows = db.fetchall(
        f"SELECT id, COALESCE(sort_order, 0) AS so FROM {table} "
        f"WHERE {where} ORDER BY sort_order, id", args)
    ...
    for pos, rid in enumerate(ids):
        db.execute(f"UPDATE {table} SET sort_order=? WHERE id=?", (pos, rid))


async def admin_move_subject(request: web.Request) -> web.Response:
    await _require_admin(request)
    subject_id = int(request.match_info["subject_id"])
    data = await request.post()
    direction = "up" if (data.get("dir") or "up") == "up" else "down"
    await asyncio.to_thread(_move_row_sync, "subjects", subject_id, direction)
    raise _back(data, "Порядок предметов изменён")


async def admin_move_section(request: web.Request) -> web.Response:
    await _require_admin(request)
    section_id = int(request.match_info["section_id"])
    data = await request.post()
    direction = "up" if (data.get("dir") or "up") == "up" else "down"
    ...
    def _run():
        row = db.fetchone("SELECT subject_id FROM sections WHERE id=?", (section_id,))
        if row:
            _move_row_sync("sections", section_id, direction,
                           "subject_id", row["subject_id"])
    await asyncio.to_thread(_run)
    raise _back(data, "Порядок разделов изменён")


async def admin_move_lesson(request: web.Request) -> web.Response:
    await _require_admin(request)
    lesson_id = int(request.match_info["lesson_id"])
    data = await request.post()
    direction = "up" if (data.get("dir") or "up") == "up" else "down"
    ...
    def _run():
        row = db.fetchone("SELECT section_id FROM lessons WHERE id=?", (lesson_id,))
        if row:
            _move_row_sync("lessons", lesson_id, direction,
                           "section_id", row["section_id"])
    await asyncio.to_thread(_run)
    raise _back(data, "Порядок уроков изменён")


async def admin_pin_subject(request: web.Request) -> web.Response:
    """Закрепить предмет — он встаёт первым в каталоге у всех учеников."""
    await _require_admin(request)
    subject_id = int(request.match_info["subject_id"])
    data = await request.post()

    def _run():
        cur = db.fetchone("SELECT COALESCE(is_pinned,0) AS p FROM subjects WHERE id=?",
                          (subject_id,))
        new = 0 if (cur and cur["p"]) else 1
        if new:
            # Закреплённый всегда один: иначе «первое место» теряет смысл
            db.execute("UPDATE subjects SET is_pinned=0")
        db.execute("UPDATE subjects SET is_pinned=? WHERE id=?", (new, subject_id))
    await asyncio.to_thread(_run)
    raise _back(data, "Закрепление обновлено")
...
async def admin_edit_section(request: web.Request) -> web.Response:
    ...
    data = await request.post()
    title = (data.get("title") or "").strip()
    raw_price = str(data.get("price_stars") or "").strip()
    price = int(raw_price) if raw_price.isdigit() else None
    sale_on = 1 if data.get("sale_enabled") == "on" else 0

    def _save():
        if title:
            db.execute("UPDATE sections SET title=? WHERE id=?", (title, section_id))
        # Цена раздела: пусто = считать от цены предмета; число = своя цена
        db.execute("UPDATE sections SET price_stars=?, sale_enabled=? WHERE id=?",
                   (price, sale_on, section_id))
    await asyncio.to_thread(_save)
...
async def admin_create_lesson(request: web.Request) -> web.Response:
    ...
    _raw_price = str(data.get("price_stars") or "").strip()
    _price_stars = int(_raw_price) if _raw_price.isdigit() else None
    ...
    if test_mode == "none" or (test_mode == "text" and not test_text.strip()) or \
        ...
        def _insert_and_images():
            db.execute(
                ...
                "youtube_id, is_paid, price_stars) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def _lesson_edit_data_sync(lesson_id: int) -> Optional[dict]:
    ...
    # Старый ярлык (до v69): контент пока общий с оригиналом — одна кнопка
    # делает его независимым. Новые копии независимы сразу.
    out["is_copy"] = bool(lesson["original_id"])
    from services import copy_compare as _cmp
    out["copy_info"] = _cmp.lesson_info(dict(lesson))
    out["copy_diff"] = None
    if out["copy_info"]:
        try:
            out["copy_diff"] = _cmp.lesson_diff(lesson_id)
        except Exception as e:
            logger.warning("сравнение урока-копии %s: %s", lesson_id, e)
    # Что за файл рабочей тетради сейчас прикреплён (для карточки в админке)
    from services import workbook_service as _wb
    _info = _wb.get(lesson_id)
    if _info:
        _info["size_label"] = _wb.size_label(_info.get("size") or 0)
    out["workbook"] = _info
    if out["is_copy"]:
...
async def admin_edit_lesson(request: web.Request) -> web.Response:
    ...
    # Своя цена урока в Stars (пусто — общая цена предмета). Раньше это поле
    # не читалось, а имя было не определено — сохранение урока падало.
    _raw_price = str(data.get("price_stars") or "").strip()
    _price_stars = int(_raw_price) if _raw_price.isdigit() else None
    ...
    # Рабочая тетрадь: прикрепить новый файл или убрать прежний
    from services import workbook_service as _wb
    wb_msg = ""
    if (data.get("workbook_remove") or "") == "on":
        await asyncio.to_thread(_wb.delete, lesson_id)
        wb_msg = " Рабочая тетрадь удалена."
    else:
        field = data.get("workbook_file")
        if field is not None and hasattr(field, "file"):
            raw = field.file.read()
            if raw:
                ok_wb, msg_wb = await asyncio.to_thread(
                    _wb.save, lesson_id, raw, field.filename or "workbook.pdf")
                wb_msg = " " + msg_wb
    ...
    if title:
        if update_video:
            await asyncio.to_thread(
                ...
                "video_url=?, youtube_id=?, free_override=?, quizlet_url=?, price_stars=? WHERE id=?",
                (title, description, content_html, status, is_paid, video_url, youtube_id,
                 free_override, quizlet_url, _price_stars, lesson_id),
            )
        else:
            await asyncio.to_thread(
                db.execute,
                "UPDATE lessons SET title=?, description=?, content_html=?, status=?, is_paid=?, "
                "free_override=?, quizlet_url=?, price_stars=? WHERE id=?",
                (title, description, content_html, status, is_paid, free_override,
                 quizlet_url, _price_stars, lesson_id),
            )
    ...
    from urllib.parse import quote as _q
    invalidate_catalog_cache()     # правка урока должна быть видна ученикам сразу
    raise web.HTTPFound(
        f"/admin/learn/lessons/{lesson_id}/edit?message={_q('Урок обновлён.' + wb_msg)}")
...
def _list_private_tests_sync(tg_id: int) -> list:
    ...
    for r in rows:
        r = dict(r)
        if not utils.deadline_active(r["access_expires_at"]):
...
async def admin_test_settings(request: web.Request) -> web.Response:
    ...
    settings = (
        ...
        1 if data.get("allow_retry_wrong") == "on" else 0,
        test_id,
    )
    await asyncio.to_thread(
        db.execute,
        "UPDATE tests SET show_correct=?, show_explanation=?, show_results=?, "
        "attempts_limit=?, time_per_question=?, shuffle_questions=?, "
        "allow_retry_wrong=? WHERE id=?",
        settings,
    )
...
# === Админка: точечная правка вопросов теста ===
# Раньше из-за одной опечатки приходилось сносить тест целиком и заливать
# заново. Здесь можно поправить текст, варианты, правильный ответ, пояснение
# и картинку — по одному вопросу, не трогая остальные.

def _questions_edit_sync(test_id: int) -> Optional[dict]:
    test = db.fetchone("SELECT * FROM tests WHERE id=?", (test_id,))
    if not test:
        return None
    rows = db.fetchall(
    ...
    for q in rows:
        q = dict(q)
        q["options"] = [dict(o) for o in db.fetchall(
            "SELECT * FROM question_options WHERE question_id=? ORDER BY order_num, id",
            (q["id"],))]
        q["image_url"] = (f"/uploads/questions/{os.path.basename(q['web_image_path'])}"
                          if q.get("web_image_path") else "")
        q["has_correct"] = any(o["is_correct"] for o in q["options"])
        # Буквы правильных вариантов — видит только админ
        q["correct_letters"] = ", ".join(
            chr(ord("A") + i) for i, o in enumerate(q["options"]) if o["is_correct"])
        items.append(q)
    lesson = db.fetchone("SELECT * FROM lessons WHERE test_id=?", (test_id,))
    from services import stats_service as _sts
    return {"test": dict(test), "questions": items,
            "lesson": dict(lesson) if lesson else None,
            "broken": [q["id"] for q in items if not q["has_correct"]],
            "test_stats": _sts.test_stats(test_id)}


async def admin_question_goto(request: web.Request) -> web.Response:
    """Прыжок с апелляции сразу в редактор нужного вопроса."""
    await _require_admin(request)
    qid = int(request.match_info["question_id"])
    row = await asyncio.to_thread(
        db.fetchone,
        "SELECT q.test_id, l.id AS lesson_id FROM questions q "
        "LEFT JOIN lessons l ON l.test_id = q.test_id WHERE q.id=?", (qid,))
    if not row:
        raise web.HTTPFound("/admin/appeals?message=Вопрос уже удалён")
    url = f"/admin/learn/tests/{row['test_id']}/questions"
    if row["lesson_id"]:
        url += f"?lesson_id={row['lesson_id']}"
    raise web.HTTPFound(url + ("&" if "?" in url else "?") + f"focus={qid}#q{qid}")


async def admin_questions(request: web.Request) -> web.Response:
    await _require_admin(request)
    test_id = int(request.match_info["test_id"])
    data = await asyncio.to_thread(_questions_edit_sync, test_id)
    if data is None:
        raise web.HTTPNotFound(text="Тест не найден")
    data.update(await auth.nav_context(request))
    data["message"] = request.query.get("message")
    data["lesson_id"] = request.query.get("lesson_id") or (
        data["lesson"]["id"] if data["lesson"] else "")
    # Из апелляции приходят с ?focus=<id>: карточка открывается и прокручивается сама
    data["open_q"] = request.query.get("q") or request.query.get("focus")
    return aiohttp_jinja2.render_template("admin_questions.html", request, data)


def _q_back(test_id: int, lesson_id, message: str = "", qid=None):
    from urllib.parse import quote as _q
    url = f"/admin/learn/tests/{test_id}/questions"
    parts = []
    if lesson_id:
        parts.append(f"lesson_id={lesson_id}")
    if qid:
        parts.append(f"q={qid}")
    if message:
        parts.append("message=" + _q(message))
    if parts:
        url += "?" + "&".join(parts)
    return web.HTTPFound(url)


async def admin_question_save(request: web.Request) -> web.Response:
    """Сохранить один вопрос: текст, пояснение, тему, варианты, правильный."""
    await _require_admin(request)
    ...
    def _save():
        ...
        if not text:
            return q["test_id"], "Текст вопроса не может быть пустым"
        db.execute("UPDATE questions SET text=?, explanation=?, topic=? WHERE id=?",
                   (text, (data.get("explanation") or "").strip(),
                    (data.get("topic") or "").strip(), question_id))

        opts = db.fetchall(
            "SELECT id FROM question_options WHERE question_id=? ORDER BY order_num, id",
            (question_id,))
        # Галочек может быть несколько — берём все отмеченные
        correct_raw = data.getall("correct") if hasattr(data, "getall") else \
            ([data.get("correct")] if data.get("correct") else [])
        for i, o in enumerate(opts):
            key = f"opt_{o['id']}"
            if key not in data:
                continue          # поля не было в форме — вариант не трогаем
            val = (data.get(key) or "").strip()
            if val:
                db.execute("UPDATE question_options SET text=?, order_num=? WHERE id=?",
                           (val, i, o["id"]))
            else:
                # Пустое поле = «удалить этот вариант», так и написано в форме
                db.execute("DELETE FROM question_options WHERE id=?", (o["id"],))
        # Новый вариант, если админ его вписал
        extra = (data.get("new_option") or "").strip()
        new_id = None
        if extra:
            db.execute("INSERT INTO question_options (question_id, text, is_correct, "
                       "order_num) VALUES (?,?,0,?)", (question_id, extra, len(opts)))
            row = db.fetchone("SELECT MAX(id) AS id FROM question_options "
                              "WHERE question_id=?", (question_id,))
            new_id = row["id"] if row else None

        # Правильных вариантов может быть сколько угодно. Тип вопроса
        # отдельно не храним: сколько отмечено, столько и правильных.
        marks = [str(x) for x in (correct_raw or [])]
        if marks:
            db.execute("UPDATE question_options SET is_correct=0 WHERE question_id=?",
                       (question_id,))
            for mark in marks:
                if mark == "__new__" and new_id:
                    db.execute("UPDATE question_options SET is_correct=1 WHERE id=?",
                               (new_id,))
                elif mark.isdigit():
                    db.execute("UPDATE question_options SET is_correct=1 WHERE id=? "
                               "AND question_id=?", (int(mark), question_id))
        return q["test_id"], ""

    test_id, err = await asyncio.to_thread(_save)
    if test_id is None:
        raise web.HTTPNotFound(text=err)
    raise _q_back(test_id, data.get("lesson_id"),
                  err or "Вопрос сохранён", question_id)


async def admin_question_image(request: web.Request) -> web.Response:
    """Заменить или убрать картинку вопроса."""
    await _require_admin(request)
    question_id = int(request.match_info["question_id"])
    fields, upload = {}, None
    # Форма «убрать фото» приходит без файла и может прийти обычной, не multipart —
    # на таком запросе request.multipart() падал с 500.
    if (request.content_type or "").startswith("multipart/"):
        reader = await request.multipart()
        async for part in reader:
            if part.name == "image" and part.filename:
                upload = (part.filename, await part.read())
            else:
        ...
        for k in post.keys():
            v = post.get(k)
            if hasattr(v, "file") and getattr(v, "filename", ""):
                upload = (v.filename, v.file.read())
            else:
                fields[k] = str(v)
    ...
    def _apply():
        ...
        if not q:
            return None, "Вопрос не найден"
        if fields.get("remove") == "1":
            db.execute("UPDATE questions SET web_image_path=NULL WHERE id=?", (question_id,))
            return q["test_id"], "Картинка убрана"
        if not upload:
            return q["test_id"], "Файл не выбран"
        name, blob = upload
        ext = os.path.splitext(name)[1].lower()
        if ext not in (".jpg", ".jpeg", ".png", ".webp", ".gif"):
            return q["test_id"], "Такой формат картинки не поддерживается"
        root = Path(config.DB_PATH).resolve().parent / "uploads" / "questions"
        root.mkdir(parents=True, exist_ok=True)
        fname = f"q{question_id}_{int(datetime.utcnow().timestamp())}{ext}"
        with open(root / fname, "wb") as f:
            f.write(blob)
        db.execute("UPDATE questions SET web_image_path=? WHERE id=?",
                   (f"uploads/questions/{fname}", question_id))
        return q["test_id"], "Картинка обновлена"

    test_id, message = await asyncio.to_thread(_apply)
    if test_id is None:
        raise web.HTTPNotFound(text=message)
    raise _q_back(test_id, fields.get("lesson_id"), message, question_id)


async def admin_question_delete(request: web.Request) -> web.Response:
    await _require_admin(request)
    question_id = int(request.match_info["question_id"])
    data = await request.post()

    def _del():
        q = db.fetchone("SELECT test_id FROM questions WHERE id=?", (question_id,))
        if not q:
            return None
    ...
    test_id = await asyncio.to_thread(_del)
    if test_id is None:
        raise web.HTTPNotFound(text="Вопрос не найден")
    raise _q_back(test_id, data.get("lesson_id"), "Вопрос удалён")


async def admin_question_add(request: web.Request) -> web.Response:
    ...
    def _add():
        ...
        opts = [(data.get(f"o{i}") or "").strip() for i in range(1, 5)]
        opts = [o for o in opts if o]
        if len(opts) < 2:
            return "Нужно хотя бы два варианта ответа"
        try:
            correct = int(data.get("correct") or 1)
        except ValueError:
            correct = 1
        correct = min(max(correct, 1), len(opts))
        nxt = db.fetchone("SELECT COALESCE(MAX(order_num), 0) AS m FROM questions "
                          "WHERE test_id=?", (test_id,))["m"]
        db.execute("INSERT INTO questions (test_id, text, explanation, topic, order_num) "
                   "VALUES (?,?,?,?,?)",
                   (test_id, text, (data.get("explanation") or "").strip(),
                    (data.get("topic") or "").strip(), nxt + 1))
        qid = db.fetchone("SELECT MAX(id) AS id FROM questions WHERE test_id=?",
                          (test_id,))["id"]
        for i, o in enumerate(opts):
            db.execute("INSERT INTO question_options (question_id, text, is_correct, "
                       "order_num) VALUES (?,?,?,?)", (qid, o, 1 if i + 1 == correct else 0, i))
        return ""

    message = await asyncio.to_thread(_add)
    raise _q_back(test_id, data.get("lesson_id"), message or "Вопрос добавлен")


async def admin_question_move(request: web.Request) -> web.Response:
    await _require_admin(request)
    question_id = int(request.match_info["question_id"])
    data = await request.post()
    direction = "up" if (data.get("dir") or "up") == "up" else "down"
    ...
    def _run():
        ...
        if not q:
            return None
        rows = db.fetchall("SELECT id FROM questions WHERE test_id=? "
                           "ORDER BY order_num, id", (q["test_id"],))
        ids = [r["id"] for r in rows]
        ...
        if 0 <= j < len(ids):
            ...
            for pos, qid in enumerate(ids):
                db.execute("UPDATE questions SET order_num=? WHERE id=?", (pos, qid))
        return q["test_id"]

    test_id = await asyncio.to_thread(_run)
    if test_id is None:
        raise web.HTTPNotFound(text="Вопрос не найден")
    raise _q_back(test_id, data.get("lesson_id"), "Порядок вопросов изменён")
...
def _premium_log_sync(limit: int = 500) -> dict:
    ...
    for r in rows:
        ...
        items.append(r)
    # Считаем тем же решением, что и доступ (utils.deadline_active), а не
    # строковым сравнением в SQL: иначе нечитаемый срок числился «активным».
    active = sum(1 for r in db.fetchall("SELECT expires_at FROM premium_users")
                 if utils.deadline_active(r["expires_at"]))
...
async def admin_notes_log(request: web.Request) -> web.Response:
    ...
    context["notes_ttl_hours"] = await asyncio.to_thread(
        lambda: int(_setting_sync("notes_ttl_hours", "24") or 24))
    context["message"] = request.query.get("message")
    return aiohttp_jinja2.render_template("admin_notes_log.html", request, context)


async def admin_notes_ttl(request: web.Request) -> web.Response:
    """Сколько часов конспект живёт в переписке ученика."""
    await _require_admin(request)
    data = await request.post()
    try:
        hours = max(0, min(8760, int((data.get("notes_ttl_hours") or "0").strip())))
    except ValueError:
        hours = 24

    def _save():
        db.execute("INSERT OR REPLACE INTO settings (key, value) "
                   "VALUES ('notes_ttl_hours', ?)", (str(hours),))
    await asyncio.to_thread(_save)
    label = "не удалять" if hours == 0 else f"{hours} ч"
    raise web.HTTPFound(f"/admin/learn/notes-log?message=Срок хранения: {label}")
...
async def admin_copy_subject(request: web.Request) -> web.Response:
    """Копия предмета — всегда независимая (services/deep_copy_service): своё
    содержимое и настройки, правки в ней не трогают оригинал, и наоборот."""
    admin_tg = await _require_admin(request)
    data = await request.post()
    subject_id = int(request.match_info["subject_id"])
    from services import deep_copy_service as _dc
    try:
        new_id = await asyncio.to_thread(
            _dc.copy_subject_full, subject_id, (data.get("title") or "").strip(), admin_tg)
    except Exception as e:
        logger.exception("копия предмета %s", subject_id)
        raise _back(data, f"⚠️ Копия не создана ({e}). Оригинал не изменён.")
    if not new_id:
        raise _back(data, "Предмет не найден")
    stats = await asyncio.to_thread(_dc.summary, new_id)
    invalidate_catalog_cache()
    from urllib.parse import quote as _q
    raise web.HTTPFound(
        _hier_url(new_id) + "&message=" + _q(
            f"Создана независимая копия: разделов {stats['sections']}, уроков {stats['lessons']}, "
            f"вопросов {stats['questions']}. Меняйте в ней что угодно — оригинал не изменится"))


async def admin_copy_subject_full(request: web.Request) -> web.Response:
    """Прежний адрес «полной копии» — то же самое."""
    return await admin_copy_subject(request)


async def admin_convert_subject(request: web.Request) -> web.Response:
    """Старая копия-ярлык → независимая (обычно это происходит само при запуске)."""
    await _require_admin(request)
    data = await request.post()
    subject_id = int(request.match_info["subject_id"])
    from services import copy_convert as _cc
    try:
        res = await asyncio.to_thread(_cc.convert_subject, subject_id)
    except Exception as e:
        logger.exception("перевод копии-ярлыка %s", subject_id)
        raise _back(data, f"⚠️ Не получилось ({e}). Ничего не изменено.")
    invalidate_catalog_cache()
    raise _back(data, "Копия стала независимой: теперь в ней можно менять всё, прогресс учеников сохранён"
                if res else "Эта копия уже независимая")


async def admin_convert_lesson(request: web.Request) -> web.Response:
    await _require_admin(request)
    lesson_id = int(request.match_info["lesson_id"])
    from services import copy_convert as _cc
    from urllib.parse import quote as _q
    try:
        res = await asyncio.to_thread(_cc.convert_lesson, lesson_id)
        msg = ("Урок стал независимым: теперь здесь можно менять всё, прогресс учеников сохранён"
               if res else "Этот урок уже независимый")
    except Exception as e:
        logger.exception("перевод урока-ярлыка %s", lesson_id)
        msg = f"⚠️ Не получилось ({e}). Ничего не изменено."
    invalidate_catalog_cache()
    raise web.HTTPFound(f"/admin/learn/lessons/{lesson_id}/edit?message={_q(msg)}")


async def admin_copy_compare(request: web.Request) -> web.Response:
    """«Сравнить с оригиналом» — только посмотреть, ничего не меняет."""
    await _require_admin(request)
    subject_id = int(request.match_info["subject_id"])
    from services import copy_compare as _cmp
    cmp = await asyncio.to_thread(_cmp.compare, subject_id)
    if cmp is None:
        raise web.HTTPNotFound(text="Предмет не найден")
    data = await auth.nav_context(request)
    data["cmp"] = cmp
    return aiohttp_jinja2.render_template("admin_copy_compare.html", request, data)


def _idents_to_users(idents: list) -> tuple:
    """(найденные пользователи, ненайденные идентификаторы)."""
    found, missing = [], []
    for ident in idents:
        u = utils.find_user_by_arg(ident)
        if u:
            found.append(u)
        else:
...
def _stop_msg(action: str, found: list, missing: list) -> str:
    msg = f"{action}: {len(found)}"
    if missing:
        msg += f"; не найдены (ещё не запускали бота): {', '.join(missing)}"
    return msg


async def admin_stop_unlock(request: web.Request) -> web.Response:
    """Уроки до выбранного включительно считать пройденными у ученика."""
    admin_id = await _require_admin(request)
    subject_id = int(request.match_info["subject_id"])
    data = await request.post()
    idents = _parse_bulk_idents(data.get("user_tg_id") or "", limit=100)
    lesson_id = _int_or_none(data.get("lesson_id"))
    if not idents or not lesson_id:
        raise _back(data, "Укажите ученика и урок")

    def _do():
        found, missing = _idents_to_users(idents)
        for u in found:
            _so.unlock_upto(u["tg_id"], subject_id, lesson_id, admin_id)
        return found, missing

    found, missing = await asyncio.to_thread(_do)
    raise _back(data, _stop_msg("Уроки открыты", found, missing))


async def admin_stop_off(request: web.Request) -> web.Response:
    """Снять стоп-уроки предмета с ученика целиком."""
    admin_id = await _require_admin(request)
    subject_id = int(request.match_info["subject_id"])
    data = await request.post()
    idents = _parse_bulk_idents(data.get("user_tg_id") or "", limit=100)
    if not idents:
        raise _back(data, "Укажите ученика")

    def _do():
        found, missing = _idents_to_users(idents)
        for u in found:
            _so.set_off(u["tg_id"], subject_id, admin_id)
        return found, missing

    found, missing = await asyncio.to_thread(_do)
    raise _back(data, _stop_msg("Стоп-уроки отключены", found, missing))


async def admin_stop_remove(request: web.Request) -> web.Response:
    await _require_admin(request)
    subject_id = int(request.match_info["subject_id"])
    data = await request.post()
    tg = _int_or_none(data.get("tg_id"))
    if not tg:
        raise _back(data, "Ученик не указан")
    kind = (data.get("kind") or "").strip() or None
    await asyncio.to_thread(_so.remove, tg, subject_id, kind)
    raise _back(data, "Послабление убрано — стоп-уроки снова действуют")


async def admin_copy_section(request: web.Request) -> web.Response:
    """Независимая копия раздела (со всеми уроками) в другой предмет."""
    admin_tg = await _require_admin(request)
    data = await request.post()
    section_id = int(request.match_info["section_id"])
    ...
    from services import deep_copy_service as _dc
    try:
        new_id = await asyncio.to_thread(_dc.copy_section_full, section_id, target, admin_tg)
    except Exception as e:
        logger.exception("копия раздела %s", section_id)
        raise _back(data, f"⚠️ Раздел не скопирован ({e}). Ничего не изменено.")
    if not new_id:
        raise _back(data, "Раздел не найден")
    from urllib.parse import quote as _q
    raise web.HTTPFound(_hier_url(target, new_id) + "&message=" + _q(
        "Раздел скопирован. Это независимая копия — её уроки можно менять как угодно"))


async def admin_copy_lesson(request: web.Request) -> web.Response:
    """Независимая копия урока в другой раздел (любого предмета)."""
    admin_tg = await _require_admin(request)
    data = await request.post()
    lesson_id = int(request.match_info["lesson_id"])
    ...
    from services import deep_copy_service as _dc
    try:
        new_id = await asyncio.to_thread(_dc.copy_lesson_full, lesson_id, target, admin_tg)
    except Exception as e:
        logger.exception("копия урока %s", lesson_id)
        raise _back(data, f"⚠️ Урок не скопирован ({e}). Ничего не изменено.")
    if not new_id:
        raise _back(data, "Урок не найден")
    raise _back(data, "Урок скопирован. Это независимая копия — её можно менять как угодно",
                _hier_url(None, target))


def _copy_targets_sync() -> dict:
    """Списки предметов и разделов для выпадашек «куда копировать»."""
    subs = db.fetchall(
        "SELECT id, title, original_id, copied_from_subject_id FROM subjects ORDER BY sort_order, id")
    secs = db.fetchall(
...
def _premium_users_sync() -> list:
    ...
    for r in rows:
        r = dict(r)
        # Тем же решением, что и доступ ученика — список не расходится с ботом
        if not utils.deadline_active(r["expires_at"]):
            continue
...
async def admin_grant_premium(request: web.Request) -> web.Response:
    ...
    def _grant():
        ...
        for ident in idents:
            ...
            if user.get("tg_id"):
                targets.append(user["tg_id"])
        return done, missing, targets

    done, missing, targets = await asyncio.to_thread(_grant)

    # Человеку: стандартное поздравление, а следом — инструкция из админки.
    # Сайт и бот работают раздельно, поэтому поднимаем клиента по токену.
    if targets:
        try:
            from aiogram import Bot
            ...
            try:
                for target in targets:
                    await _onb.notify_manual_grant(bot, target, days, source="admin_site")
            finally:
                await bot.session.close()
        except Exception as e:
            logger.warning("premium notify (сайт): %s", e)
...
async def admin_revoke_premium(request: web.Request) -> web.Response:
    ...
    await asyncio.to_thread(
        ...
        (target,))
    # Премиум отозван — вознаграждение аннулируется, из рейтинга предмета выбывает сразу
    from services import reward_service as _rs
    await asyncio.to_thread(_rs.check_user, target)
...
def register_routes(app: web.Application) -> None:
    ...
    app.router.add_get("/learn/{subject_id:\\d+}/section/{section_id:\\d+}", learn_section)
    app.router.add_get("/learn/lesson/{lesson_id:\\d+}", learn_lesson)
    app.router.add_get("/learn/lesson/{lesson_id:\\d+}/workbook", lesson_workbook)
    ...
    app.router.add_get("/learn/test/{attempt_id:\\d+}/retry-wrong", learn_retry_wrong)
    ...
    app.router.add_get("/admin/learn/tests/{test_id:\\d+}/questions", admin_questions)
    app.router.add_get("/admin/learn/questions/{question_id:\\d+}/goto", admin_question_goto)
    app.router.add_post("/admin/learn/questions/{question_id:\\d+}/save", admin_question_save)
    app.router.add_post("/admin/learn/questions/{question_id:\\d+}/image", admin_question_image)
    app.router.add_post("/admin/learn/questions/{question_id:\\d+}/delete", admin_question_delete)
    app.router.add_post("/admin/learn/questions/{question_id:\\d+}/move", admin_question_move)
    app.router.add_post("/admin/learn/tests/{test_id:\\d+}/questions/add", admin_question_add)
    ...
    app.router.add_post("/admin/learn/notes-ttl", admin_notes_ttl)
    ...
    app.router.add_post("/admin/learn/subjects/{subject_id:\\d+}/copy-full", admin_copy_subject_full)
    app.router.add_post("/admin/learn/subjects/{subject_id:\\d+}/convert", admin_convert_subject)
    app.router.add_get("/admin/learn/subjects/{subject_id:\\d+}/compare", admin_copy_compare)
    app.router.add_post("/admin/learn/lessons/{lesson_id:\\d+}/convert", admin_convert_lesson)
    app.router.add_post("/admin/learn/subjects/{subject_id:\\d+}/stop/unlock", admin_stop_unlock)
    app.router.add_post("/admin/learn/subjects/{subject_id:\\d+}/stop/off", admin_stop_off)
    app.router.add_post("/admin/learn/subjects/{subject_id:\\d+}/stop/remove", admin_stop_remove)
    ...
    app.router.add_post("/admin/learn/subjects/{subject_id:\\d+}/move", admin_move_subject)
    app.router.add_post("/admin/learn/subjects/{subject_id:\\d+}/pin", admin_pin_subject)
    app.router.add_post("/admin/learn/sections/{section_id:\\d+}/move", admin_move_section)
    app.router.add_post("/admin/learn/lessons/{lesson_id:\\d+}/move", admin_move_lesson)
