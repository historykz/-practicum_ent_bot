...
def _list_subjects_public_sync(tg_id: Optional[int]) -> list:
    ...
    is_adm = bool(tg_id) and utils.is_admin(tg_id)
    result = []
    for s in subjects:
        s = dict(s)
...
def _subject_detail_sync(subject_id: int, tg_id: Optional[int]) -> Optional[dict]:
    ...
    if subject["is_private"] and not has_access and not (bool(tg_id) and utils.is_admin(tg_id)):
    ...
    return {"subject": dict(subject), "sections": result_sections, "has_access": has_access}
...
async def _check_subject_subscription(request: web.Request, subject_id: int,
    ...
    if tg_id is None:
        return None
    if await asyncio.to_thread(utils.is_admin, tg_id):
        return None
...
async def admin_create_subject(request: web.Request) -> web.Response:
    ...
    if title:
        await asyncio.to_thread(
            ...
            "VALUES (?, ?, ?, ?, ?)",
            (title, description, is_open, is_private, tg_id),
        )
...
async def admin_edit_subject(request: web.Request) -> web.Response:
    ...
    if title:
        await asyncio.to_thread(
            db.execute,
            "UPDATE subjects SET title=?, description=?, is_open=?, is_private=? WHERE id=?",
            (title, description, is_open, is_private, subject_id),
        )
...
def _restore_backup_sync(zip_path: str) -> str:
    ...
    if not zipfile.is_zipfile(zip_path):
        return "Файл не является ZIP-архивом (или загрузка не завершилась)"
    tmp = tempfile.mkdtemp(prefix="restore_")
    with zipfile.ZipFile(zip_path) as zf:
        names = zf.namelist()
        if "bot.db" not in names:
            return "В архиве нет bot.db — это не полный бэкап сайта"
        zf.extract("bot.db", tmp)
        data_root = Path(config.DB_PATH).resolve().parent
        for n in names:
            # защита от zip-slip: только файлы внутри uploads/, без ../
            if n.startswith("uploads/") and ".." not in n and not n.endswith("/"):
                zf.extract(n, data_root)

    src = sqlite3.connect(os.path.join(tmp, "bot.db"))
    try:
        with db.db_lock() as conn:
            src.backup(conn)
    finally:
