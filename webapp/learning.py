...
async def admin_create_lesson(request: web.Request) -> web.Response:
    ...
    if test_mode == "none" or (test_mode == "text" and not test_text.strip()) or \
        ...
        await asyncio.to_thread(
            db.execute,
            "INSERT INTO lessons (section_id, title, description, content_html, video_url, youtube_id) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (section_id, title, description, content_html, video_url, youtube_id),
        )
    ...
    draft_id = await asyncio.to_thread(
        ...
        test_mode, test_text, zip_bytes, video_url, youtube_id,
    )
...
def _create_draft_sync(admin_tg_id: int, section_id: int, lesson_id: Optional[int],
                        ...
                        video_url: Optional[str] = None, youtube_id: Optional[str] = None) -> int:
    if test_mode == "text":
...
async def admin_edit_lesson(request: web.Request) -> web.Response:
    ...
    if title:
        if update_video:
            await asyncio.to_thread(
                db.execute,
                "UPDATE lessons SET title=?, description=?, content_html=?, status=?, "
                "video_url=?, youtube_id=? WHERE id=?",
                (title, description, content_html, status, video_url, youtube_id, lesson_id),
            )
        else:
            await asyncio.to_thread(
                db.execute,
