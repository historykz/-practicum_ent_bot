...
async def live_play_page(request):
    """Экран игрока. Открывается и по ссылке, и по QR-коду.

    Вход не требуем: внутри мини-приложения человек авторизуется сам, и
    выбрасывать его на главную нельзя — он потеряет комнату. Если комнаты
    нет или игра уже закончилась, показываем понятное объяснение, а не
    пустой экран.
    """
    ...
    ctx = await auth.nav_context(request)
    ctx["code"] = code
    ctx["room_mode"] = room["mode"] if room else ""
    if not room:
        ctx["room_error"] = "notfound"
    elif room["status"] == "finished":
        ctx["room_error"] = "finished"
    else:
        ctx["room_error"] = ""
...
# Сколько показывать статистику перед автоматическим переходом к следующему
# вопросу: успеть увидеть правильный ответ, но не заскучать.
STATS_PAUSE_SEC = 6


def _advance_once(room, from_index, to_index, expect_status):
    """Один переход — и только один.

    Таймер и кнопка ведущего могут сработать в одну секунду. Переключаем
    строго через UPDATE с проверкой текущего состояния: побеждает тот, чей
    запрос пришёл первым, второй просто ничего не меняет. Поэтому перескочить
    через задание невозможно.
    """
    now = _iso(_now())
    if to_index is None:      # дальше вопросов нет — комната закончилась
        cur = db.execute(
            "UPDATE live_rooms SET status='finished', advance_lock=? "
            "WHERE id=? AND status=? AND current_index=?",
            (now, room["id"], expect_status, from_index))
    else:
        cur = db.execute(
            "UPDATE live_rooms SET status='question', current_index=?, "
            "question_started_at=?, stats_started_at=NULL, advance_lock=? "
            "WHERE id=? AND status=? AND current_index=?",
            (to_index, now, now, room["id"], expect_status, from_index))
    changed = getattr(cur, "rowcount", 0) or 0
    return changed > 0


def _close_question(room, idx):
    """Закрыть приём ответов и показать статистику. Тоже строго один раз."""
    cur = db.execute(
        "UPDATE live_rooms SET status='stats', stats_started_at=? "
        "WHERE id=? AND status='question' AND current_index=?",
        (_iso(_now()), room["id"], idx))
    return (getattr(cur, "rowcount", 0) or 0) > 0


def _auto_advance_sync(room):
    """Ход времени в комнате. Считает только сервер, поэтому у всех участников
    состояние одинаковое, независимо от того, когда кто опросил /state.

    1. Время вышло (или ответили все) → приём ответов закрыт, показ статистики.
    2. Если ведущий включил авто-переход, после короткой паузы сам открывает
       следующее задание. Если выключил — ждём кнопку ведущего.
    """
    if room["status"] == "question":
        ...
        answered = db.fetchone(
            ...
            (room["id"], idx))["c"]
        players = db.fetchone(
            "SELECT COUNT(*) c FROM live_players WHERE room_id=? AND kicked=0",
            (room["id"],))["c"]
        if _remaining_seconds(room) <= 0 or (players > 0 and answered >= players):
            _close_question(room, idx)
            room = db.fetchone("SELECT * FROM live_rooms WHERE id=?", (room["id"],))

    if room["status"] == "stats" and _auto_advance_on(room):
        started = _parse(room["stats_started_at"])
        if started is None:
            # Комната из прошлой версии: статистику уже показывали, но метки
            # времени не было. Ставим сейчас — отсчёт пойдёт от этого момента.
            db.execute("UPDATE live_rooms SET stats_started_at=? WHERE id=? "
                       "AND stats_started_at IS NULL", (_iso(_now()), room["id"]))
            return db.fetchone("SELECT * FROM live_rooms WHERE id=?", (room["id"],))
        if (_now() - started).total_seconds() >= STATS_PAUSE_SEC:
            idx = room["current_index"]
            _, total = _question_at(room, 0)
            nxt = idx + 1
            _advance_once(room, idx, nxt if nxt < total else None, "stats")
            return db.fetchone("SELECT * FROM live_rooms WHERE id=?", (room["id"],))
    return room


def _auto_advance_on(room):
    """Включён ли авто-переход. Старые комнаты без колонки — считаем включённым."""
    try:
        val = room["auto_advance"]
    except (KeyError, IndexError, TypeError):
        return True
    return val is None or bool(val)


def _stats_left(room):
    """Сколько секунд осталось до автоматического перехода (для отсчёта в UI)."""
    if room["status"] != "stats" or not _auto_advance_on(room):
        return None
    started = _parse(room["stats_started_at"])
    if not started:
        return STATS_PAUSE_SEC
    return max(0, round(STATS_PAUSE_SEC - (_now() - started).total_seconds(), 1))
...
def _player_state(room, tg_id):
    ...
    if room["status"] == "question" and q:
        ...
        # Ученику показываем отсчёт, чтобы переход не был внезапным
        out["auto_advance"] = _auto_advance_on(room)
        out["next_in"] = _stats_left(room)
    elif room["status"] == "finished":
...
async def live_answer(request):
    ...
    def _do():
        ...
        # Ответ — набор отмеченных вариантов. Засчитываем только полное
        # совпадение с правильным набором, как и в остальных режимах.
        chosen = _ac.belongs_to_question(
            q["id"], option_ids if option_ids is not None else option_id)
        if not chosen:
            return {"error": "bad_option"}
        is_correct = _ac.is_answer_correct(q["id"], chosen)
        ...
        try:
            db.execute(
                ...
                "option_ids, is_correct, answer_ms, points) VALUES (?,?,?,?,?,?,?,?,?)",
                (room["id"], idx, q["id"], tg_id, chosen[0], _ac.dump(chosen),
                 1 if is_correct else 0, answer_ms, points))
        except Exception:
...
async def live_host_page(request):
    ...
    # Ссылка и QR ведут в мини-приложение Telegram, а не на сайт: аккаунтов
    # на сайте больше нет, вход происходит сам, и ученик сразу попадает
    # в нужную комнату.
    _short = (getattr(config, "WEB_APP_SHORT_NAME", "") or "").strip()
    _base = f"https://t.me/{config.WEB_BOT_USERNAME}"
    if _short:
        _base += f"/{_short}"
    ctx["join_url"] = f"{_base}?startapp=live_{code}"
...
async def live_host_state(request):
    ...
    def _do():
        ...
        out = {"status": room["status"], "code": code, "mode": room["mode"],
               ...
               "rating_visibility": room["rating_visibility"],
               "auto_advance": _auto_advance_on(room),
               "next_in": _stats_left(room)}
        q, total = _question_at(room, room["current_index"])
        out["q_num"] = room["current_index"] + 1
        out["q_total"] = total
        if room["status"] == "question" and q:
...
async def live_host_action(request):
    ...
    def _do():
        ...
        if action == "start":
            ...
            db.execute("UPDATE live_rooms SET status='stats', stats_started_at=? "
                       "WHERE id=? AND status='question'", (_iso(_now()), room["id"]))
        elif action == "next":
            # Кнопка ведущего и авто-переход идут одной дорогой: кто успел
            # первым, тот и переключил. Второй ничего не делает, поэтому
            # через задание не перескакиваем.
            nxt = idx + 1
            _advance_once(room, idx, nxt if nxt < total else None, room["status"])
        elif action == "finish":
            ...
            auto = body.get("auto_advance")
            if auto is not None:
                db.execute("UPDATE live_rooms SET auto_advance=? WHERE id=?",
                           (1 if auto else 0, room["id"]))
        return {"ok": True}

    return web.json_response(await asyncio.to_thread(_do))


# ============ Отчёты преподавателя: история, баллы, ошибки ============

def _live_history_sync(tg_id):
    rows = db.fetchall(
        """SELECT r.id, r.code, r.status, r.created_at, t.title,
                  (SELECT COUNT(*) FROM live_players WHERE room_id=r.id AND kicked=0) players
           FROM live_rooms r LEFT JOIN tests t ON t.id = r.test_id
           WHERE r.host_tg_id=? OR ?=1 ORDER BY r.id DESC LIMIT 100""",
        (tg_id, 1 if utils.is_owner(tg_id) else 0))
    return [{"code": r["code"], "status": r["status"], "title": r["title"] or "-",
             "players": r["players"], "when": (r["created_at"] or "")[:16].replace("T", " ")}
            for r in rows]


async def live_history_page(request):
    tg_id = await _require_admin(request)
    ctx = await auth.nav_context(request)
    ctx["rooms"] = await asyncio.to_thread(_live_history_sync, tg_id)
    return aiohttp_jinja2.render_template("live_history.html", request, ctx)


def _live_report_sync(code, tg_id):
    room = _room_by_code(code)
    if not room:
        return None
    if room["host_tg_id"] != tg_id and not utils.is_owner(tg_id):
        return None
    test = db.fetchone("SELECT title FROM tests WHERE id=?", (room["test_id"],))
    try:
        qids = json.loads(room["question_order"] or "[]")
    except (ValueError, TypeError):
        qids = []
    ...
    for p in db.fetchall(
            "SELECT * FROM live_players WHERE room_id=? AND kicked=0 "
            "ORDER BY score DESC, total_time_ms ASC", (room["id"],)):
        answers = db.fetchall(
            """SELECT la.question_index, la.is_correct, q.text qtext, uo.text user_ans,
                      (SELECT text FROM question_options WHERE question_id=q.id AND is_correct=1 LIMIT 1) correct_ans
               FROM live_answers la JOIN questions q ON q.id = la.question_id
               LEFT JOIN question_options uo ON uo.id = la.option_id
               WHERE la.room_id=? AND la.tg_id=? ORDER BY la.question_index""",
            (room["id"], p["tg_id"]))
        mistakes = [{"n": a["question_index"] + 1, "q": a["qtext"],
                     "user": a["user_ans"] or "—", "correct": a["correct_ans"] or "—"}
                    for a in answers if not a["is_correct"]]
        answered = len(answers)
        players.append({
            "name": p["name"], "tg_id": p["tg_id"], "score": p["score"],
            "correct": p["correct"], "wrong": max(0, answered - p["correct"]),
            "skipped": max(0, len(qids) - answered),
            "percent": round(p["correct"] / len(qids) * 100) if qids else 0,
            "best_streak": p["best_streak"],
            "avg_sec": round(p["total_time_ms"] / answered / 1000, 1) if answered else 0,
            "mistakes": mistakes})
    hard = []
    for idx, qid in enumerate(qids):
        q = db.fetchone("SELECT text FROM questions WHERE id=?", (qid,))
        if not q:
            continue
        tot = db.fetchone("SELECT COUNT(*) c FROM live_answers WHERE room_id=? AND question_index=?",
                          (room["id"], idx))["c"]
        ok = db.fetchone("SELECT COUNT(*) c FROM live_answers WHERE room_id=? AND question_index=? AND is_correct=1",
                         (room["id"], idx))["c"]
        if tot:
            hard.append({"n": idx + 1, "text": q["text"], "total": tot, "correct": ok,
                         "percent": round(ok / tot * 100)})
    hard.sort(key=lambda h: h["percent"])
    return {"code": code, "test_title": test["title"] if test else "-",
            "players": players, "hard": hard[:10], "q_total": len(qids)}


async def live_report_page(request):
    tg_id = await _require_admin(request)
    data = await asyncio.to_thread(_live_report_sync, request.match_info["code"], tg_id)
    if not data:
        raise web.HTTPNotFound(text="Отчёт не найден")
    data.update(await auth.nav_context(request))
    return aiohttp_jinja2.render_template("live_report.html", request, data)


async def live_report_csv(request):
    import csv
    import io
    tg_id = await _require_admin(request)
    code = request.match_info["code"]
    data = await asyncio.to_thread(_live_report_sync, code, tg_id)
    if not data:
        raise web.HTTPNotFound()
    buf = io.StringIO()
    buf.write("\ufeff")
    w = csv.writer(buf, delimiter=";")
    w.writerow(["Место", "Ученик", "ID", "Баллы", "Верно", "Неверно", "Пропущено", "%", "Серия", "Ср.время"])
    for i, p in enumerate(data["players"], 1):
        w.writerow([i, p["name"], p["tg_id"], p["score"], p["correct"], p["wrong"],
                    p["skipped"], p["percent"], p["best_streak"], p["avg_sec"]])
    w.writerow([])
    w.writerow(["ОШИБКИ УЧЕНИКОВ"])
    w.writerow(["Ученик", "№", "Вопрос", "Ответ ученика", "Правильный"])
    for p in data["players"]:
        for m in p["mistakes"]:
            w.writerow([p["name"], m["n"], m["q"], m["user"], m["correct"]])
    return web.Response(body=buf.getvalue().encode("utf-8"),
        headers={"Content-Type": "text/csv; charset=utf-8",
                 "Content-Disposition": f'attachment; filename="live_{code}.csv"'})


def register_routes(app):
    app.router.add_get("/admin/live/history", live_history_page)
    app.router.add_get("/admin/live/report/{code:\\d+}", live_report_page)
    app.router.add_get("/admin/live/report/{code:\\d+}.csv", live_report_csv)
    # ученик
