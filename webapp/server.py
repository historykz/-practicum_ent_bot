...
async def error_pages_middleware(request: web.Request, handler):
    try:
        ...
        raise web.HTTPFound(
            f"{referer}?message=Файл слишком большой (максимум 50МБ). "
            "Для больших видео используйте ссылку на YouTube."
        )


def create_app() -> web.Application:
    # Railway обрывает слишком долгие загрузки по таймауту прокси (не настраивается
    # из кода приложения) — большому файлу лучше сразу чётко отказать, чем зависнуть
    # на несколько минут и оборваться само по себе. 50МБ — разумный компромисс;
    # для видео длиннее пары минут всё равно надёжнее ссылка на YouTube.
    app = web.Application(
        client_max_size=50 * 1024 * 1024,  # 50МБ
