...
from webapp import appeals_web, accounts, music, publications, broadcast_links, rewards_web
        ...
        try:
            if tg_id and await asyncio.to_thread(utils.is_site_admin, tg_id):
                ...
                frames = [f for f in traceback.extract_tb(e.__traceback__)
                          if f.filename.startswith(root)][-4:]
                debug = f"{type(e).__name__}: {str(e)[:400]}\n" + "\n".join(
                    f"{os.path.relpath(f.filename, root)}:{f.lineno} {f.name}" for f in frames)
        except Exception:
            debug = ""
