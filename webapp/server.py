...
from webapp import appeals_web, accounts, music, publications, broadcast_links, rewards_web, autoruns_web
...
async def error_pages_middleware(request: web.Request, handler):
    ...
    try:
        ...
        # Администратору — сама причина прямо на экране: тип, текст и строки
        # кода. Без доступа к логам Railway её иначе не увидеть. Ученики
        # по-прежнему видят только общий текст.
        debug = ""
        try:
            if tg_id and await asyncio.to_thread(utils.is_site_admin, tg_id):
                root = os.path.realpath(str(BASE_DIR.parent))
                frames = [f for f in traceback.extract_tb(e.__traceback__)
                          if os.path.realpath(f.filename).startswith(root)][-4:]
                debug = f"{type(e).__name__}: {str(e)[:400]}\n" + "\n".join(
                    f"{os.path.relpath(os.path.realpath(f.filename), root)}:{f.lineno} {f.name}"
                    for f in frames)
                # Частая причина — на сервер дошла только часть файлов новой версии
                from webapp import build_check as _bc
                note = _bc.summary()
                if note:
                    debug += "\n\n" + note
        except Exception:
        ...
        if _wants_json(request):
            body = {"ok": False, "error": code, "request_id": rid}
            if debug:
                body["debug"] = debug
        ...
        try:
            ctx = {"static_v": static_version(), "logged_in": False,
                   ...
                   "request_id": rid, "status": status, "debug": debug,
            ...
            return web.Response(
                ...
                text=(f"<h2>Что-то пошло не так</h2><p>Обновите страницу.</p>"
                      ...
                      + (f"<pre style='white-space:pre-wrap'>{utils.escape_html(debug)}</pre>"
                         if debug else "")))
...
_CRITICAL_COLUMNS = (
    ...
    ("test_attempts", "cloned_from_attempt_id"),
    ("subjects", "copied_from_subject_id"),
)
...
async def health_ready(request: web.Request) -> web.Response:
    ...
    if missing:
        ok = False
    # Только для сведения: сервер работает, но файлы могут быть из разных версий
    try:
        from webapp import build_check as _bc
        b = _bc.check()
        checks["build"] = ("ok" if b["ok"] else
                           {"version": b["version"], "stale": b["stale"], "missing": b["missing"]})
        if b.get("version"):
            checks["version"] = b["version"]
    except Exception:
        pass
...
def create_app() -> web.Application:
    ...
    app = web.Application(
        ...
        # request_context — снаружи: код запроса появляется до обработки ошибки,
        # и на странице ошибки виден настоящий код, а не «-».
        middlewares=[request_context_middleware, error_pages_middleware],
    )
    # Все ли файлы на сервере из одной версии — сразу при запуске (итог в лог)
    try:
        from webapp import build_check as _bc
        _bc.check()
    except Exception as e:
    ...
    broadcast_links.register_routes(app)
    rewards_web.register_routes(app)
    autoruns_web.register_routes(app)
