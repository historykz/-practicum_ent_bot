...
AI_PROMPT = """Ты — генератор банка вопросов для учебного зачёта. Сгенерируй вопросы по предмету [ПРЕДМЕТ/ТЕМА КУРСА] СТРОГО в текстовом формате ниже. Не пиши ничего до и после — ни приветствий, ни пояснений, ни markdown (никаких **, #, ```), ни нумерованных списков кроме указанной разметки.

ФОРМАТ (соблюдай посимвольно, включая двоеточия и пробелы):
...
ПАРАМЕТРЫ (заполни под себя):
- Количество тем: [ЧИСЛО ТЕМ]
- Названия тем: [ПЕРЕЧИСЛИ ЧЕРЕЗ ЗАПЯТУЮ]
- Вопросов в каждой теме: [ЧИСЛО, например 30] — во ВСЕХ темах одинаково.

ЖЁСТКИЕ ПРАВИЛА:
1. Ключевые слова ровно "Тема N:", "Вопрос N:", "Ответ N:" (с большой буквы, двоеточие, затем пробел). Не заменяй их синонимами, не пиши "Тема №1", "В1", "О1".
2. Нумерация вопросов внутри КАЖДОЙ темы начинается заново с 1 (у каждой темы свои Вопрос 1, Вопрос 2 ...). Номер у Вопрос N и Ответ N обязан совпадать.
3. Ответ — на ОТДЕЛЬНОЙ строке сразу под своим вопросом. Одна строка = один вопрос или один ответ, без переноса вопроса на несколько строк.
4. Ответы КОРОТКИЕ и однозначные: дата/год, имя, термин, название, число, одно короткое словосочетание. Никаких развёрнутых предложений и объяснений — ответ сверяется автоматически по совпадению.
5. Если у ответа есть допустимые синонимы/варианты написания — перечисли их через слэш в одну строку. Пример: "Ответ 1: он ок будун / онокбудун / он-ок". Система засчитает любой из них.
6. Числа и даты пиши цифрами (552, а не «пятьсот пятьдесят два»); при необходимости добавь «до н.э.» — система сравнивает по цифрам.
7. Не повторяй вопросы между темами и внутри темы. Формулировки вопросов делай разными по сложности: простые факты и вопросы на понимание.
8. Между темами оставляй одну пустую строку (для читаемости). Внутри темы пустая строка между парами вопрос-ответ по желанию.
9. Пиши на русском (или на языке предмета), кодировка UTF-8. Не используй эмодзи, кавычки-ёлочки внутри ответов и таблицы.

ПРИМЕР (правильно оформленный фрагмент):
Тема 1: Саки. Источники
Вопрос 1: Первый письменный источник о саках?
Ответ 1: Древнеперсидские тексты
Вопрос 2: В каком году Дарий I пленил вождя тиграхауда?
Ответ 2: 519 до н.э. / 519

Тема 2: Саки. Политическая история
Вопрос 1: Как звали сакскую царицу, победившую Кира?
Ответ 1: Томирис

Выдай сразу готовый файл в этом формате, целиком, без обрамления."""
...
def grade_attempt_sync(attempt_id: int, tg_id: int, user_answers: dict):
    ...
    if att and att["passed"]:
        ...
        _rs.on_learning_event(att["user_tg_id"], None, lesson_id=att["lesson_id"])
    return _result_payload(att, lesson)
...
def _zachet_access_sync(lesson_id: int, tg_id: int):
    ...
    if not section:
        return None
    if not learning._subject_gate_ok_sync(section["subject_id"], tg_id) \
...
async def zachet_page(request):
    ...
    _sec = await asyncio.to_thread(db.fetchone, "SELECT subject_id FROM sections WHERE id=?",
                                   (lesson["section_id"],))
    if _sec and await asyncio.to_thread(_lg._needs_start_sync, _sec["subject_id"], tg_id):
        raise web.HTTPFound(f"/learn/{_sec['subject_id']}?start=required#start")
...
async def admin_zachet_import(request):
    ...
    field = data.get("bank_file")
    text = ""
    if field is not None and hasattr(field, "file"):
        raw = field.file.read()
        # убираем BOM (Windows/некоторые редакторы) — иначе первая строка не парсится
        text = raw.decode("utf-8-sig", errors="replace")
    else:
        text = data.get("bank_text") or ""
...
async def admin_zachet_settings(request):
    ...
    def _int(name, lo, hi, dflt):
        try:
            ...
            return dflt
    per = _int("per_attempt", 1, 500, 20)
    thr = _int("topic_threshold", 1, 100, 65)
    pass_pct = _int("pass_percent", 1, 100, 70)
