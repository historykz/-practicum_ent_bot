...
def _available_lessons_sync(tg_id, need_test: bool) -> list:
    ...
    # Число вопросов по всем тестам — одним запросом, а не по одному на урок
    qcount = {r["test_id"]: r["c"] for r in db.fetchall(
        "SELECT test_id, COUNT(*) AS c FROM questions GROUP BY test_id")}
    out = []
    for subj in learning._list_subjects_public_sync(tg_id):
        ...
        for lesson in learning._flatten_subject_lessons_sync(subj["id"]):
            ...
            items.append({
                ...
                "locked": locked,
                "qcount": qcount.get(lesson.get("test_id"), 0) if lesson.get("test_id") else 0,
            })
        if items:
...
async def cards_page(request: web.Request) -> web.Response:
    # Гостя не выкидываем: раздел открывается, просто без личных данных.
    # Раньше отсюда шёл редирект, и нажатие вкладки уводило на другой экран.
...
async def premium_page(request: web.Request) -> web.Response:
    ...
    def _state():
        ...
        qs = db.fetchone(
            ...
            "WHERE COALESCE(l.is_paid,0)=1 AND q.copied_from_question_id IS NULL")["c"]  # копии — не новые вопросы
