...
def _format_dt(iso_str: Optional[str]) -> Optional[str]:
    ...
    if not iso_str:
        return None
    dt = utils.parse_utc_naive(iso_str)
    if dt is None:
        return iso_str
    ...
    try:
        rows = db.fetchall(
            ...
            WHERE ta.user_id = ? AND ta.status = 'finished'
              AND ta.cloned_from_attempt_id IS NULL
...
def _safe_tests_done(user_id: int) -> int:
    try:
        return db.fetchone(
            ...
            "AND cloned_from_attempt_id IS NULL",
            (user_id,),
        )["c"]
    except Exception:
...
def _safe_game(tg_id: int, user_id: int) -> tuple:
    """Общий балл по предметам и достижения. Только для Премиума.

    Считается по тем же данным, что и всё остальное (test_attempts.score,
    прогресс уроков, user_achievements) — отдельной системы баллов нет.
    """
    try:
        if not utils.is_premium(user_id):
            return None, []
        from services import gamification as g
        return g.overall(tg_id, user_id), g.earned(user_id)
    except Exception:
        logger.exception("webapp: геймификация tg_id=%s", tg_id)
        return None, []
...
def _dashboard_data_sync(tg_id: int) -> Optional[dict]:
    ...
    _game = _safe_game(tg_id, user_id)
    ...
    return {
        ...
        "game_overall": _game[0],
        "game_achievements": _game[1],
    }
