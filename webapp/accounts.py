...
# Пауза между повторными отправками одного и того же конспекта
NOTE_RESEND_SECONDS = 600
...
def register_routes(app):
    ...
    app.router.add_post("/video/send/{lesson_id:\\d+}", send_video_to_owner)


# ===================== выдача конспекта владельцу аккаунта =====================

# (tg_id, lesson_id) -> когда последний раз слали видео: защита от серии нажатий
_video_sent_at: dict = {}
VIDEO_RESEND_SECONDS = 120


async def send_video_to_owner(request: web.Request) -> web.Response:
    """Отправить видео урока в Telegram ВЛАДЕЛЬЦА аккаунта — из мини-приложения.

    Раньше кнопка была ссылкой t.me?start=video_…: внутри Mini App она часто
    не срабатывала, и видео не приходило. Теперь видео уходит так же, как
    конспект: с сервера, через бота, с меткой ученика и запретом пересылки.
    """
    ...
    if not acc.can_reset_via_telegram(user):
        return web.json_response({
            ...
            "error": "К аккаунту ещё не привязан Telegram. Откройте бота — "
                     "аккаунт свяжется автоматически, и видео придёт в Telegram.",
            "bot_url": f"https://t.me/{config.WEB_BOT_USERNAME}"}, status=409)
    owner_tg = int(user["tg_id"])
    bot_url = f"https://t.me/{config.WEB_BOT_USERNAME}"
    key = (owner_tg, sc.orig_lesson_id(lesson_id))
    last = _video_sent_at.get(key, 0)
    if _time.time() - last < VIDEO_RESEND_SECONDS:
        left = int((VIDEO_RESEND_SECONDS - (_time.time() - last)) // 60) + 1
        return web.json_response({
            "ok": True, "already": True, "bot_url": bot_url,
            "error": f"Видео уже отправлено — откройте бота, оно там. "
    ...
    try:
        ...
        bot = Bot(token=config.BOT_TOKEN)
        try:
            sent = await send_lesson_video(bot, owner_tg, owner_tg, lesson_id)
        finally:
            await bot.session.close()
    except Exception as e:
        msg = str(e).lower()
        if "chat not found" in msg or "blocked" in msg or "initiate" in msg:
            return web.json_response({
                "ok": False, "need_bot": True, "bot_url": bot_url,
                "error": "Сначала откройте бота и нажмите Start — тогда видео придёт."},
                status=409)
        log.exception("send video to owner: %s", e)
        return web.json_response({"ok": False, "error": "Не удалось отправить видео."},
                                 status=500)
    if not sent:
        return web.json_response({"ok": False, "bot_url": bot_url,
                                  "error": "Видео недоступно для этого аккаунта."},
                                 status=403)
    _video_sent_at[key] = _time.time()
    if len(_video_sent_at) > 5000:
        _video_sent_at.clear()
    return web.json_response({"ok": True, "bot_url": bot_url})
...
async def send_note_to_owner(request: web.Request) -> web.Response:
    ...
    # Повторные нажатия. Раньше кнопку можно было жать сколько угодно, и бот
    # слал бы конспект снова и снова. Теперь между отправками одного урока —
    # пауза, а на повторное нажатие просто зовём в бота, где всё уже лежит.
    # Считаем по журналу отправок: он не чистится, поэтому ограничение
    # работает и после того, как сам конспект уже удалён из переписки по сроку.
    real_lesson = sc.orig_lesson_id(lesson_id)
    recent = await asyncio.to_thread(
        db.fetchone,
        "SELECT created_at FROM auth_events WHERE tg_id=? AND event='note_sent' "
        "AND details=? ORDER BY id DESC LIMIT 1",
        (owner_tg, f"lesson={real_lesson}"))
    if recent and recent.get("created_at"):
        ...
        if age < NOTE_RESEND_SECONDS:
            ...
            return web.json_response({
                ...
                "bot_url": f"https://t.me/{config.WEB_BOT_USERNAME}",
                "error": f"Конспект уже отправлен — откройте бота, он там. "
                         f"Прислать заново можно через {left} мин."})
