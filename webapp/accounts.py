...
async def send_note_to_owner(request: web.Request) -> web.Response:
    ...
    recent = await asyncio.to_thread(
        db.fetchone,
        "SELECT sent_at FROM note_messages WHERE chat_id=? AND lesson_id=? "
        "AND deleted=0 ORDER BY id DESC LIMIT 1",
        (owner_tg, sc.orig_lesson_id(lesson_id)))
    if recent and recent.get("sent_at"):
        try:
            age = (datetime.utcnow() - datetime.fromisoformat(
                str(recent["sent_at"]).replace(" ", "T"))).total_seconds()
        except ValueError:
