...
def test_results_sync(test_id: int) -> dict:
    ...
    rows = db.fetchall(
        """SELECT ta.id, ta.correct_answers, ta.wrong_answers, ta.skipped_answers,
                  ...
                  ta.is_first_attempt, ta.reward_tx_id,
           ...
           WHERE ta.test_id=? AND ta.status IN ('finished','aborted')
             AND COALESCE(ta.publication_id, 0)=0 AND ta.cloned_from_attempt_id IS NULL
    ...
    for r in rows:
        ...
        attempts.append({
            ...
            "first": bool(r["is_first_attempt"]) and r["status"] == "finished",
            "rewarded": bool(r["reward_tx_id"]),
            "duration": dur,
